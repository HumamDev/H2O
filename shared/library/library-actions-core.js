...
//
// Phase 7B - canonical pure module for Library action contracts and planning.
// Used by both native and Studio through a self-publishing IIFE on
// `window.H2O.LibraryActionsCore` and `window.H2O.Library.LibraryActionsCore`.
//
...
// no chrome/runtime calls, no IndexedDB, no archive calls, no events, no UI):
//
//   normalizeActionTarget(target, options?)
//   buildAddToLibraryPatch(target, options?)
//   buildSaveToFolderPlan(target, options?)
//   resolveOpenLinkedTarget(target, options?)
//   normalizeActionResult(result, options?)
//   diagnose()
