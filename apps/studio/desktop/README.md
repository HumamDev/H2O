...
| `scripts/prepare-dist.mjs` | The Studio-asset-copy script that produces `dist/` |
...
|---|---|---|
| `dist/` | `scripts/prepare-dist.mjs` | Tauri `frontendDist`; populated from `apps/extensions/chatgpt/chrome/prod/surfaces/studio/` via the prepare-dist copy + Tauri-friendly filename sanitization |
...
apps/extensions/chatgpt/chrome/prod/surfaces/studio/  (built by the chatgpt+chrome production builder; gitignored)
       ↓
apps/studio/desktop/scripts/prepare-dist.mjs
       ↓
...
the desktop shell = edit `src-tauri/` and/or `scripts/prepare-dist.mjs`
here.
