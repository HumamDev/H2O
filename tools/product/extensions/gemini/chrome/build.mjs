#!/usr/bin/env node
// @version 1.0.0  (Phase 8G-7: minimal gemini+chrome dev stub builder)
//
// Builds the H2O Gemini Chrome dev variant. Same Chrome-MV3 pattern as the
// claude+chrome builder from Phase 8G-5; only HOST + paths differ.
//
// Reads:
//   src/extensions/gemini/chrome/config/manifest.dev.json
//   src/extensions/gemini/chrome/scripts/content.js
//   config/extensions/gemini/chrome/keys.json
//
// Writes:
//   apps/extensions/gemini/chrome/dev/manifest.json
//   apps/extensions/gemini/chrome/dev/content.js
//   apps/extensions/gemini/chrome/dev/README.txt
//
// All output paths are gitignored. Output is deterministic; same inputs → same
// bytes. Deliberately self-contained: imports only node built-ins +
// tools/paths.mjs. No dependency on the chatgpt+chrome legacy pipeline.
...
function buildReadme(id) {
  return [
    "H2O Gemini Chrome dev stub",
    "",
    "Built by: tools/product/extensions/gemini/chrome/build.mjs",
    "Variant : " + variant,
    "Host    : " + HOST,
    "Browser : " + BROWSER,
    id ? "Chrome ID (key-derived): " + id : "Chrome ID: (no key found; path-derived fallback active)",
    "",
    "This is a Phase 8G-7 proof-of-chain stub. The content script logs to",
    "console + sets document.documentElement.dataset.h2oGeminiChromeDev='loaded'",
    "on gemini.google.com. No real Gemini integration is implemented yet.",
    "",
    "Source: src/extensions/gemini/chrome/",
...
function main() {
  ...
  console.log("[H2O gemini/chrome] built:", variant);
  console.log("[H2O gemini/chrome] out:   ", OUT_DIR);
  if (id) console.log("[H2O gemini/chrome] ext ID:", id);
}
