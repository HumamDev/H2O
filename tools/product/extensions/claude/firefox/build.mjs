#!/usr/bin/env node
// @version 1.0.0  (Phase 8G-8: minimal claude+firefox dev stub builder)
//
// Builds the H2O Claude Firefox dev variant. Clone of the Phase 8G-6
// chatgpt+firefox builder with HOST="claude"; same Firefox MV3 pattern
// (gecko_id-based identity, NOT Chrome SPKI-key-based).
//
// Reads:
//   src/extensions/claude/firefox/config/manifest.dev.json
//   src/extensions/claude/firefox/scripts/content.js
//   config/extensions/claude/firefox/keys.json
//
// Writes:
//   apps/extensions/claude/firefox/dev/manifest.json
//   apps/extensions/claude/firefox/dev/content.js
//   apps/extensions/claude/firefox/dev/README.txt
//
// Deliberately self-contained: imports only node built-ins + tools/paths.mjs.
// Does NOT depend on the chatgpt+chrome legacy pipeline or any other builder.
...
function buildReadme(gecko_id) {
  return [
    "H2O Claude Firefox dev stub",
    "",
    "Built by: tools/product/extensions/claude/firefox/build.mjs",
    "Variant : " + variant,
    "Host    : " + HOST,
    "Browser : " + BROWSER,
    gecko_id
    ...
    "",
    "This is a Phase 8G-8 proof-of-chain stub. The content script logs to",
    "console + sets document.documentElement.dataset.h2oClaudeFirefoxDev='loaded'",
    "on claude.ai. No real Claude Firefox integration is implemented yet.",
    "",
    "Source: src/extensions/claude/firefox/",
...
function main() {
  ...
  console.log("[H2O claude/firefox] built:", variant);
  console.log("[H2O claude/firefox] out:   ", OUT_DIR);
  if (gecko_id) console.log("[H2O claude/firefox] gecko_id:", gecko_id);
}
