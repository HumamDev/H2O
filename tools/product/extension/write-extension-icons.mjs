 ...
 *
 * Responsibilities:
 * - Prefer copying a ready icon pack from assets/chrome-dev-controls-icons/
...
function getReadyIconDirCandidates(options) {
  ...
  const candidates = [];
  const explicitReadyDirs = Array.isArray(options.readyIconDirs) ? options.readyIconDirs : [];
  const readyDirNames = Array.isArray(options.readyIconDirNames) && options.readyIconDirNames.length
    ? options.readyIconDirNames
    : DEFAULT_READY_DIR_NAMES;

  if (envDir) candidates.push(path.resolve(envDir));
  if (options.readyIconDir) candidates.push(path.resolve(options.readyIconDir));
  candidates.push(...explicitReadyDirs.map((dir) => path.resolve(String(dir))));

  for (const root of collectSearchRoots(options)) {
    for (const parts of readyDirNames) {
...
function usage(scriptName) {
  return [
    ...
    '- Ready icon pack mode uses assets/chrome-dev-controls-icons when present.',
  ].join('\n');
}
