...
const LEGACY_ALIAS_COMPAT = Object.freeze({
  "0C1d._Chat_Mechanisms_Router_.js": "1C1c._Outline_Mechanisms_Router_(_Unmount_&_Pagination_Integration)_.js",
  "0B1d._Chat_Mechanisms_Router_(_Integration)_.js": "1C1c._Outline_Mechanisms_Router_(_Unmount_&_Pagination_Integration)_.js",
  "1A2c._Chat_Pages_Controller_(MiniMap_Add-on)_.js": "1C1b._Thread_Pages_Controller_.js",
  "1E1a._Answer_Title_.js": "1C1a._Turn_Title_Bar_.js",
  "1C1a._Title_Bar_.js": "1C1a._Turn_Title_Bar_.js",
});
...
let compatLinked = 0;
let compatSkipped = 0;
for (const [legacyAlias, currentAlias] of Object.entries(LEGACY_ALIAS_COMPAT)) {
  const legacyPath = path.join(ALIAS_DIR, legacyAlias);
  const currentPath = path.join(ALIAS_DIR, currentAlias);
  if (!fs.existsSync(currentPath)) {
    compatSkipped++;
    continue;
  }
  try {
    const relCurrent = path.relative(path.dirname(legacyPath), currentPath);
    fs.symlinkSync(relCurrent, legacyPath);
    compatLinked++;
  } catch {
    try {
      fs.copyFileSync(currentPath, legacyPath);
      compatLinked++;
    } catch {
      compatSkipped++;
    }
...
console.log("[H2O] compat aliases linked/copied:", compatLinked);
if (compatSkipped) console.warn("[H2O] compat aliases skipped:", compatSkipped);
