...
if (failures.length === 0) {
  ...
  assertContains(chromeLiveBackgroundFile, 'operationType !== "change-folder-color"', 'Background folder metadata fallback is color-only besides create/rename');
  assertContains(chromeLiveBackgroundFile, 'invalid-folder-color', 'Background color fallback validates hex color');
  assertContains(chromeLiveBackgroundFile, 'folder-color-not-allowed', 'Background color fallback rejects unsafe targets');
  ...
  assertContains(chromeLiveBackgroundFile, 'background-color-fallback', 'External native owner response exposes color fallback status');
  ...
  assertContains(chromeLiveBackgroundFile, 'colorFallbackStatus', 'Chrome background diagnostics expose color fallback status');
  assertContains(chromeLiveBackgroundFile, 'colorResultCount', 'Chrome background diagnostics expose color fallback count');
