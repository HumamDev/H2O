#!/usr/bin/env node
//
// W3.4b-1-R2 renewed operator sacrificial write approval validator.
...
const approvalPath = 'release-evidence/2026-07-12/real-transport-w3-4b-1-r2-renewed-operator-approval.md';
...
const W34B3A_LIVE_EXECUTOR_COMMIT = '3048ab2dba3f4cbff4ec199dbb36093975659b52';
const W34B25_R2_READINESS_COMMIT = '1efd1dca9e10e0a061b7bdab8cd2e67f98a9a8c2';
const EXPIRED_APPROVAL_EXPIRY = '2026-07-10T16:00:00Z';
...
const CONCRETE_APPROVAL = 'I approve exactly one sacrificial probe object write: at most two create-only PUT requests to a single deterministic probe path, the second expected to fail with 412, one read-back GET, nothing else. No user data. No archive or fullBundle content. Expires 2026-07-15T20:00:00Z.';
...
for (const token of [
  ...
  W34B3A_LIVE_EXECUTOR_COMMIT,
  W34B25_R2_READINESS_COMMIT,
]) {
  mustContain(evidence, token, `anchor ${token}`);
}

mustContain(evidence, APPROVAL_WORDING, 'exact approval wording');
mustContain(evidence, CONCRETE_APPROVAL, 'concrete renewed approval instance');
...
assert.notEqual(expiryUtc, EXPIRED_APPROVAL_EXPIRY, 'expired approval window must not be reused');
...
for (const token of [
  `Expires ${EXPIRED_APPROVAL_EXPIRY}.`,
  ...
  mustNotContain(flatEvidence, token, `forbidden live/expired claim ${token}`);
}
...
console.log(JSON.stringify({
  ok: true,
  validator: 'validate-real-transport-w3-4b-1-r2-renewed-operator-approval',
