#!/usr/bin/env node
//
// W3.4b-2 write-grade receipt/token-hash evidence validator.
...
const evidencePath = 'release-evidence/2026-07-07/real-transport-w3-4b-2-write-grade-receipt.md';
const receiptPath = 'release-evidence/2026-07-07/real-transport-w3-4b-2-write-grade-receipt-core.json';
const approvalPath = 'release-evidence/2026-07-07/real-transport-w3-4b-1-operator-sacrificial-write-approval.md';

const EXPECTED_RECEIPT_HASH = 'sha256:267688e94be9359d83cebfbd6ce4d2ecd5259808d15ab5d818973f90973d1fb7';
...
for (const commit of COMMITS) {
  ...
  mustContain(JSON.stringify(receipt), commit, `receipt anchor ${commit}`);
}
...
for (const [pattern, label] of [
  ...
  assert.ok(!pattern.test(JSON.stringify(receipt)), `receipt: ${label} found`);
}

console.log(JSON.stringify({
  ok: true,
  validator: 'validate-real-transport-w3-4b-2-write-grade-receipt',
  evidencePath,
  receiptPath,
  receiptHash,
