#!/usr/bin/env node
//
// W3.4b-2.5-R5 final pre-invocation readiness validator.
...
const evidencePath = 'release-evidence/2026-07-12/real-transport-w3-4b-2-5-r5-pre-invocation-readiness.md';
...
function inspectAppLocalRegistry() {
  ...
  assert.equal(fs.existsSync(consumedMarker), false, 'R5 receipt already has a consumed marker');
}
...
for (const token of [
  ...
  'transportReady:true',
  'W3.4b-3B-R5 can proceed automatically',
]) {
...
mustContain(receiptEvidence, `receiptCoreHash: \`${RECEIPT_HASH}\``, 'W3.4b-2-R5 receipt evidence hash');
mustContain(receiptEvidence, 'receiptConsumed:false', 'W3.4b-2-R5 unconsumed receipt');
mustContain(receiptEvidence, 'receiptInvoked:false', 'W3.4b-2-R5 uninvoked receipt');
mustContain(receiptEvidence, 'payloadHashMatchesExecutorDeterministicSentinel:true', 'W3.4b-2-R5 payload binding');
...
console.log(JSON.stringify({
  ok: true,
  validator: 'validate-real-transport-w3-4b-2-5-r5-pre-invocation-readiness',
  evidencePath,
