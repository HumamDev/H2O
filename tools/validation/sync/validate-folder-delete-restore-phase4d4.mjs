...
assertContains(runner, 'inspectChromeLatestForRequest', 'exact chrome-latest request inspection');
assertContains(runner, 'chrome-delete-request-not-exported', 'exact Chrome delete request missing export blocker');
assertContains(runner, 'chromeLatestHasRequest', 'Chrome latest exact request diagnostic');
assertContains(runner, 'chromeLatestRequestPath', 'Chrome latest request path diagnostic');
assertContains(runner, 'chromeLatestRequestCount', 'Chrome latest request count diagnostic');
assertContains(runner, 'desktopDeleteRequestImported', 'Desktop exact request import diagnostic');
assertContains(runner, 'desktop-delete-request-not-imported', 'Desktop exact request missing blocker');
assertContains(runner, 'desktop-delete-request-wrong-status', 'Desktop exact request wrong status blocker');
assertContains(runner, 'findExactRequest', 'exact request id/folder id matching helper');
...
assertContains(runner, 'countDiagnostics', 'same-surface count diagnostics helper');
assertContains(runner, 'chatCountDecreased', 'chat count decrease detection');
assertContains(runner, 'snapshotCountDecreased', 'snapshot count decrease detection');
assertContains(runner, 'chatDelta === null || chatDelta >= 0', 'chat delete guard must allow same-surface increases');
assertContains(runner, 'snapshotDelta === null || snapshotDelta >= 0', 'snapshot delete guard must allow same-surface increases');
assertContains(runner, 'baselineChromeChatCount', 'top-level Chrome baseline chat count');
assertContains(runner, 'baselineDesktopChatCount', 'top-level Desktop baseline chat count');
assertContains(runner, 'finalChromeChatCount', 'top-level Chrome final chat count');
assertContains(runner, 'finalDesktopChatCount', 'top-level Desktop final chat count');
assertContains(runner, 'baselineChromeSnapshotCount', 'top-level Chrome baseline snapshot count');
assertContains(runner, 'baselineDesktopSnapshotCount', 'top-level Desktop baseline snapshot count');
assertContains(runner, 'finalChromeSnapshotCount', 'top-level Chrome final snapshot count');
assertContains(runner, 'finalDesktopSnapshotCount', 'top-level Desktop final snapshot count');
assertContains(runner, 'chromeChatCountDelta', 'top-level Chrome chat delta');
assertContains(runner, 'desktopChatCountDelta', 'top-level Desktop chat delta');
assertContains(runner, 'chromeSnapshotCountDelta', 'top-level Chrome snapshot delta');
assertContains(runner, 'desktopSnapshotCountDelta', 'top-level Desktop snapshot delta');
assertContains(runner, 'steps: stepResults', 'steps alias for count read diagnostics');
