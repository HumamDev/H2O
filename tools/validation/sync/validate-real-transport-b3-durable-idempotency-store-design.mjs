#!/usr/bin/env node
//
// Real-transport B3 - durable idempotency store design validator.
//
// Proves the B3 design: it respects the B2 (09bf7701), B1 (b2e10531), and B8+B7 (26e6241b) designs and the gap review
// (d2bea4c0); B3 is design-only (no durable idempotency store / key / failure codes are implemented in product source);
// no real transport authorization is introduced; NO raw endpoint / credential / path / bundle-body value appears in the
// design; productSyncReady:false and transportReady:false remain; fullBundle.v3 remains deferred/not-started; Chat
// Saving CAS remains blocked/deferred; and no cleanup/a950 mutation authority is introduced. Evidence/validator-only;
...
const designPath = 'release-evidence/2026-07-01/real-transport-b3-durable-idempotency-store-design.md';
...
// ---------------------------------------------------------------------------
// (1) Design content anchors: verdict, 9 areas, store namespace, lifecycle, failure modes, B4-B6, next lane.
// ---------------------------------------------------------------------------
for (const token of [
  'B3 DURABLE REAL-TRANSPORT IDEMPOTENCY STORE IS DESIGNED (DESIGN / SPECIFICATION ONLY)',
  'design/specification evidence + validator slice only',
  '09bf7701', 'b2e10531', '26e6241b', 'd2bea4c0', '15a33852',
  // 1. ownership
  '## 1. Durable Idempotency Record Ownership',
  "`h2o:sync:real-transport-idempotency:v1`",
  'h2o.desktop.sync.real-transport-idempotency-record.v1',
  'the idempotency store is DESKTOP-authority only',
  'SEPARATE from the relay\noutbox'.replace(/\n\s*/g, ' '),
  '## 2. Idempotency Key Material',
  'candidatePayloadHash',
  'candidateBundleHash',
  'endpointRefHash', 'remoteRootRefHash', 'peerIdentityBindingHash', 'credentialRefHash',
  ...
  'pending state must not auto-write',
  'not blind retry',
  'disabled kill switch blocks resume',
  // 5. failure modes
  'real-transport-b3-idempotency-record-missing',
  'real-transport-b3-idempotency-record-corrupted',
  'real-transport-b3-target-hash-mismatch',
  'real-transport-b3-payload-hash-mismatch',
  'real-transport-b3-approval-hash-mismatch',
  'real-transport-b3-kill-switch-token-mismatch',
  'real-transport-b3-sequence-constraint-mismatch',
  'real-transport-b3-duplicate-changed-payload-target',
  // 6. privacy
  'idempotencyRecordHashOnly: true',
  'rawBundleBodyStored: false',
  'casKeysExposed: false',
  'touchChatSavingCas: false',
  // 7. B7/B8
  'the idempotency store alone does not make `transportReady` true',
  'the idempotency store does not replace the real approval',
  'the idempotency store does not replace the kill switch',
  // 8. a950 / 9. fullBundle
...
for (const blocker of [
  ...
  assertIncludes(flat, blocker, `B4-B6 open blocker ${blocker}`);
}
...
// ---------------------------------------------------------------------------
// (2) NO raw endpoint / credential / path / bundle-body values anywhere in the design.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// (4) DESIGN-ONLY: no B3 store / codes implemented in source; existing durable stores unchanged; invariants intact.
// ---------------------------------------------------------------------------
assertNotIncludes(transportGates, 'real-transport-b3-', 'B3 failure codes must not be implemented in source (design-only)');
assert.ok(!fs.existsSync(path.join(root, 'src-surfaces-base/studio/sync/real-transport-idempotency.tauri.js')),
  'B3 durable idempotency store must not be created as a source module (design-only)');
assert.ok(!relayOutbox.includes('real-transport-idempotency'), 'B3 store must not be minted into the relay outbox source (design-only)');
for (const token of [
...
// the existing durable stores this design references remain (unchanged, separate)
...
console.log(JSON.stringify({
  schema: 'h2o.studio.transport.real-transport-b3-durable-idempotency-store-design.validator.v1',
  lane: 'real-webdav-cloud-relay-transport',
  phase: 'b3-durable-idempotency-store-design',
  evidence: designPath,
  verdict: 'B3_DESIGNED_DESIGN_ONLY_REAL_TRANSPORT_STILL_BLOCKED',
  b2DesignRespected: '09bf7701',
  b1DesignRespected: 'b2e10531',
  b8b7DesignRespected: '26e6241b',
  gapReviewRespected: 'd2bea4c0',
  designOnly: true,
  durableIdempotencyStoreImplemented: false,
  realTransportAuthorizationIntroduced: false,
  rawEndpointCredentialPathBundlePresent: false,
  proposedStoreNamespace: 'h2o:sync:real-transport-idempotency:v1',
  realTransportApprovalAccepted: false,
  transportReady: false,
  productSyncReady: false,
  remainingBlockers: ['B4', 'B5', 'B6'],
  fullBundleV3Started: false,
  chatSavingCasBlocked: true,
  cleanupAuthorityIntroduced: false,
  recommendedNextLane: 'B4-real-enqueue-outbox-boundary-design-only',
}, null, 2));
console.log('PASS validate-real-transport-b3-durable-idempotency-store-design');
