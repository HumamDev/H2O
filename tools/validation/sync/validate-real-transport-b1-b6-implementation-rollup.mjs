...
function assertW1bAwareWiring(rel, label) {
  ...
  if (!w1bPresent) {
    ...
    assertNotIncludes(packStudio, path.basename(rel), `${label} pre-W1b not packed by pack-studio.mjs`);
    return;
  }
...
// B1-B6 were intentionally non-wired before W1b. After W1b, they may be
// registered only through the expected evaluate-only loader entries.
for (const spec of Object.values(modules)) {
  assertW1bAwareWiring(spec.path.replace('src-surfaces-base/studio/', ''), path.basename(spec.path));
}
