...
(() => {
   ...
   */
  const chatAtlasRevealDiagnostics = (...a) => chatAtlasCoreApi()?.revealDiagnostics?.(...a) ?? null;
  const chatAtlasRevealRunOneShot = (...a) => chatAtlasCoreApi()?.revealRunOneShot?.(...a);
  const chatAtlasRevealReconcileTick = (...a) => chatAtlasCoreApi()?.revealReconcileTick?.(...a);
  const chatAtlasRevealSupersede = (...a) => chatAtlasCoreApi()?.revealSupersede?.(...a);
  // Two read-only Reveal facts; the Reveal state object itself stays in 0A3a.
  const chatAtlasRevealTransactionState = () => chatAtlasCoreApi()?.getRevealTransactionState?.() ?? '';
  const chatAtlasRevealReadinessTarget = () => chatAtlasCoreApi()?.getRevealReadinessTarget?.() ?? null;
  ...
  function chatAtlasCoreLedgerMembers() {
    const m = chatAtlasCoreApi()?.getLedgerMembers?.();
    return Array.isArray(m) ? m : [];
  }

  function chatAtlasCoreLedgerVersion() {
    return Number(chatAtlasCoreApi()?.getLedgerVersion?.() || 0);
  }
  ...
  const buildChatAtlasLedgerCanonicalRecords = (...args) => {
    const out = chatAtlasCoreApi()?.buildLedgerCanonicalRecords?.(...args);
    return Array.isArray(out) ? out : [];
  };

  const scheduleChatAtlasLedgerFlush = (...args) => chatAtlasCoreApi()?.scheduleLedgerFlush?.(...args);
   ...
   * Every entry here is read-only for the Ledger; it writes none of this state.
   * Milestone 2B deletes most of these as their implementations move to 0A3a.
   */
  const CHAT_ATLAS_HOST_SURFACE = {
    ...
    chatAtlasQuestionEditPlanState,
    ...
    getCompleteTurnIndexProjectionStatus,
    listTurnRecords,
    // The Ledger detects the return-to-canonical case but must not write
    // central Atlas state itself; H2O Core stays the only writer of the
    // trusted-intent field.
    clearTrustedSelectedPathIntent() {
      completeTurnIndexAuthorityState.trustedSelectedPathIntent = null;
    },
