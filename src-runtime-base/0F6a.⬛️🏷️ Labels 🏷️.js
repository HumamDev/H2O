...
(() => {
  ...
  /* R4.7.3 — R4.6.3 per-element org gate retired alongside the
   * labels sidebar UI. Code moved to:
   ...
   * is itself retired. The label CRUD entrypoints (createLabel,
   * renameLabel, deleteLabel) and the per-turn lbsc-chip-color UI
   * stay live in 0F6a and were never gated. */
  ...
  /* R4.7.3 — openLabelActionsPop retired alongside the labels
   * sidebar UI. Code moved to:
   *   retired-features/native-library-ui/0F6a-labels-ui/labels-sidebar.js
   * (Block 2). This was the row context-menu popup that hosted the
   * sidebar's rename + delete UI handlers. The rename/delete CRUD
   * entrypoints (function renameLabel, function deleteLabel) stay
   * live in 0F6a — Studio MV3 fallback (S0Z1g) calls
   * H2O.Labels.renameLabel / deleteLabel directly via the MOD API
   * exposure, bypassing this UI shell. */
  ...
  /* R4.7.3 — makeFallbackSidebarHeader + prepareLabelsSection
   * retired alongside the labels sidebar UI. Code moved to:
   *   retired-features/native-library-ui/0F6a-labels-ui/labels-sidebar.js
   * (Block 3 + Block 4). The section render path that set
   * data-cgxui="lbsc-root" is now archived. */

  /* R4.7.3 — buildLabelsSection retired alongside the labels
   * sidebar UI. Code moved to:
   *   retired-features/native-library-ui/0F6a-labels-ui/labels-sidebar.js
   * (Block 5). This was the main sidebar renderer — section header
   * + per-type groups + per-label rows + "Manage labels" + "Label
   * current chat" action rows + row context-menu + inline previews.
   * Replaced by Desktop Studio's S0Z1g labels section + S0F1m label
   * editor modals + S0F1n batch toolbar + S0F6b actions.
   *
   * A no-op stub remains because MOD.buildSection (in the MOD API
   * exposure block) forwards to this function. Returning null
   * preserves the legacy call shape without mounting any DOM. */
  function buildLabelsSection(projectsSection, existingSection = null, reason = 'build') {
    return null;
  }

  /* R4.7.3 — Sidebar lifecycle functions retired alongside the
   * labels sidebar UI. Code moved to:
   *   retired-features/native-library-ui/0F6a-labels-ui/labels-sidebar.js
   * (Block 6). The seven functions below are kept as no-op stubs
   * because they have external callers we must NOT break in this
   * slice:
   *   - rerenderLabelsSection — called from createLabel,
   *     renameLabel, deleteLabel, afterLabelMutation, setTypeVisible,
   *     and the show-counts toggle.
   *   - syncLabelSidebarActiveState — called from workspace
   *     viewer paths (closeViewer / openLabelsViewer /
   *     openLabelViewer); R4.7.4 will retire those callers.
   *   - scheduleEnsure / ensureInjected — exposed via the MOD API
   *     and from boot late-init.
   * Each stub returns the legacy-compatible "nothing happened"
   * value so callers behave as if the sidebar simply didn't render
   * (which matches the post-R4.6.4 default-flip behavior). */
  function activePageLabelKey() { return ''; }
  function syncLabelSidebarActiveState(_reason = 'sync') { /* no-op (R4.7.3) */ }
  function scheduleLabelSidebarActiveSync(_reason = 'sync') { /* no-op (R4.7.3) */ }
  function rerenderLabelsSection(_reason = 'rerender') { /* no-op (R4.7.3) */ }
  function ensureSidebarObserver(_root) { /* no-op (R4.7.3) */ }
  function scheduleEnsure(_reason = 'schedule') { /* no-op (R4.7.3) */ }
  function ensureInjected(_reason = 'ensure') { return false; }
