...
function DATA_renderWebDAVCredsInline(panel) {
  ...
  function DATA_openWebDAVModal(){
    ...
    modal.addEventListener('click', (e) => {
      if (e.target === modal) { try { modal.remove(); } catch {} }
    });
    ...
    // cleanup key handler when modal removed
    ...
    function readCredsFromUI(){
      ...
      return { baseUrl, url: baseUrl, server: srv, port: prt, username, password, remember, folder };
    }
    ...
    async function doTest(label){
      ...
      try {
        ...
        renderStatus(res?.ok ? `Test OK ✅ ${res?.status || ''}`.trim()
          : `Test failed ❌ (${(res && ('status' in res)) ? String(res.status) : ''}) ${res?.message || res?.step || res?.reason || 'unknown error'}`.trim());
      } catch (e) {
        renderStatus(`Test failed ❌ ${e?.message || e}`);
      }
    }
    ...
    async function doLink(){
      ...
      try {
        ...
        renderStatus(res?.ok ? `Linked ✅ (folder: ${res.folderUrl || ''})`
          : `Link failed ❌ (${(res && ('status' in res)) ? String(res.status) : ''}) ${res?.message || res?.step || res?.reason || 'unknown error'}`.trim());
      } catch (e) {
        renderStatus(`Link failed ❌ ${e?.message || e}`);
      }
    }
    ...
    // Click outside card closes (and prevents bubbling into other overlays)
    modal.addEventListener('pointerdown', (e) => {
      if (e.target === modal) { try { modal.remove(); } catch {} }
    }, true);
    // NOTE: stop bubbling to the page, but do NOT capture (capture would block button clicks)
    modal.addEventListener('click', (e) => {
      e.stopPropagation();
    });
