...
/* ───────────────────────────── ⬜️ 8) SYNC (unified) + Legacy WebDAV shim ───────────────────────────── */
  ...
  function SYNC_saveCreds(creds) {
    ...
    store.setJSON(SYNC_KEY_CREDS, safe);
    ...
    EV_emit(EV_SYNC_WEBDAV_CHANGED, { linked: true, url: safe.url, root: safe.root, username: safe.username });
    return safe;
  }
  ...
  function LIVE_installNativeStorageHooksOnce() {
    try {
      ...
      SP.setItem = function(k, v) {
        try {
          if (LIVE_ENABLED_CACHE && !LIVE_REMOTE_APPLY_GUARD) {
            LIVE_trackLocalWrite(String(k || ''), 'native.setItem');
            ...
            LIVE_tryAutoPush({ reason: 'native', key: String(k || ''), source: 'native.setItem' }, false);
          }
        } catch {}
      ...
      SP.removeItem = function(k) {
        try {
          if (LIVE_ENABLED_CACHE && !LIVE_REMOTE_APPLY_GUARD) {
            LIVE_trackLocalWrite(String(k || ''), 'native.removeItem');
            LIVE_tryAutoPush({ reason: 'native', key: String(k || ''), source: 'native.removeItem' }, false);
          }
        } catch {}
...
function SYNC_setTarget(target) {
  ...
  function SYNC_getTarget() {
    ...
    if (t.type === 'webdav') {
      ...
      return { ...t, linked: !!creds, hasPassword: !!(SYNC_getPassword(creds) || '') };
    }
    return t;
  }
...
function SYNC_fileUrl(creds, filename) {
  try {
    const P = P_webdav();
    if (P?.fileUrl) return P.fileUrl(creds, filename);
  } catch {}
...
async function SYNC_getRemoteHash(creds, filename) {
  try {
    const P = P_webdav();
    if (!P?.getJSON) return null;
    const got = await P.getJSON(creds, filename);
  ...
  sync.webdav.setCreds = (creds) => {
    ...
    merged.password = String(merged.password || merged.pass || '');
    merged.rememberPassword = !!(merged.rememberPassword || merged.remember) && merged.password.length > 0;
  ...
  sync.webdav.isLinked = () => !!SYNC_loadCreds();
  ...
  sync.webdav.test = async () => {
    ...
    const P = P_webdav(); if (!P?.test) return { ok:false, status:0, message:'provider missing' }; const res = await P.test(creds);
  ...
  sync.providers.webdav.test = async () => sync.webdav.test();
  sync.providers.webdav.putJSON = async (filename, obj) => {
    const c = SYNC_loadCreds(); if (!c) throw new Error('no creds');
    const pass = SYNC_getPassword(c);
    const P = P_webdav(); if (!P?.putJSON) throw new Error('provider missing'); return P.putJSON({ ...c, password: pass }, filename, obj);
  };
  sync.providers.webdav.getJSON = async (filename) => {
    const c = SYNC_loadCreds(); if (!c) throw new Error('no creds');
    const pass = SYNC_getPassword(c);
    const P = P_webdav(); if (!P?.getJSON) throw new Error('provider missing'); return P.getJSON({ ...c, password: pass }, filename);
  };
  ...
  async function SYNC_push(kind, opts = {}) {
    ...
    const P = P_webdav();
    if (!P?.putJSON && !P?.putText) throw new Error('provider missing');
    const put = await (P.putText ? P.putText(creds, filename, payloadText) : P.putJSON(creds, filename, payload));
  ...
  async function SYNC_pull(kind, opts = {}) {
    ...
    const P = P_webdav(); if (!P?.getJSON) throw new Error('provider missing'); const got = await P.getJSON(creds, filename);
    const last = SYNC_loadLast();
