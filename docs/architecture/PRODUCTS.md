...
| 12 | **Dev server (Python HTTP, port 5500)** | `cd apps/dev-server && python3 serve.py 5500` | `apps/dev-server/alias/` (regenerated by `tools/loader/make-aliases.mjs`), `apps/dev-server/dev_output/proxy/_paste-pack.ext.txt` (regenerated by `tools/loader/make-ext-proxy-pack.mjs`), `scripts/` (referenced via alias-farm symlinks) | HTTP at `http://127.0.0.1:5500/alias/{aliasId}?v={ts}` + `/dev_output/proxy/_paste-pack.ext.txt` | `H2O_SERVER_DIR=apps/dev-server` (default; env-overridable) | n/a | `npm run dev:check` (alias farm + proxy pack consistency) |
