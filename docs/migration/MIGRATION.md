...
> **STRUCTURAL CHANGES TO DATE**: many — inside `h2o-source/` (the git repo): `cockpit-pro-site/` → `apps/site/` (3B); 8 chrome-ext outputs → `apps/extensions/chatgpt/chrome/` (4C-B); `../h2o-dev-server/` → `apps/dev-server/` (5B); `apps/studio-desktop/` → `apps/studio/desktop/` (6B); `apps/studio-mobile/` → `apps/studio/mobile/` (6C-3); 16 future-structure placeholder READMEs added (7D). Outside the git tree (within outer `cockpit-pro/`): Phase 1B deleted dead `cockpit-pro/apps/`; Phase 7F moved `h2o-source/references/` (52 MB) to `cockpit-pro/references/`; Phase 7G-1 moved `h2o-source/{tmp,plans,s-files}/` to outer + consolidated `AGENTS.md` + `CLAUDE.md` + `tasks-manual-*.md` into `cockpit-pro/operator-notes/`; deleted root-level stale `identity-provider.local.json` and recurring `.DS_Store`. **`h2o-source/` remains the git toplevel; `.git` has not moved.** Outer `cockpit-pro/` is the **outer-workspace-shell** (§2.1) — a non-git host directory containing `.claude/` + `operator-notes/` + `plans/` + `references/` + `s-files/` + `tmp/` + `h2o-source/`.
...
└── h2o-source/                                           ← THE git repo
    ...
    ├── archive/      55+ daily snapshots  (gitignored; deferred to 7G-2 outer-shell move)
...
>
> **Phase 7G-1 note**: 3 additional gitignored folders (`tmp/`, `plans/`, `s-files/`) and 4 operator notes (`AGENTS.md`, `CLAUDE.md`, `tasks-manual-1.md`, `tasks-manual-2.md` — consolidated into `cockpit-pro/operator-notes/`) were moved out of `h2o-source/`. Stale duplicate `identity-provider.local.json` at root deleted (canonical is `config/local/identity-provider.local.json`). Zero tooling changes; zero tracked-file diff (all moved/deleted items were untracked + gitignored).

### 2.1 Outer-workspace-shell concept (formalized in Phase 7F, expanded in Phase 7G-1)

The outer `cockpit-pro/` directory is **not** a git repository. It is a non-git host directory that "wraps" the git repo (`h2o-source/`) with workspace-but-not-source artifacts. As of Phase 7G-1 it contains:
...
2. **Outer-shell artifacts have ZERO tooling/config consumers inside `h2o-source/` (UNLESS gated by an `H2O_*_DIR` env var).** Phase 7F (references/) + Phase 7G-1 (tmp/, plans/, s-files/, operator notes) all satisfy the strict zero-consumer constraint. Phase 7G-2/7G-3 will introduce the env-var-gated variant for `archive/` (H2O_ARCHIVE_DIR) and `meta/` (H2O_META_DIR), where tooling consumers exist but resolve their target via `tools/paths.mjs` constants that honor the env override.
...
5. **Future outer-shell candidates** under active planning: `archive/` (Phase 7G-2 — pending `archive/.state/lastVersions.json` tracked-sentinel resolution + `H2O_ARCHIVE_DIR` paths.mjs/archive-snapshot.mjs refactor); `meta/` (Phase 7G-3 — `H2O_META_DIR` paths.mjs/commit-auto.mjs refactor). **Must stay inside** h2o-source/: `node_modules/` (npm convention), `build/` (Phase 4C-B symlink bridge), `assets/` (build-critical icon packs), `artifacts/` (Phase 7D README-only placeholder), `.bump/` (release pipeline working dir; auto-recreated), `scripts/`, `supabase/`.
...
_Last updated: 2026-05-19 (Phase 8A-1 complete — Chrome manifest key hardening. Generated 8 RSA-2048 key pairs in-memory, retained only public keys (committed in new `config/extension-keys.json`), discarded private keys. New module `tools/product/extension/chrome-extension-keys.mjs` provides `getExtensionKey(variant)` + `deriveVariantFromOutDir()`. Wired key injection into 3 manifest builders (`chrome-live-manifest.mjs`, `make-chrome-ops-panel-extension.mjs`, `pack-desk.mjs`). All 8 unpacked extension manifests now include a stable `"key"` field; Chrome IDs are now path-agnostic. **OAuth-Google ID rotated**: `amjponmninhldimbkdkfhcmclmjfbibi` (path-derived, pre-8A-1) → `ogcjkeaiicglflamhjaaimdhphjlgkbb` (key-derived). Updated 2 hardcoded validators + 3 doc sites in `docs/identity/IDENTITY_PHASE_3_0_SUPABASE_PREP.md`. Phase 3_9c validator now PASSES (was failing pre-8A-1 due to manifest-missing-identity-permission); Phase 3_5b still fails on a pre-existing path-literal docs-drift issue unrelated to 8A-1 (confirmed identical via git stash round-trip). NEW deterministic ext build hash baseline `77bd47cf904c6e4b2b9062a90d8e2faaa62393d79eebfe2f105a217a64a46e8a` (dev-controls variant, in a `chrome-ext-dev-controls`-named OUT_DIR; supersedes the pre-8A-1 `c4db3bdb…6f14` baseline due to the intentional manifest.key addition). **OPERATOR ACTION REQUIRED before Phase 8B (h2o-source rename)**: update Supabase Auth Redirect URL to `https://ogcjkeaiicglflamhjaaimdhphjlgkbb.chromiumapp.org/identity/oauth/google` + remove the old `amjponmninhldimbkdkfhcmclmjfbibi.chromiumapp.org/...` entry. After that update, Phase 8B is safe — all 8 IDs survive any future folder rename. **Carried operator items** (non-blocking, unchanged from 7G-1 unless noted): (1) reload H2O unpacked extensions in `chrome://extensions` (NEW: IDs have changed from path-derived to key-derived; the new IDs are recorded in `config/extension-keys.json`); (2) confirm Cloudflare Pages dashboard "Root directory" reads `apps/site`; (3) **NEW**: Supabase Auth Redirect URL update (see above). Future structural moves remain deferred: Phase 7G-2 (`H2O_ARCHIVE_DIR` + `archive/`), Phase 7G-3 (`H2O_META_DIR` + `meta/`), Phase 8B (h2o-source rename, now unblocked pending operator Supabase update), apps/studio/web scaffold, packages/shared-library extraction, h2o-source flatten, 3_5b docs drift fix.)._
