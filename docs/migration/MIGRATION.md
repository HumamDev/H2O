...
_Last updated: 2026-05-17 (Phase 6Z complete — `apps/` consolidation arc closed at a stable checkpoint. 5 apps under `apps/`: site, extensions/chatgpt/chrome, dev-server, studio/desktop, studio/mobile. 243 tracked source files; 0 build/cache artifacts tracked anywhere under apps/. Deterministic ext build hash `edf2baa…7af772` stable across **19 consecutive phase boundaries** (0J→2B→2D→3B→3D→3E→4B-1→4B-1b→4B-2→4C-B→4C-C→5B→5C→6B→6B-C→6C-1→6C-3→6C-4→6Z). Recommended pause point. Future structural moves (apps/studio/web, packages/shared-library, h2o-source flatten, identity-docs cleanup) can each be evaluated independently against this baseline. **Carried operator items** (non-blocking): (1) reload H2O unpacked extensions in `chrome://extensions` from legacy `build/chrome-ext-*` symlink paths; (2) confirm Cloudflare Pages dashboard "Root directory" reads `apps/site`.)._
