...
_Last updated: 2026-05-19 (Phase 7G-1 complete — outer-workspace-shell expansion. Moved 3 folders (`tmp/` 488 KB, `plans/` 164 KB, `s-files/` 199 MB) + 4 operator notes (AGENTS.md, CLAUDE.md, tasks-manual-1.md, tasks-manual-2.md) from `h2o-source/` to outer `cockpit-pro/{tmp,plans,s-files,operator-notes}/`. Deleted root-level stale `identity-provider.local.json` (canonical is `config/local/identity-provider.local.json`) + recurring `.DS_Store`. Zero tooling/path/package.json/.gitignore changes. Zero tracked-file diff for moves and deletes (all 8 items untracked + gitignored). Outer-workspace-shell now contains 6 outer items + h2o-source/. Deterministic ext build hash `c4db3bdb…6f14` byte-identical pre vs post — **23 consecutive phase boundaries stable**. Validators: validate-loader-order exit 0; dev:check exit 1 (pre-existing archiveWorkbench drift, unchanged from 7F). **Pre-existing unrelated working-tree diff** in `apps/studio/mobile/ios/H2OStudio/H2OStudio.entitlements` was present before 7G-1 started and was NOT touched (appears to be from prior Xcode regeneration; needs separate operator decision). **Carried operator items** (non-blocking): (1) reload H2O unpacked extensions in `chrome://extensions` from legacy `build/chrome-ext-*` symlink paths; (2) confirm Cloudflare Pages dashboard "Root directory" reads `apps/site`. Future structural moves remain deferred: Phase 7G-2 (`H2O_ARCHIVE_DIR` + `archive/` outer-shell — blocked on `archive/.state/lastVersions.json` tracked-sentinel question); Phase 7G-3 (`H2O_META_DIR` + `meta/` outer-shell); apps/studio/web scaffold; packages/shared-library extraction; h2o-source flatten; historical identity-docs cleanup.)._
