...
_Last updated: 2026-05-17 (Phase 5C complete — Phase 5B dev-server absorption fully closed. 5B `8d7734d` shipped the move + path-default flip + tasks.json/gitignore updates; 5C (this commit) verified operator cutover (`serve.py` PID 29204 running from `apps/dev-server` on port 5500 with full CORS+no-cache headers preserved), all path-helper outputs, all tracking state, deterministic build hash `edf2baa…7af772` stable across **13 consecutive phase boundaries** (0J→2B→2D→3B→3D→3E→4B-1→4B-1b→4B-2→4C-B→4C-C→5B→5C), and all workspace builds. Outer `cockpit-pro/` now contains only `h2o-source/`. **Carried operator items**: (1) reload H2O unpacked extensions in `chrome://extensions` from legacy `build/chrome-ext-*` symlink paths — IDs should remain stable per Phase 4C-A2 empirical test; (2) confirm Cloudflare Pages dashboard "Root directory" reads `apps/site`.)._
