...
_Last updated: 2026-05-17 (Phase 5B complete — `../h2o-dev-server/` absorbed into `apps/dev-server/`. `serve.py` now git-tracked; `alias/` + `dev_output/` gitignored. `tools/paths.mjs::SERVER_ROOT_DEFAULT` flipped to in-tree path; `H2O_SERVER_DIR` env override preserved. **URL contract (`http://127.0.0.1:5500/...`) preserved** — no manifest change, no extension reinstall, deterministic ext build still byte-identical to baseline `edf2baa…7af772` (12 consecutive phase boundaries stable). Outer `cockpit-pro/` now contains only `h2o-source/`. **Operator manual steps**: (1) kill any HTTP server still bound to port 5500 from the old sibling path, then restart from new location: `cd apps/dev-server && python3 serve.py 5500`; (2) reload H2O unpacked extensions in `chrome://extensions` from legacy `build/chrome-ext-*` symlink paths — IDs should remain stable per Phase 4C-A2 empirical test (carried from Phase 4C); (3) confirm Cloudflare Pages dashboard "Root directory" reads `apps/site` — carried from Phase 3.)._
