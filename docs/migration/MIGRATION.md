...
_Last updated: 2026-05-17 (Phase 6B-C complete — Phase 6B Studio Desktop relocation fully closed at the repository level. 6B `b50f019` shipped the structural move + workspaces update + import-depth fix; 6B-C (this commit) verified all 11 structural checks + all 9 validators including deterministic ext build hash `edf2baa…7af772` stable across **15 consecutive phase boundaries** (0J→2B→2D→3B→3D→3E→4B-1→4B-1b→4B-2→4C-B→4C-C→5B→5C→6B→6B-C), workspace builds via both folder-cd and package-name (`-w @h2o/studio-desktop`), and cross-app workspace builds (site, studio-mobile). Phase 6C (mobile move) deferred — recommended split into 6C-1 (identity-validator path pre-refactor) + 6C-2 (folder move + metro.config.js depth fix + iOS Pods regeneration) to manage ~89 hardcoded path refs across 9 identity validators. **Carried operator items** (unchanged): (1) reload H2O unpacked extensions in `chrome://extensions` from legacy `build/chrome-ext-*` symlink paths; (2) confirm Cloudflare Pages dashboard "Root directory" reads `apps/site`.)._
