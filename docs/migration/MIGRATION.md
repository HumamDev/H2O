...
| **4C-C** — Phase 4C-B closeout verification | ✅ complete | `migration-phase-4C-C-complete` | (this commit) | **VERIFICATION + DOC-ONLY closeout.** Confirmed: 4C-B commit `4d32d24` pushed; `extensionBuildDir()` default returns `apps/extensions/chatgpt/chrome/<variant>` for all 8 variants; opt-in `H2O_EXT_BUILD_ROOT` still works; all 8 legacy `build/chrome-ext-*` paths are symlinks with correct relative targets; every symlink resolves to an existing real directory under `apps/extensions/chatgpt/chrome/`; 0 build artifacts tracked under either root; deterministic ext build hash matches the Phase 0J baseline `edf2baa…7af772` (11 consecutive phase boundaries stable); `dev:check`, `validate-loader-order`, `npm -w cockpit-pro-site run build`, `cd apps/site && npm run build`, `apps/studio-desktop prepare-dist`, `apps/studio-mobile tsc --noEmit --skipLibCheck` — all pass. **Operator manual step (carried)**: reload H2O unpacked extensions in `chrome://extensions` from legacy `build/chrome-ext-*` symlink paths; Chrome IDs should remain stable per Phase 4C-A2 empirical test. |

**Latest stabilized checkpoint**: `migration-phase-4C-C-complete` (this phase). All inside-git work through Phase 4C-B is in place and `origin/main` is self-consistent post-4C-C verification. The chrome extension subtree is now homed at `apps/extensions/chatgpt/chrome/` with `build/chrome-ext-*` symlinks providing backwards-compat. The full migration chain — paths.mjs centralization + extension-build determinism + workspaces + lockfile + site move + extension move — is the most cohesive structural state since the migration started. Phase 4C is fully closed at the repository level; the only remaining operator-side items are doc-side (Cloudflare dashboard "Root directory" confirmation carried from Phase 3, and Chrome unpacked-extension reload from symlink paths after this push).

---

## 2. Repo Topology (as of 2026-05-17, post Phase 4C-B)
...
_Last updated: 2026-05-17 (Phase 4C-C complete — Phase 4C is fully closed at the repository level. 4C-B `4d32d24` shipped the structural move + symlink bridge + default flip; 4C-C (this commit) verified all path-helper outputs, all 8 symlinks, gitignore/tracking state, deterministic build hash (`edf2baa…7af772` stable across 11 consecutive phase boundaries 0J→2B→2D→3B→3D→3E→4B-1→4B-1b→4B-2→4C-B→4C-C), and all workspace builds (site, studio-desktop, studio-mobile). **Operator manual steps**: (1) reload H2O unpacked extensions in `chrome://extensions` from legacy `build/chrome-ext-*` symlink paths — IDs should remain stable per Phase 4C-A2 empirical test; (2) confirm Cloudflare Pages dashboard "Root directory" reads `apps/site` — carried from Phase 3.)._
