...
| **8F-1** — PRODUCTS.md architecture map | ✅ complete | `migration-phase-8F-1-complete` | `ee3555a` | **DOCS-ONLY.** Created tracked file `docs/architecture/PRODUCTS.md` (a single-page operator index of "what produces what"). Documents: (a) conceptual 4-quadrant layout (runtime source / build tooling / generated products / reusable libs) — "platform" treated as conceptual grouping only, NO physical folder; (b) master product table covering 12 runnable/generated products (8 chrome extension variants + Studio Desktop + Studio Mobile + Marketing site + Dev server) with builder/source-inputs/env-vars/output/extension-ID/validation per product; (c) 7 embedded sub-products shipped INSIDE extensions (identity bundle, identity surfaces, studio surfaces, billing bundle, popup, folder-bridge, pilot-observer, icons); (d) ASCII flow diagram (platform → tools → apps); (e) operator quick-reference command table; (f) 6 architectural rules; (g) "Do not move yet" section anchoring each forbidden move to its specific coupling; (h) 5 future cleanup candidates (none authorized). NO filesystem moves; NO source code changes; NO package.json changes. Build hash UNCHANGED `77bd47cf...6e8a`. Validators: validate-loader-order exit 0; identity-3_5b/3_9b/3_9c exit 0; dev:check exit 1 (pre-existing archiveWorkbench drift). |

| **8G-1** — multi-host / multi-browser architecture audit | ✅ complete | — (audit-only) | `ee3555a` | **AUDIT-ONLY.** Comprehensive architectural review for adding new hosts (claude, gemini) + new browsers (firefox) without disturbing the chatgpt+chrome legacy. Classified every top-level folder + every tool by host/browser specificity (~80% chatgpt+chrome-coupled, ~15% genuinely shared, ~5% browser-specific but host-agnostic). Compared 4 architecture options; selected the source-vs-output-separated pattern: `src/extensions/<host>/<browser>/` for new source + `apps/extensions/<host>/<browser>/<variant>/` for generated outputs (canonical since 4C-B + 8E-3) + `tools/product/extensions/<host>/<browser>/` for builders + `packages/host-adapters/<host>/` + `packages/browser-adapters/<browser>/` for graduated shared logic. Defined: glossary, source-vs-generated boundary, multi-host scaling strategy, multi-browser scaling strategy (Chrome SPKI key-derived IDs vs Firefox `browser_specific_settings.gecko.id`), shared incubator (`src/extensions/_shared/`) with A→B graduation rules, tooling reorganization plan, risk table per migration, 8-phase roadmap (8G-2 scaffolding through 8G-7+ template-driven combos), new-product checklist, validation checklist, rollback strategy. Critical constraint: scripts/+surfaces/+config/ stay frozen at the top level forever (147 file couplings). Recommended: Phase 8G-2 placeholder scaffolding first. |
| **8G-2** — multi-host / multi-browser placeholder scaffolding | ✅ complete | `migration-phase-8G-2-complete` | (this commit) | **DOCS + EMPTY-FOLDER SCAFFOLDING.** Created the future folder skeleton for new hosts + new browsers without touching any runtime code. New tracked content: 30 `.gitkeep` files (empty-folder placeholders); 16 README.md files explaining the per-host/per-browser layout; 1 comprehensive architecture doc `docs/architecture/MULTI_HOST_ARCHITECTURE.md` (the canonical multi-host/multi-browser reference); PRODUCTS.md cross-reference update. New folders created (all empty or README-only): `src/extensions/_shared/{browser,dom,overlay,storage}/` (cross-host incubator); `src/extensions/{chatgpt/firefox, claude/{chrome,firefox}, gemini/{chrome,firefox}}/{scripts,surfaces,config,assets}/` (5 host+browser source roots, each with 4 subdirs); `tools/product/extensions/{_shared, chatgpt/firefox, claude/{chrome,firefox}, gemini/{chrome,firefox}}/` (builder homes mirror the source layout); `config/extensions/` (per-host/per-browser config home). **Zero functional impact**: no scripts/ touched; no supabase/ touched; no package.json changes; no `.gitignore` changes; no tools/ code changes; no apps/ output changes. Deterministic ext build hash UNCHANGED at `77bd47cf904c6e4b2b9062a90d8e2faaa62393d79eebfe2f105a217a64a46e8a`. Validators: validate-loader-order exit 0; identity-3_5b/3_9b/3_9c exit 0; dev:check exit 1 (pre-existing archiveWorkbench drift, unchanged). The skeleton is now ready to receive new-host/new-browser source code in future phases (8G-3+ tooling reorganization audit, 8G-4 chatgpt+chrome builder moves into `tools/product/extensions/chatgpt/chrome/`, 8G-5 first chatgpt+firefox stub, 8G-6 first claude+chrome stub, 8G-7 template-from-stubs, 8G-8+ remaining combos). |

| **8G-3** — chatgpt+chrome tooling reorganization audit | ✅ complete | — (audit-only) | `3a88029` | **AUDIT-ONLY.** Inventoried 19 .mjs files across `tools/product/extension/` (11), `tools/product/desk/` (1), `tools/dev-controls/ops-panel/` (1), `tools/dev-controls/popup/` (6). Mapped every internal import (5 files have cross-folder deps), every external consumer (~50 path-string refs across `.vscode/tasks.json`, `tools/dev/dev-all.mjs`, 32 identity validators, 11 docs/READMEs), and every refs-to-leave-historical (MIGRATION.md narrative + scripts/0F1e doc comment + 3_9b/3_9c content-check anchor literals at "build/chrome-ext-..."). Classified each file: 11 chrome-live build files → `tools/product/extensions/chatgpt/chrome/`; pack-desk + pack-ops-panel (renamed from make-chrome-ops-panel-extension) co-located; 6 popup generators → `popup/` subfolder. Recommended target architecture: mirror the `apps/extensions/<host>/<browser>/` shape. Risk-assessed each move; produced 6-stage implementation plan (A: git mv, B: internal imports, C: external consumers, D: docs, E: validate, F: MIGRATION.md). Build-hash invariance proven by `tools/paths.mjs` name-agnostic resolution + content-agnostic build pipeline. |
| **8G-4** — chatgpt+chrome tooling move | ✅ complete | `migration-phase-8G-4-complete` | (this commit) | **STRUCTURAL: tooling reorganization.** Moved 19 .mjs files from 4 source locations into `tools/product/extensions/chatgpt/chrome/` (+ a `popup/` subfolder for the 6 popup generators). One file renamed: `make-chrome-ops-panel-extension.mjs` → `pack-ops-panel.mjs` for naming consistency. Removed 4 empty source folders (`tools/product/extension/`, `tools/product/desk/`, `tools/dev-controls/ops-panel/`, `tools/dev-controls/popup/`, plus the now-empty `tools/dev-controls/` parent). Removed the 8G-2 placeholder `.gitkeep` from the destination since real files now exist there. **Internal imports** (5 files, 13 line-edits): build-chrome-live-extension.mjs, pack-desk.mjs, pack-ops-panel.mjs, chrome-extension-keys.mjs, chrome-live-build-context.mjs — all relative imports updated for the new depth (paths.mjs is now 4 levels up; sibling tools/product/{studio,identity,billing}/ are 3 levels up; popup/ generators are now `./popup/` siblings of build-chrome-live-extension.mjs). One non-import path computation fix: `chrome-live-background.mjs` line 7 hardcoded `path.resolve(SCRIPT_DIR, "../../../packages/studio-core/...")` updated to 5-levels-up form. **External consumers** (~50 path-string updates across 38+ files via batch sed): `.vscode/tasks.json` (5 sites); `tools/dev/dev-all.mjs` (3 sites); `tools/validation/identity/run-identity-release-gate.mjs` (6 sites); 32 identity validators (path-string constants + `read()` calls); 11 docs+READMEs (`MULTI_HOST_ARCHITECTURE.md`, `PRODUCTS.md`, `IDENTITY_PHASE_{2_8,3_0,5_0E}*.md`, `apps/studio/desktop/README.md`, `apps/studio/desktop/scripts/prepare-dist.mjs`, `apps/extensions/chatgpt/firefox/README.md`, `packages/{extension-core,host-adapters/chatgpt}/README.md`, `tools/product/extensions/README.md`); `tools/loader/make-ext-proxy-pack.mjs` (1 doc-comment site). **Intentionally left**: MIGRATION.md historical narrative (60+ refs); scripts/0F1e doc comment (frozen); 3_9b/3_9c validator content-check anchors at `"build/chrome-ext-dev-controls-oauth-google"` literals in `run-identity-release-gate.mjs` doc-comments and section 15.28 of `IDENTITY_PHASE_3_0_SUPABASE_PREP.md` (validator-required). **Deterministic ext build hash UNCHANGED** at `77bd47cf904c6e4b2b9062a90d8e2faaa62393d79eebfe2f105a217a64a46e8a` (verified by rebuild at new location). All 8 chatgpt+chrome variants build successfully from new paths; all 8 manifests still contain valid Phase-8A-1 keys; Chrome IDs unchanged. Validators: validate-loader-order exit 0; identity-3_5b/3_9b/3_9c all exit 0; dev:check exit 1 (pre-existing archiveWorkbench drift, unchanged). |

| **8G-5** — claude+chrome minimal stub (first new-host proof-of-chain) | ✅ complete | `migration-phase-8G-5-complete` | (this commit) | **STRUCTURAL: first new-host extension end-to-end.** Built the minimal claude+chrome dev variant to prove the new multi-host architecture chain works: source under `src/extensions/claude/chrome/` → builder at `tools/product/extensions/claude/chrome/build.mjs` → generated unpacked extension at `apps/extensions/claude/chrome/dev/` (gitignored) → Chrome `Load unpacked` on `claude.ai`. **Created tracked files**: `src/extensions/claude/chrome/scripts/content.js` (minimal stub: console-log + non-visible `data-h2o-claude-chrome-dev` marker on `document.documentElement`); `src/extensions/claude/chrome/config/manifest.dev.json` (MV3 manifest template — claude.ai host permissions only, no OAuth, no identity, no background worker, no storage); `tools/product/extensions/claude/chrome/build.mjs` (self-contained builder; reads source + manifest template + keys.json; writes manifest.json + content.js + README.txt; deterministic; NO dependency on chatgpt+chrome legacy pipeline); `config/extensions/claude/chrome/keys.json` (Phase 8A-1-style RSA-2048 public key + derived Chrome ID for the `dev` variant). **Updated**: 3 READMEs (`src/extensions/claude/chrome/README.md`, `tools/product/extensions/claude/chrome/README.md`, both with build instructions + manual-load procedure), `.gitignore` (added `apps/extensions/claude/chrome/**` + `!.../README.md` exception), `docs/architecture/PRODUCTS.md` (claude+chrome added as product #9; subsequent product numbers shifted by 1), `docs/architecture/MULTI_HOST_ARCHITECTURE.md` (status note updated). **New Chrome extension ID**: `pdhldppkggpefneaemodleadcgpmpmnc` (key-derived; path-agnostic). **Removed**: 3 `.gitkeep` placeholders that are no longer needed (`src/extensions/claude/chrome/{scripts,config}/.gitkeep`, `tools/product/extensions/claude/chrome/.gitkeep`). **Zero touches** to: scripts/, surfaces/, config/{dev-order.tsv,loader-deps.json,loader-tiers.json}, supabase/, existing chatgpt+chrome builders, package.json. **chatgpt+chrome build hash UNCHANGED** at `77bd47cf904c6e4b2b9062a90d8e2faaa62393d79eebfe2f105a217a64a46e8a` (verified post-claude-build); all 8 chatgpt+chrome Phase-8A-1 manifest keys preserved. Validators: validate-loader-order exit 0; identity-3_5b/3_9b/3_9c exit 0; dev:check exit 1 (pre-existing archiveWorkbench drift). **Operator manual validation step**: `chrome://extensions` → enable Developer mode → Load unpacked → select `apps/extensions/claude/chrome/dev/` → open `https://claude.ai/` → DevTools console must show `[H2O Claude Chrome dev stub] loaded on claude.ai`. |

**Latest stabilized checkpoint**: `migration-phase-8G-5-complete` (this phase). The multi-host/multi-browser architecture is now proven end-to-end: the claude+chrome dev variant builds from `src/extensions/claude/chrome/` through `tools/product/extensions/claude/chrome/build.mjs` to a loadable Chrome unpacked extension at `apps/extensions/claude/chrome/dev/`. The chatgpt+chrome legacy is untouched and continues to build deterministically at hash `77bd47cf…6e8a`. The same template can now be applied to chatgpt+firefox, claude+firefox, gemini+chrome, and gemini+firefox in future phases (8G-6+). Future structural work remains as previously documented. The repo now has a complete future-product skeleton for chatgpt+firefox, claude+chrome, claude+firefox, gemini+chrome, gemini+firefox — including source homes (`src/extensions/<host>/<browser>/`), builder homes (`tools/product/extensions/<host>/<browser>/`), config homes (`config/extensions/<host>/<browser>/`), and a cross-host incubator (`src/extensions/_shared/`) — all documented in [docs/architecture/MULTI_HOST_ARCHITECTURE.md](../architecture/MULTI_HOST_ARCHITECTURE.md). The chatgpt+chrome legacy (`scripts/`, `surfaces/`, `config/`) remains frozen at the top level and continues to be the source for the 8 production extension variants. Source-vs-generated boundary is enforced: `git ls-files src/` returns only handwritten source + READMEs; `git ls-files apps/extensions/` returns only README placeholders. Three of the largest gitignored local-state trees live outside the git repo: `references/` (52 MB, Phase 7F), `s-files/` (199 MB, Phase 7G-1), `archive/` (1.8 GB, Phase 8C). Chrome extension IDs are key-derived (Phase 8A-1) and survive folder renames. Future structural work — Phase 7G-3 (`H2O_META_DIR` + `meta/` outer-shell move), Phase 8G-3 (tooling reorg audit), Phase 8G-4 (chatgpt+chrome builder moves), Phase 8G-5+ (first new-host stubs), Studio web scaffold, h2o-source flatten — all remain deferred until explicit operator authorization.
