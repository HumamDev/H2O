...
_Last updated: 2026-05-17 (Phase 6C-3 complete — Studio Mobile relocated from `apps/studio-mobile` to `apps/studio/mobile`. The `apps/studio/<platform>/` convention is now FULLY established. 6C-1 `31678f9` pre-refactored the 9 validator coupling; 6C-2 audited 4 depth-coded files; 6C-3 (this commit) atomically moved 211 tracked files + applied the 4 depth-math fixes + updated workspaces + ran `npm install` + ran `LANG=en_US.UTF-8 pod install` (110 pods installed at new iOS location). All identifiers (package name `studio-mobile`, bundle ID `com.anonymous.studio-mobile`, Expo slug, URL scheme) UNCHANGED. Deterministic ext build hash `edf2baa…7af772` stable. **Carried operator items**: (1) reload H2O unpacked extensions in `chrome://extensions` from legacy `build/chrome-ext-*` symlink paths; (2) confirm Cloudflare Pages dashboard "Root directory" reads `apps/site`.)._
