...
> **STRUCTURAL CHANGES TO DATE**: many — inside the git repo: `cockpit-pro-site/` → `apps/site/` (3B); 8 chrome-ext outputs → `apps/extensions/chatgpt/chrome/` (4C-B); `../h2o-dev-server/` → `apps/dev-server/` (5B); `apps/studio-desktop/` → `apps/studio/desktop/` (6B); `apps/studio-mobile/` → `apps/studio/mobile/` (6C-3); 16 future-structure placeholder READMEs added (7D); stable manifest `"key"` injection across all 8 chrome variants (8A-1). Outside the git tree (within outer `cockpit-pro/`): Phase 1B deleted dead `cockpit-pro/apps/`; Phase 7F moved `references/` (52 MB) out; Phase 7G-1 moved `{tmp,plans,s-files}/` out + consolidated `AGENTS.md`+`CLAUDE.md`+`tasks-manual-*.md` into `cockpit-pro/operator-notes/`; deleted root-level stale `identity-provider.local.json` and recurring `.DS_Store`. **Phase 8B renamed the git-toplevel folder itself**: `cockpit-pro/h2o-source/` → `cockpit-pro/h2o-cp-source/`. `.git/` moved with the folder; `.git` has NOT been relocated; outer `cockpit-pro/` is NOT a git repo. Outer `cockpit-pro/` is the **outer-workspace-shell** (§2.1) — a non-git host directory containing `.claude/` + `operator-notes/` + `plans/` + `references/` + `s-files/` + `tmp/` + `h2o-cp-source/`.
...
### 2.1 Outer-workspace-shell concept (formalized in Phase 7F, expanded in Phase 7G-1, renamed in Phase 8B)

The outer `cockpit-pro/` directory is **not** a git repository. It is a non-git host directory that "wraps" the git repo (`h2o-cp-source/`, renamed from `h2o-source/` in Phase 8B) with workspace-but-not-source artifacts. As of Phase 8B it contains:
...
_Last updated: 2026-05-19 (Phase 8B complete — git repo-root folder renamed from `h2o-source/` to `h2o-cp-source/`. Plain `mv` (atomic, single-inode); `.git/` moved with the folder; outer `cockpit-pro/` is NOT a git repo. Operator pre-action satisfied: Supabase Auth Redirect URL updated to `https://ogcjkeaiicglflamhjaaimdhphjlgkbb.chromiumapp.org/identity/oauth/google` before phase began. Tracked diff (3 files): `package.json` name → `h2o-cp-source`; `package-lock.json` regenerated by `npm install`; `apps/studio/mobile/ios/Podfile.lock` regenerated by `pod install`. Caches regenerated (NOT tracked): `apps/studio/desktop/src-tauri/target/` (2.3 GB) cleared; iOS `Pods/` reinstalled with `LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8` (110 pods, exit 0). All workspace symlinks were already relative and survived the rename intrinsically. Deterministic ext build hash `77bd47cf904c6e4b2b9062a90d8e2faaa62393d79eebfe2f105a217a64a46e8a` UNCHANGED (REPO_ROOT is name-agnostic; manifest contents depend on config/extension-keys.json, not paths). All 8 Chrome extension IDs UNCHANGED (key-derived since 8A-1 — exactly what 8A-1 was for). Validators: validate-loader-order exit 0; identity-3_9c exit 0; studio-desktop prepare-dist exit 0; cockpit-pro-site Vite build exit 0; studio-mobile tsc exit 0 (via npx). dev:check exit 1 (pre-existing archiveWorkbench drift, unchanged). 5 cosmetic doc-comment refs to "h2o-source" intentionally deferred (Phase 8A audit "optional" list). **Carried operator items**: (1) reload all 8 H2O unpacked extensions in `chrome://extensions` from the new paths `cockpit-pro/h2o-cp-source/build/chrome-ext-*` — IDs will match the Phase 8A-1 key-derived values; (2) confirm Cloudflare Pages dashboard "Root directory" still resolves; (3) if VS Code is open at the old path, reopen at `cockpit-pro/h2o-cp-source/`. Pre-existing carried items unchanged. Future structural work — Phase 7G-2/7G-3, Studio web, packages/shared-library, doc-comment cleanups (8B carry-over), 3_5b docs drift fix, h2o-source flatten — remain deferred.)._
