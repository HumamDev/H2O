// ==UserScript==
// @h2o-id      1a5.quote.badges.minimap.plugin
// @name         1A5.🔴🗨️ Quote Badges (MiniMap Plugin) 🗺️
// @namespace    H2O.Prime.CGX.MiniMap.QuoteBadges
// @version      1.3.0
...
.${CLS_MM_BTN} .cgxui-mm-qto[data-on="1"]{
  ...
  function buildMaps(force=false){
    ...
    if (turns.length){
      for (const t of turns){
        ...
        const qEl = t?.qEl || null;
        if (!qEl || !qEl.querySelectorAll) continue;

        const qboxes = qEl.querySelectorAll('.cgxui-qswr-quoteBox');
      ...
      const qboxes = document.querySelectorAll('.cgxui-qswr-quoteBox');
      qboxes.forEach(qb => {
        const fromAIdx = parseFromAIdx(qb);
        if (!fromAIdx) return;
  ...
  function bindQuoteEventsOnce(){
    ...
    const onQuoteEvt = (e) => {
      // invalidate cache + ask MiniMap to rebuild (safe even if core not loaded yet)
      QB_CACHE.stamp = 0;
      try { W.H2O_scheduleMiniMapRebuild?.(`quote:${e?.type || 'evt'}`); } catch {}
      // Also attempt a cheap sync pass (helps when rebuild is throttled)
      try { scheduleSyncAllButtons(); } catch {}
    };

    W.addEventListener('h2o:quote:pending', onQuoteEvt, true);
    W.addEventListener('h2o:quote:chip',    onQuoteEvt, true);
    W.addEventListener('evt:h2o:minimap:ready', onQuoteEvt, true);
    W.addEventListener('h2o:minimap:ready', onQuoteEvt, true);
    W.addEventListener('evt:h2o:minimap:phase', onQuoteEvt, true);
    W.addEventListener('h2o:minimap:phase', onQuoteEvt, true);
  }
