// ==UserScript==
// @h2o-id      z1a1e.minimap.context.menu
// @name         1B2.🟥🖱️ MiniMap Context Menu 🧩🗺️
// @namespace    H2O.Prime.CGX.MiniMapCtxMenu
// @version      0.1.0
...
(() => {
  ...
  function boot() {
    ...
    cleanup.add(on(window, 'mousedown', (e) => {
      if (!STATE.menuEl) return;
      const inside = (e?.target && STATE.menuEl.contains(e.target));
      if (!inside) hideMenu();
    }, true));
