...
(() => {
  ...
  function requestNoButtonsRebuild(reason = 'ensure-ui') {
    const refs = getRefs();
    if (!(refs?.root && refs?.panel)) return false;
    if (countMiniMapButtons(refs) > 0) return false;
    if (!hasAssistantAnswersInDom()) return false;
    const now = Date.now();
    if ((now - Number(state.noButtonsRebuildAt || 0)) < 1500) return false;
    state.noButtonsRebuildAt = now;
    try {
      MM_core()?.scheduleRebuild?.(`shell:no-buttons:${String(reason || 'ensure-ui')}`);
      return true;
    } catch {
      return false;
    }
