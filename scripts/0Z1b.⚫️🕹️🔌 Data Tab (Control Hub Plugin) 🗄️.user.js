...
// @name               0Z1b.⚫️🕹️🔌 Data Tab (Control Hub Plugin) 🗄️
...
function boot(api){
  ...
  if (typeof api.registerPlugin !== 'function') return false;
    ...
    const DATA_CONTROLS = [
      ...
      },
      {
        ...
        buttons:[
          { label:'Open Reader', primary:false, action: DATA_openArchiveReaderAction },
  ...
  function DATA_getArchiveBootApi(){
    ...
    const required = ['captureNow', 'openReader', 'listSnapshots', 'loadSnapshot', 'exportBundle', 'importBundle'];
  ...
  function DATA_renderArchiveWorkspace(ctx){
    ...
    const toolDefs = [
      ...
      ['Open Reader', '', async () => DATA_openArchiveReaderAction()],
    ];
  ...
  async function DATA_openArchiveReaderAction(){
    ...
    if (!ab) return { message: DATA_archiveModuleMissingMessage() };
    const chatId = String(H2O.util?.getChatId?.() || '');
    await ab.openReader(chatId);
    return { ok: true, message: 'Opened Archive Reader.' };
  }
