...
(() => {
  ...
  function DOM_QT_getPrevAssistantForUserMsg(userMsgEl) {
    const root = DOM_QT_getConversationRoot() || document;
  ...
  function CORE_QT_installQuoteOriginCapture() {
    ...
    // Observe only composer root; fallback to body only when composer root is unavailable.
...
function CORE_QT_boot() {
  ...
  const startMO = () => {
    ...
    const mo = new MutationObserver((muts) => {
      ...
      if (needChip) DOM_QT_tryCaptureComposerQuote(DOM_QT_getComposerRoot() || document);
      if (needScan) scheduleScan('mo');
      if (needRepair) scheduleResolve('mo:repair', { forceFull: true });
    });

    // Observe only conversation root; fallback to body only when conversation root is unavailable.
    mo.observe(root, { childList: true, subtree: true });
    st.mo = mo;
