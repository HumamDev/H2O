...
(() => {
  ...
  const DPANEL_RAIL_ITEMS = Object.freeze([
    ...
    { view:'navigator', title:'Navigator', color:'#D47A38', txt:'V' },       // 🟠
    { view:'context', title:'Context', color:'#6740A8', txt:'C' },       // 🟣
    { view:'slot7', title:'Capture Box', color:'#C05C95', txt:'P' },     // 🌸
    { view:'slot8', title:'Finder', color:'#3FA7D6', txt:'F' },          // 🔷
  ]);
...
/** @helper Sidebar controller (rail-aware). */
      ...
      <div class="${CSS_DPANEL_CLS_HDR}" ${ATTR_DPANEL_CGXUI}="${UI_DPANEL_HDR}">
        <div class="${CSS_DPANEL_CLS_TITLE_DD}" ${ATTR_DPANEL_VIEW_DD}="1" ${ATTR_DPANEL_CGXUI_STATE}="">
          ...
          <div class="${CSS_DPANEL_CLS_MENU}" ${ATTR_DPANEL_VIEW_MENU}="1" aria-hidden="true" ${ATTR_DPANEL_CGXUI}="${UI_DPANEL_MENU}">
            ...
            <button type="button" class="${CSS_DPANEL_CLS_MENU_ITEM}" ${ATTR_DPANEL_SET_VIEW}="navigator">Navigator</button>
            <button type="button" class="${CSS_DPANEL_CLS_MENU_ITEM}" ${ATTR_DPANEL_SET_VIEW}="context">Context</button>
            <button type="button" class="${CSS_DPANEL_CLS_MENU_ITEM}" ${ATTR_DPANEL_SET_VIEW}="capture">Capture Box</button>
            <button type="button" class="${CSS_DPANEL_CLS_MENU_ITEM}" ${ATTR_DPANEL_SET_VIEW}="finder">Finder</button>
          </div>
        </div>
  ...
  /** @helper Resolve a human title for the current Dock view. */
  function CORE_DP_getViewTitle(view) {
    ...
    const tab = Dock.getTab?.(v) || Dock.tabs?.[v] || null;
  ...
  function CORE_DP_syncHeaderForView() {
    ...
    if (titleText) titleText.textContent = CORE_DP_getViewTitle(S.view);
  ...
  function CORE_DP_render() {
    ...
    const fallbackTitle = CORE_DP_getViewTitle(S.view);
    if (fallbackTitle && fallbackTitle !== S.view) {
      S.listEl.innerHTML = `<div class="${CSS_DPANEL_CLS_EMPTY}">${UTIL_DP_escapeHtml(fallbackTitle)} Tab script not loaded.</div>`;
      return;
    }
