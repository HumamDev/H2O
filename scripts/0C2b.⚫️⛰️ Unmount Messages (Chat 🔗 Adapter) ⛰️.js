// ==UserScript==
// @h2o-id             0c2b.unmount.messages.chat.adapter
...
(() => {
  ...
  const SEL_UNMOUNTM_ANSWER_TITLE_BAR = '[data-cgxui="atns-answer-title"][data-cgxui-owner="atns"]';
  const SEL_UNMOUNTM_ANSWER_TITLE_ICON = '[data-cgxui="atns-answer-title-icon"][data-cgxui-owner="atns"]';
  const SEL_UNMOUNTM_RESPONSE_ACTIONS = '[aria-label="Response actions"], [data-testid="response-actions"]';
  ...
  const CFG_UNMOUNTM_COMMAND_BAR_RETRY_MS = 350;
  const CFG_UNMOUNTM_COMMAND_BAR_RETRY_MAX_ATTEMPTS = 40;
...
`;
  ...
  function CORE_UM_getSharedTurnRecordForGroup(group) {
    if (group?.sharedTurnRecord?.turnId) return group.sharedTurnRecord;
  ...
  function CORE_UM_syncRuntimeIdentity(el, uid, turnNo) {
    if (!el?.dataset) return false;
    UTIL_UM_migrateLegacyAttrs(el);

    const nextUid = UTIL_UM_normalizeId(uid);
    if (nextUid) el.dataset[ATTR_UNMOUNTM_H2O_UID] = nextUid;

    const nextTurnNo = Math.max(0, Number(turnNo || 0) || 0);
    if (nextTurnNo > 0) {
      el.dataset[ATTR_UNMOUNTM_H2O_NUM] = String(nextTurnNo);
      DOM_UM_refreshUnmountedPlaceholder(el);
    }
    return true;
  }

  function CORE_UM_collectRecordAliasIds(record, items = []) {
    const out = new Set();
    CORE_UM_addAliasId(out, record?.turnId || '');
    CORE_UM_addAliasId(out, record?.qId || '', { turnVariant: true });
    CORE_UM_addAliasId(out, record?.primaryAId || '', { turnVariant: true, assistantTurnVariant: true });

    for (const answerId of Array.isArray(record?.answerIds) ? record.answerIds : []) {
      CORE_UM_addAliasId(out, answerId, { turnVariant: true, assistantTurnVariant: true });
    }
    for (const item of Array.isArray(items) ? items : []) {
      CORE_UM_addAliasId(out, item?.uid || '', {
        turnVariant: true,
        assistantTurnVariant: item?.role === STR_UNMOUNTM_ROLE_A,
      });
    }
    return Array.from(out);
  }

  function CORE_UM_buildTurnGroupsFromRuntime() {
    const api = CORE_UM_getTurnRuntimeApi();
    if (!api || typeof api.listTurnRecords !== 'function') return null;

    let records = null;
    try { records = api.listTurnRecords() || []; } catch (_) { records = null; }
    if (!Array.isArray(records) || !records.length) return null;

    const out = [];
    for (const record of records) {
      const turnNo = Math.max(1, Number(record?.turnNo || record?.idx || record?.index || 0) || 0);
      if (!turnNo) continue;

      const items = [];
      const turnId = UTIL_UM_normalizeId(record?.turnId || '') || `turn:${turnNo}`;
      const qEl = record?.live?.qEl || record?.qEl || null;
      const qId = UTIL_UM_normalizeId(record?.qId || '') || turnId;
      if (qEl?.isConnected) {
        CORE_UM_syncRuntimeIdentity(qEl, qId, turnNo);
        items.push({ el: qEl, uid: qId, num: String(turnNo), role: STR_UNMOUNTM_ROLE_Q });
      }

      const answerIds = Array.isArray(record?.answerIds) && record.answerIds.length
        ? record.answerIds.slice()
        : [record?.primaryAId || record?.answerId || ''];
      const answerElsRaw = Array.isArray(record?.live?.answerEls) && record.live.answerEls.length
        ? record.live.answerEls.slice()
        : (record?.live?.primaryAEl || record?.primaryAEl ? [record?.live?.primaryAEl || record?.primaryAEl] : []);
      const answerEls = answerElsRaw.filter((el) => !!el?.isConnected);

      for (let i = 0; i < answerEls.length; i += 1) {
        const answerEl = answerEls[i];
        const answerId = UTIL_UM_normalizeId(answerIds[i] || answerIds[answerIds.length - 1] || '') || turnId;
        CORE_UM_syncRuntimeIdentity(answerEl, answerId, turnNo);
        items.push({ el: answerEl, uid: answerId, num: String(turnNo), role: STR_UNMOUNTM_ROLE_A });
      }

      if (!items.length) continue;

      const questionItem = items.find((item) => item.role === STR_UNMOUNTM_ROLE_Q) || null;
      const assistantItem = items.find((item) => item.role === STR_UNMOUNTM_ROLE_A) || null;
      const primaryItem = questionItem || items[0] || null;
      if (!primaryItem?.el) continue;
      ...
      const group = {
        ...
        assistantItem,
        primaryItem,
        primaryUid: String(primaryItem.uid || turnId).trim(),
        primaryEl: primaryItem.el,
        uids: items.map((item) => String(item?.uid || '').trim()).filter(Boolean),
        key: String(primaryItem.uid || turnId).trim(),
        aliasIds: CORE_UM_collectRecordAliasIds(record, items),
        sharedTurnRecord: record,
      };
      out.push(group);
    }
  ...
  function CORE_UM_getPageVisibilityForGroup(group) {
    const record = CORE_UM_getSharedTurnRecordForGroup(group);
    if (!record) return null;

    const page = (record.page && typeof record.page === 'object') ? record.page : null;
    if (!page) return null;

    const turnNo = Math.max(1, Number(record?.turnNo || group?.num || 0) || 0);
    const turnIndex0 = turnNo > 0 ? (turnNo - 1) : -1;
    const turnStart = Number(page.turnStart);
    const turnEnd = Number(page.turnEnd);
    const hasTurnWindow = Number.isFinite(turnStart) && Number.isFinite(turnEnd) && turnStart >= 0 && turnEnd >= turnStart;

    if (page.inCurrentPage === true || page.inBufferedWindow === true) {
      return { available: true, isFar: false, source: 'page:current' };
    }
    if (hasTurnWindow && turnIndex0 >= 0) {
      return {
        available: true,
        isFar: (turnIndex0 < turnStart) || (turnIndex0 > turnEnd),
        source: 'page:turn-window',
      };
    }
    if (Number.isFinite(page.pageIndex) && Number.isFinite(page.pageCount)) {
      return { available: true, isFar: page.inCurrentPage === false, source: 'page:index' };
    }
  ...
  function CORE_UM_requestMiniMapSync(reason, extra = null) {
    ...
    const mmCore =
      W.H2O?.MM?.mnmp?.api?.core ||
      W.H2O_MM_SHARED?.get?.()?.api?.core ||
      null;

    if (typeof mmCore?.syncActiveFromViewport === 'function') {
      try { mmCore.syncActiveFromViewport({}); } catch (_) {}
    } else {
      try { W.updateActiveMiniMapBtn?.({}); } catch (_) {}
    }
  ...
  function CORE_UM_getAnswerToolbars(msgEl) {
    const turnHost = CORE_UM_getAnswerTurnHost(msgEl);
    if (!turnHost?.querySelectorAll) return [];
    try { return Array.from(turnHost.querySelectorAll(SEL_UNMOUNTM_RESPONSE_ACTIONS)); } catch (_) { return []; }
  }

  function CORE_UM_setAnswerTitleShellCollapsed(msgEl, collapsed) {
    const bar = CORE_UM_getAnswerTitleBar(msgEl);
    const icon = bar?.querySelector?.(SEL_UNMOUNTM_ANSWER_TITLE_ICON) || null;
    if (msgEl?.setAttribute) {
      if (collapsed) msgEl.setAttribute('data-at-collapsed', '1');
      else msgEl.removeAttribute('data-at-collapsed');
    }
    if (bar?.setAttribute) {
  ...
  function CORE_UM_emitLegacyAnswerCollapse(answerId, collapsed) {
    const id = UTIL_UM_normalizeId(answerId);
    if (!id) return false;
    try { W.dispatchEvent(new CustomEvent('evt:h2o:answer:collapse', { detail: { answerId: id, collapsed: !!collapsed } })); return true; } catch (_) {}
    return false;
  }

  function CORE_UM_normalizeManualSource(source) {
    const raw = String(source || '').trim().toLowerCase();
    if (raw === 'answer-title' || raw === 'title-list-row' || raw === 'page-collapse') return raw;
    return 'default';
  }
  ...
  function CORE_UM_findAssistantMessageByUid(uid) {
    ...
    const direct = CORE_UM_findMessageByUid(id);
    if (direct?.getAttribute?.('data-message-author-role') === STR_UNMOUNTM_ROLE_A) return direct;
    return null;
  }

  function CORE_UM_resolveManualAnswerId(anyId) {
    const id = UTIL_UM_normalizeId(anyId);
    if (!id) return '';
    if (S.manualCollapseById.has(id)) return id;
    const group = CORE_UM_getTurnGroupByUid(id, CORE_UM_getMessages());
    const answerId = UTIL_UM_normalizeId(group?.assistantItem?.uid || '');
    return answerId || id;
  }

  function CORE_UM_getManualRecord(anyId) {
    const id = CORE_UM_resolveManualAnswerId(anyId);
    return id ? (S.manualCollapseById.get(id) || null) : null;
  }

  function CORE_UM_isManualCollapsedById(anyId, opts = {}) {
    const record = CORE_UM_getManualRecord(anyId);
    if (!record?.sources?.size) return false;
    const source = CORE_UM_normalizeManualSourceFilter(opts?.source);
    if (!source) return false;
    if (source === 'default' && !String(opts?.source || '').trim()) return true;
    return record.sources.has(source);
  }

  function CORE_UM_getManualCollapsedIds(opts = {}) {
    const source = CORE_UM_normalizeManualSourceFilter(opts?.source);
    if (source === null) return [];
    const out = [];
    for (const [id, record] of S.manualCollapseById.entries()) {
      if (!id || !record?.sources?.size) continue;
      if (source && source !== 'default' && !record.sources.has(source)) continue;
      out.push(id);
    }
    return out;
  }

  function CORE_UM_isTurnGroupManualCollapsed(group) {
    if (!group) return false;
    if (CORE_UM_isManualCollapsedById(group?.assistantItem?.uid || '')) return true;
    for (const raw of group.aliasIds || []) {
      if (CORE_UM_isManualCollapsedById(raw)) return true;
    }
    return false;
  }
  ...
  function CORE_UM_createManualRecord(answerId, msgEl, opts = {}) {
    ...
    const record = {
      ...
      sources: new Set(),
      sourceMeta: new Map(),
      shellMode: String(opts?.preserveShell || 'default'),
      questionHiddenByManual: false,
    ...
    if (opts?.preserveQuestionShell !== true) {
      const questionHost = CORE_UM_getQuestionTurnHost(msgEl);
      const snapshot = CORE_UM_hideManualNode(questionHost);
      if (snapshot) {
        record.hiddenNodes.push(snapshot);
        record.questionHiddenByManual = true;
      }
    }

    CORE_UM_setAnswerTitleShellCollapsed(msgEl, true);
    return record;
  }

  function CORE_UM_restoreManualRecord(record) {
    ...
    for (const snapshot of record.hiddenNodes || []) {
      CORE_UM_restoreManualNode(snapshot);
    }

    CORE_UM_setAnswerTitleShellCollapsed(record.msgEl, false);
    return { ok: true, status: 'ok', id: record.id, collapsed: false };
  }

  function CORE_UM_applyManualCollapseById(anyId, opts = {}) {
    const source = CORE_UM_normalizeManualSource(opts?.source);
    let msgEl = CORE_UM_findAssistantMessageByUid(anyId);
    let answerId = UTIL_UM_normalizeId(msgEl?.dataset?.[ATTR_UNMOUNTM_H2O_UID] || anyId);
    if (!answerId || !msgEl) return { ok: false, status: 'answer-missing', id: UTIL_UM_normalizeId(anyId), collapsed: false };
    let tookOverBackgroundUnmount = false;
    if (msgEl.dataset?.[ATTR_UNMOUNTM_H2O_UNMOUNTED] === '1') {
      const primaryId = CORE_UM_resolvePrimaryUid(answerId) || answerId;
      const saved = S.unmountMap.get(primaryId) || S.unmountMap.get(answerId) || null;
      if (!saved) {
        return { ok: false, status: 'background-unmounted', id: answerId, collapsed: false };
      }
      CORE_UM_softRemount(primaryId, 'manual-takeover');
      msgEl = CORE_UM_findAssistantMessageByUid(answerId);
      answerId = UTIL_UM_normalizeId(msgEl?.dataset?.[ATTR_UNMOUNTM_H2O_UID] || answerId);
      if (!answerId || !msgEl || msgEl.dataset?.[ATTR_UNMOUNTM_H2O_UNMOUNTED] === '1') {
        return { ok: false, status: 'manual-takeover-failed', id: UTIL_UM_normalizeId(answerId || anyId), collapsed: false };
      }
      tookOverBackgroundUnmount = true;
    }

    let record = S.manualCollapseById.get(answerId) || null;
    const hadSource = !!record?.sources?.has(source);
    if (!record) {
      record = CORE_UM_createManualRecord(answerId, msgEl, opts);
      if (!record) return { ok: false, status: 'collapse-failed', id: answerId, collapsed: false };
      record.returnToBackgroundUnmount = !!tookOverBackgroundUnmount;
      S.manualCollapseById.set(answerId, record);
    } else if (opts?.preserveQuestionShell !== true && !record.questionHiddenByManual) {
      const questionHost = CORE_UM_getQuestionTurnHost(record.msgEl);
      const snapshot = CORE_UM_hideManualNode(questionHost);
      if (snapshot) {
        record.hiddenNodes.push(snapshot);
        record.questionHiddenByManual = true;
    ...
    if (tookOverBackgroundUnmount) record.returnToBackgroundUnmount = true;

    record.shellMode = String(opts?.preserveShell || record.shellMode || 'default');
    record.sources.add(source);
    record.sourceMeta.set(source, {
      preserveShell: String(opts?.preserveShell || 'default'),
      preserveQuestionShell: opts?.preserveQuestionShell === true,
      emitLegacyAnswerCollapse: opts?.emitLegacyAnswerCollapse === true,
    });
    ...
    return {
      ok: true,
      status: hadSource ? 'already-collapsed' : (tookOverBackgroundUnmount ? 'taken-over-collapsed' : 'collapsed'),
      id: answerId,
      collapsed: true,
      source,
      tookOverBackgroundUnmount,
      legacyAnswerCollapseChanged,
    };
  }

  function CORE_UM_applyManualExpandById(anyId, opts = {}) {
    const source = CORE_UM_normalizeManualSource(opts?.source);
    const answerId = CORE_UM_resolveManualAnswerId(anyId);
    const record = answerId ? (S.manualCollapseById.get(answerId) || null) : null;
    if (!record) return { ok: false, status: 'not-collapsed', id: answerId || UTIL_UM_normalizeId(anyId), collapsed: false };

    const hadSource = record.sources.has(source);
    if (source !== 'default' || String(opts?.source || '').trim()) {
      record.sources.delete(source);
      record.sourceMeta.delete(source);
    } else {
      record.sources.clear();
      record.sourceMeta.clear();
    }
  ...
  function CORE_UM_toggleManualCollapseById(anyId, opts = {}) {
    const source = CORE_UM_normalizeManualSource(opts?.source);
    if (CORE_UM_isManualCollapsedById(anyId, { source })) {
      return CORE_UM_applyManualExpandById(anyId, { ...opts, source });
    }
    return CORE_UM_applyManualCollapseById(anyId, { ...opts, source });
  }

  function CORE_UM_applyManualMany(ids, mode = 'collapse', opts = {}) {
    const normalized = Array.from(new Set((Array.isArray(ids) ? ids : []).map(UTIL_UM_normalizeId).filter(Boolean)));
    const results = [];
    for (const id of normalized) {
      results.push(mode === 'expand'
        ? CORE_UM_applyManualExpandById(id, opts)
        : CORE_UM_applyManualCollapseById(id, opts));
    }
    const failed = results.filter((item) => !item?.ok);
    const changed = results.filter((item) => item?.ok && item?.status !== 'already-collapsed' && item?.status !== 'unchanged' && item?.status !== 'source-cleared').length;
    return {
      ok: failed.length === 0,
      status: failed.length ? (failed.length < results.length ? 'partial-failure' : 'failed') : 'ok',
      ids: normalized,
      changed,
  ...
  function CORE_UM_restartIntervalTimer() {
    ...
    if (!S.pageChangedBound) {
      S.pageChangedBound = true;
      const EV_PG = 'evt:h2o:pagination:pagechanged';
      const onPageChanged = () => {
        if (!C.enabled) return;
      ...
      S.offPageChanged = () => {
        W.removeEventListener(EV_PG, onPageChanged);
        S.pageChangedBound = false;
  ...
  function API_UM_collapseById(id, opts = {}) {
    return CORE_UM_applyManualCollapseById(id, opts);
  }

  function API_UM_expandById(id, opts = {}) {
    return CORE_UM_applyManualExpandById(id, opts);
  }

  function API_UM_toggleById(id, opts = {}) {
    return CORE_UM_toggleManualCollapseById(id, opts);
  }

  function API_UM_collapseManyByIds(ids, opts = {}) {
    return CORE_UM_applyManualMany(ids, 'collapse', opts);
  }

  function API_UM_expandManyByIds(ids, opts = {}) {
    return CORE_UM_applyManualMany(ids, 'expand', opts);
  }

  function API_UM_isCollapsedById(id, opts = {}) {
    return CORE_UM_isManualCollapsedById(id, opts);
  }
  ...
  function CORE_UM_runUnmountPass(reason) {
    const runtimeGroups = CORE_UM_buildTurnGroupsFromRuntime();
    let msgs = null;
    let groups = runtimeGroups;
    let msgCount = 0;

    if (Array.isArray(groups) && groups.length) {
      msgCount = groups.reduce((sum, group) => sum + (Array.isArray(group?.items) ? group.items.length : 0), 0);
    } else {
      msgs = CORE_UM_getMessages();
      if (!msgs.length) return;

      CORE_UM_ensureMessageIds(msgs);
      groups = CORE_UM_buildTurnGroups(msgs);
    ...
    if (msgCount < C.minMsgsForUnmount) {
    ...
    for (const group of groups) {
      const pageVisibility = CORE_UM_getPageVisibilityForGroup(group);
      let isFar = false;

      if (pageVisibility?.available) {
        isFar = !!pageVisibility.isFar;
      } else {
        const bounds = CORE_UM_getTurnGroupBounds(group);
        if (!bounds) continue;
        isFar = (bounds.absBottom < aboveLine) || (bounds.absTop > belowLine);
      }

      const isUnmounted = CORE_UM_isTurnGroupUnmounted(group);
      const isProtected = CORE_UM_isTurnGroupProtected(group);
      const isManualCollapsed = CORE_UM_isTurnGroupManualCollapsed(group);
      ...
      if ((isProtected && isUnmounted) || (!isFar && isUnmounted && shouldScrollRestore)) {
        ...
        remountedCount += 1;
      } else if (isFar && !isUnmounted && !isProtected && !isManualCollapsed) {
  ...
  function CORE_UM_resolveMutationNode(node) {
    const el = (node && node.nodeType === 1) ? node : (node?.parentElement || null);
    if (!el || el.nodeType !== 1) return null;
    try {
      return el.closest?.(`${SEL_UNMOUNTM_MSG_ANY}, ${SEL_UNMOUNTM_CONV_TURN}`) || el;
    } catch (_) {
      return el;
    }
  }

  function CORE_UM_isOwnedMutationNode(node) {
    if (!node || node.nodeType !== 1) return false;
    try {
      if (
        node.hasAttribute?.(`data-${ATTR_UNMOUNTM_CGXUI_OWNER}`)
        && !node.querySelector?.(SEL_UNMOUNTM_MSG_ANY)
      ) {
        if (PERF_ASSERT_ON) console.debug('[H2O][Observer] skipped UI node');
        return true;
      }
      if (node.closest?.(`[data-${ATTR_UNMOUNTM_CGXUI_OWNER}]`)) {
        if (PERF_ASSERT_ON) console.debug('[H2O][Observer] skipped UI node');
        return true;
      }
      return false;
  ...
  function CORE_UM_isRelevantMutationNode(node) {
    const el = CORE_UM_resolveMutationNode(node);
    if (!el) return false;
    try {
      if (
        el.matches?.(SEL_UNMOUNTM_MSG_ANY)
        || el.matches?.(SEL_UNMOUNTM_CONV_TURN)
        || el.querySelector?.(SEL_UNMOUNTM_MSG_ANY)
        || el.querySelector?.(SEL_UNMOUNTM_CONV_TURN)
      ) {
        if (PERF_ASSERT_ON) console.debug('[H2O][Observer] accepted mutation:', el);
        return true;
      }
    } catch (_) {
      return false;
    }
    if (CORE_UM_isOwnedMutationNode(el)) return false;
    return false;
  }
  ...
  function CORE_UM_installRootMO() {
    ...
    if (hub && typeof hub.onMutations === 'function') {
      if (S.hubMutOff) return;
      S.hubMutOff = hub.onMutations('unmount:mut', (payload) => {
    ...
    S.rootMO = new MutationObserver((mutations) => {
      ...
      for (const m of mutations) {
        for (const node of Array.from(m?.addedNodes || [])) {
          const candidate = CORE_UM_resolveMutationNode(node) || CORE_UM_resolveMutationNode(m?.target);
          if (!CORE_UM_isRelevantMutationNode(candidate)) continue;
          relevant = true;
          break;
        }
        if (relevant) break;
  ...
  function SURFACE_UM_scheduleCommandBarBindRetry(attempt = 1) {
    ...
    if (nextAttempt > CFG_UNMOUNTM_COMMAND_BAR_RETRY_MAX_ATTEMPTS) return 0;
    S.commandBarBindTimer = W.setTimeout(() => {
      S.commandBarBindTimer = 0;
      if (S.commandBarBound && SURFACE_UM_getCommandBarApi()) return;
      if (SURFACE_UM_getCommandBarApi()) {
        SURFACE_UM_bindCommandBarFeature();
        return;
      }
      if (nextAttempt < CFG_UNMOUNTM_COMMAND_BAR_RETRY_MAX_ATTEMPTS) {
        SURFACE_UM_scheduleCommandBarBindRetry(nextAttempt + 1);
      }
    }, CFG_UNMOUNTM_COMMAND_BAR_RETRY_MS);
  ...
  function SURFACE_UM_syncCommandBarControls() {
    ...
    if (api && !S.commandBarBound) return !!SURFACE_UM_bindCommandBarFeature();
  ...
  function SURFACE_UM_bindCommandBarFeature() {
    ...
    if (!S.commandBarBound) {
      ...
      api.registerControl({
        ...
        /* User-facing action: belongs in Side Actions → Other tab */
        sideAction: true,
        sideTab: 'other',
        onClick: () => {
  ...
  function CORE_UM_dispose() {
    try {
      ...
      if (typeof S.hubMutOff === 'function') {
        try { S.hubMutOff(); } catch (_) {}
        S.hubMutOff = null;
      }
      if (S.rootMO) { try { S.rootMO.disconnect(); } catch (_) {} S.rootMO = null; }
  ...
  const ADAPTER = Object.freeze({
    boot: CORE_UM_boot,
    dispose: CORE_UM_dispose,
    forceRemountByUid: API_UM_forceRemountByUid,
    collapseById: API_UM_collapseById,
    expandById: API_UM_expandById,
    toggleById: API_UM_toggleById,
    collapseManyByIds: API_UM_collapseManyByIds,
    ...
    getConfig: API_UM_getConfigSnapshot,
    applySetting: API_UM_applySetting,
    setEnabled: API_UM_setEnabled,
    runPass: API_UM_runPass,
    getCollapsedGroupCount: CORE_UM_getCollapsedGroupCount,
    syncCommandBarControls: SURFACE_UM_syncCommandBarControls,
    emitConfigChanged: SURFACE_UM_emitConfigChanged,
    waitForMessagesThenBoot: CORE_UM_waitForMessagesThenBoot,
  });
