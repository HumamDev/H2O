...
// @name               1B1a.🟥⚡ MiniMap Quick Controls 🗺️
...
${S_ROOT}[${ATTR.CGXUI_THEME}="contrast"]{
  ...
  function dividerSectionHtml(){
    ...
    return `
      <div class="mmqc-sec" data-sec="divider">
        ${sectionLabel('〰', 'Divider', 'Select a divider, adjust its style and color, then drag it directly on the MiniMap.')}
        <div class="mmqc-row">
  ...
  function ensurePopoverLayout(pop){
    ...
    const wrap = document.createElement('div');
    wrap.innerHTML = `<div class="mmqc-div"></div>${dividerSectionHtml()}`;
  ...
  function dividerEditorControls(pop){
    return {
  ...
  function dividerList(api){
    const items = api?.getMiniDividers?.() || [];
  ...
  function syncDividerEditor(pop, refs, opts = {}){
    if (!pop) return;
    const { selectEl, noteEl, createBtn, removeBtn } = dividerEditorControls(pop);
    ...
    if (!selectedId) selectedId = String(api?.getSelectedMiniDividerId?.() || '').trim();
    ...
    if (api && items.length && !String(api?.getSelectedMiniDividerId?.() || '').trim() && selectedId) {
      try { api.selectMiniDivider?.(selectedId, '', 'quick:default'); } catch {}
    }

    const selected = selectedId ? (api?.getMiniDividerById?.(selectedId) || items.find((item) => String(item?.id || '').trim() === selectedId) || null) : null;
  ...
  function updateSelectedDividerFromPopover(pop, refs, patch = {}){
    const api = coreApi();
    const dividerId = getCurrentDividerId(pop);
    if (!api || !dividerId) return false;
    const existing = api.getMiniDividerById?.(dividerId) || null;
    if (!existing) return false;
    const result = api.upsertMiniDivider?.(Object.assign({}, existing, patch, { id: dividerId })) || null;
    syncDividerEditor(pop, refs, { preserveDraft: true, selectedId: dividerId });
    return !!result?.ok;
  }
  ...
  function createDividerFromPopover(pop, refs){
    const api = coreApi();
    if (!api) return false;
    const result = api.createMiniDivider?.({
      style: getCurrentDividerStyle(pop),
      color: getCurrentDividerColor(pop),
    }) || null;
    const nextId = String(result?.item?.id || '').trim();
    if (nextId) {
      try { api.selectMiniDivider?.(nextId, '', 'quick:create'); } catch {}
    }
  ...
  function removeSelectedDividerFromPopover(pop, refs){
    ...
    const dividerId = getCurrentDividerId(pop);
    if (!api || !dividerId) return false;
    const result = api.removeMiniDividerById?.(dividerId) || null;
  ...
  function claimActiveInstance(){
    ...
    on(pop, 'change', (e) => {
      ...
      const dividerId = String(t.value || '').trim();
      try { coreApi()?.selectMiniDivider?.(dividerId, '', 'quick:select'); } catch {}
      syncDividerEditor(S.pop, getRefs(), { preserveDraft: true, selectedId: dividerId });
    }, { passive: true });
  }
