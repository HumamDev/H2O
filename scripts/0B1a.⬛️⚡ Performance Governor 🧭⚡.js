// ==UserScript==
// @h2o-id             0b1a.perf.governor
// @name               0B1a.⬛️⚡ Performance Governor 🧭⚡
...
(() => {
  ...
  S.bootRetryCount = Number(S.bootRetryCount || 0);
    ...
    if (!GV_depsReady()) {
      if (!S.bootRetryTimer) {
        ...
        S.bootRetryTimer = W.setInterval(() => {
          S.bootRetryCount = (S.bootRetryCount || 0) + 1;
          if (S.bootRetryCount > 60) {
            GV_clearBootRetry();
            GV_safeWarn('boot retry ceiling reached — deps never became ready', {
              bootRetryCount: S.bootRetryCount,
            });
            return;
          }
