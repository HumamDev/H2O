...
(() => {
  ...
  const PERF_MODULE = (H2O.perf.modules.commandBar && typeof H2O.perf.modules.commandBar === "object")
    ? H2O.perf.modules.commandBar
    : (H2O.perf.modules.commandBar = Object.create(null));
  const PERF = (() => {
    const existing = PERF_MODULE.__h2oPerfState;
    if (existing && typeof existing === "object") return existing;
    const next = createCommandBarPerfState();
    try {
  ...
  PERF_MODULE.getStats = getCommandBarPerfStats;
  PERF_MODULE.resetStats = () => {
    resetCommandBarPerfState(PERF);
    return getCommandBarPerfStats();
  };
  ...
  const DOCK_FOLD_PROFILE_GAP_PX = 8;
  ...
  const FOLD_HIDE_SHIFT_PX = 11;
  const SIDEBAR_MASK_W_PX = FOLD_HIDE_SHIFT_PX;
  ...
  const state = {
    ...
    foldPlacement: TAB_PLACEMENT_PAGE_RIGHT,
    edgeMask: null,
    foldHotRect: null,
    foldHotRectAt: 0,
  };
  ...
  function createCommandBarPerfState() {
    return {
      scheduleDockGeometryFlush: {
        callCount: 0,
        coalescedCount: 0,
        tailRequestedCount: 0,
        tailExecutedCount: 0,
        lastReason: "",
        lastAt: 0,
        reasons: Object.create(null),
      },
      readDockGeometrySnapshot: createDurationBucket(),
      applyDockGeometrySnapshot: createDurationBucket(),
      updateDockOverflowMask: {
        totalCalls: 0,
        precomputedCalls: 0,
        fallbackMeasurementCalls: 0,
        fallbackMeasurementTotalMs: 0,
      },
      pointerHotZone: {
        seenCount: 0,
        toggledCount: 0,
        missingFoldGeometryCount: 0,
      },
      statusPanelSync: {
        flushTimeCalls: 0,
        nonFlushFallbackCalls: 0,
        flushTimeTotalMs: 0,
        flushTimeMaxMs: 0,
      },
    };
  ...
  function resetDurationBucket(bucket) {
    if (!bucket) return bucket;
    bucket.count = 0;
    bucket.totalMs = 0;
    bucket.maxMs = 0;
    bucket.slowOver4Count = 0;
    bucket.slowOver8Count = 0;
    bucket.slowOver16Count = 0;
    return bucket;
  }
  ...
  function resetCommandBarPerfState(target) {
    if (!target) return target;
    target.scheduleDockGeometryFlush = {
      callCount: 0,
      coalescedCount: 0,
      tailRequestedCount: 0,
      tailExecutedCount: 0,
    ...
    target.readDockGeometrySnapshot = createDurationBucket();
    target.applyDockGeometrySnapshot = createDurationBucket();
    target.updateDockOverflowMask = {
      totalCalls: 0,
      precomputedCalls: 0,
      fallbackMeasurementCalls: 0,
      fallbackMeasurementTotalMs: 0,
    };
    target.pointerHotZone = {
      seenCount: 0,
      toggledCount: 0,
      missingFoldGeometryCount: 0,
    };
    target.statusPanelSync = {
      flushTimeCalls: 0,
      nonFlushFallbackCalls: 0,
      flushTimeTotalMs: 0,
      flushTimeMaxMs: 0,
  ...
  function getCommandBarPerfStats() {
    const flushBucket = PERF.scheduleDockGeometryFlush || {};
    const overflowBucket = PERF.updateDockOverflowMask || {};
    const pointerBucket = PERF.pointerHotZone || {};
    const statusBucket = PERF.statusPanelSync || {};
    const flushTimeCalls = Number(statusBucket.flushTimeCalls || 0);
    const flushTimeTotalMs = Number(statusBucket.flushTimeTotalMs || 0);
    return {
      scheduleDockGeometryFlush: {
        callCount: Number(flushBucket.callCount || 0),
        coalescedCount: Number(flushBucket.coalescedCount || 0),
        tailRequestedCount: Number(flushBucket.tailRequestedCount || 0),
        tailExecutedCount: Number(flushBucket.tailExecutedCount || 0),
        lastReason: String(flushBucket.lastReason || ""),
        lastAt: Number(flushBucket.lastAt || 0),
        reasons: copyPlainCounts(flushBucket.reasons),
      },
      readDockGeometrySnapshot: readDurationBucket(PERF.readDockGeometrySnapshot),
      applyDockGeometrySnapshot: readDurationBucket(PERF.applyDockGeometrySnapshot),
      updateDockOverflowMask: {
        totalCalls: Number(overflowBucket.totalCalls || 0),
        precomputedCalls: Number(overflowBucket.precomputedCalls || 0),
        fallbackMeasurementCalls: Number(overflowBucket.fallbackMeasurementCalls || 0),
        fallbackMeasurementTotalMs: perfRoundMs(overflowBucket.fallbackMeasurementTotalMs || 0) ?? 0,
      },
      pointerHotZone: {
        seenCount: Number(pointerBucket.seenCount || 0),
        toggledCount: Number(pointerBucket.toggledCount || 0),
        missingFoldGeometryCount: Number(pointerBucket.missingFoldGeometryCount || 0),
      },
      statusPanelSync: {
        flushTimeCalls,
        nonFlushFallbackCalls: Number(statusBucket.nonFlushFallbackCalls || 0),
        flushTimeTotalMs: perfRoundMs(flushTimeTotalMs) ?? 0,
        flushTimeAvgMs: flushTimeCalls > 0 ? perfRoundMs(flushTimeTotalMs / flushTimeCalls) : null,
        flushTimeMaxMs: flushTimeCalls > 0 ? perfRoundMs(statusBucket.flushTimeMaxMs || 0) : null,
      },
    };
  }
  ...
  function keyTabPlacement() {
    return `${dataNs()}:commandbar:tab-placement:v1`;
  }
  ...
  function getTabPlacement() {
    return normalizeTabPlacement(lsGetStr(keyTabPlacement(), TAB_PLACEMENT_PAGE_RIGHT));
  }

  function setTabPlacement(placementRaw) {
    const placement = normalizeTabPlacement(placementRaw);
    lsSetStr(keyTabPlacement(), placement);
    return placement;
  }
  ...
  function resolveStatusPanelSizePx(boundsRaw, customRaw = state.statusPanelSizeCustom, sizePxRaw = state.statusPanelSizePx) {
    const bounds = isObj(boundsRaw) ? boundsRaw : getStatusPanelBoundsPx();
    const custom = !!customRaw;
    const target = custom ? Number(sizePxRaw || 0) : bounds.def;
    return Math.min(bounds.max, Math.max(bounds.min, Math.round(target || bounds.def)));
  }

  function updateStatusDividerMetrics(sizePxRaw = state.statusPanelSizePx, opts = {}) {
    const divider = state.rightDivider;
    if (!divider) return;
    const { min, max } = isObj(opts.bounds) ? opts.bounds : getStatusPanelBoundsPx(opts.dockWidthPx);
  ...
  function applyStatusPanelSizePx(sizePxRaw, opts = {}) {
    ...
    const bounds = isObj(opts.bounds) ? opts.bounds : getStatusPanelBoundsPx(opts.dockWidthPx);
    const next = resolveStatusPanelSizePx(bounds, custom, sizePxRaw);
    ...
    updateStatusDividerMetrics(next, { bounds, dockWidthPx: opts.dockWidthPx });
    if (opts.persist) {
      if (custom) lsSetStr(keyStatusPanelSize(), String(next));
      else lsDel(keyStatusPanelSize());
    }
    if (!opts.skipOverflowSync) updateDockOverflowMask(opts.hasOverflow);
    return next;
  }

  function syncStatusPanelSizeToDock(opts = {}) {
    const perfBucket = PERF.statusPanelSync;
    const isFlushTime = !!(
      opts?.__h2oPerfFlush === 1
      && isObj(opts.bounds)
      && Number.isFinite(Number(opts.dockWidthPx))
    );
    if (isFlushTime) perfBucket.flushTimeCalls = Number(perfBucket.flushTimeCalls || 0) + 1;
    else perfBucket.nonFlushFallbackCalls = Number(perfBucket.nonFlushFallbackCalls || 0) + 1;
    const perfT0 = isFlushTime ? perfNow() : 0;
    try {
      ...
      return applyStatusPanelSizePx(state.statusPanelSizeCustom ? state.statusPanelSizePx : 0, {
        ...
        persist: false,
        dockWidthPx: opts.dockWidthPx,
        bounds,
        skipOverflowSync: !!opts.skipOverflowSync,
        hasOverflow: opts.hasOverflow,
      });
    } finally {
      if (isFlushTime) {
        const ms = perfNow() - perfT0;
        perfBucket.flushTimeTotalMs = Number(perfBucket.flushTimeTotalMs || 0) + ms;
        perfBucket.flushTimeMaxMs = Math.max(Number(perfBucket.flushTimeMaxMs || 0), ms);
      }
    }
  }
...
.h2o-archive-dock .perfBtn{border-color:rgba(142,207,255,.24);background:linear-gradient(180deg,rgba(43,113,156,.22),rgba(18,57,87,.16));color:#8ecfff;box-shadow:0 0 0 1px rgba(132,208,255,.08) inset}
.h2o-archive-dock .perfBtn:hover{filter:brightness(1.05)}
.h2o-archive-dock .perfBtn.on{border-color:rgba(142,207,255,.42);box-shadow:0 0 0 1px rgba(255,255,255,.04) inset,0 0 12px rgba(132,208,255,.18)}
.h2o-archive-dock .perfBtn.detail{color:#dff1ff;text-shadow:0 0 7px rgba(132,208,255,.16)}
...
.h2o-archive-dock .sidebarEdgeMask{position:fixed;top:0;bottom:0;left:calc(var(--h2o-archive-sidebar-edge-px,0px) - var(--h2o-archive-sidebar-mask-w,11px));width:var(--h2o-archive-sidebar-mask-w,11px);background:var(--h2o-archive-sidebar-mask-bg,rgba(24,24,24,.98));pointer-events:none;opacity:0;z-index:1000027;transition:opacity .18s ease,left .18s ease}
.h2o-archive-dock[data-tab-placement="sidebar-edge"] .sidebarEdgeMask{opacity:1}
.h2o-archive-dock .fold{position:fixed;bottom:${DOCK_FOLD_TAB_BOTTOM_PX}px;z-index:1000026;width:22px;min-width:22px;height:32px;min-height:32px;padding:0;display:flex;align-items:center;justify-content:center;text-align:center;font-weight:700;line-height:1;border:1px solid rgba(255,255,255,.24);background:var(--h2o-archive-dock-bg,#14161a);box-shadow:0 8px 18px rgba(0,0,0,.38);opacity:.52;transition:transform .18s ease,opacity .18s ease,filter .18s ease,left .18s ease,right .18s ease}
.h2o-archive-dock .fold[data-placement="page-right"]{right:0;left:auto;border-radius:10px 0 0 10px;border-left:1px solid rgba(255,255,255,.24);border-right:0;transform:translateX(${FOLD_HIDE_SHIFT_PX}px)}
.h2o-archive-dock .fold[data-placement="sidebar-edge"]{left:var(--h2o-archive-sidebar-edge-px,0px);right:auto;border-radius:0 10px 10px 0;border-left:0;border-right:1px solid rgba(255,255,255,.24);transform:translateX(-${FOLD_HIDE_SHIFT_PX}px)}
.h2o-archive-dock.edge-hot .fold,
.h2o-archive-dock .fold:hover{transform:translateX(0);opacity:1;filter:brightness(1.08)}
...
.h2o-archive-dock.collapsed .fold{bottom:${DOCK_FOLD_TAB_BOTTOM_PX}px;width:22px;min-width:22px;height:32px;min-height:32px;padding:0;background:var(--h2o-archive-dock-bg,#14161a);box-shadow:0 8px 18px rgba(0,0,0,.38);opacity:.52}
.h2o-archive-dock.collapsed.edge-hot .fold,
.h2o-archive-dock.collapsed .fold:hover{transform:translateX(0);opacity:1;filter:brightness(1.08)}
...
<div class="toolsPinnedLeft">
  ...
  <button type="button" class="perfBtn" aria-pressed="false" title="Perf HUD unavailable" aria-label="Perf HUD unavailable">Perf</button>
...
`;
  ...
  function cacheShellRefs(root) {
    ...
    state.perfBtn = state.toolsPinnedLeft?.querySelector(".perfBtn") || null;
  ...
  function ensureMounted() {
    ...
    if (!state.mountReady) {
      ...
      try {
        ...
        closeScopePicker();
        primeStatusPanelSize();
        applyFoldTabPlacement(getTabPlacement(), false);
  ...
  function schedulePrimeLayout() {
    ...
    state.primeLayoutRaf = W.requestAnimationFrame(() => {
      ...
      setRootDockHeightVar(0);
      scheduleDockGeometryFlush("prime-layout");
    });
  }
  ...
  function getPerfHudApi() {
    const api = TOPW.H2O?.perf?.hud;
    if (!api || typeof api !== "object") return null;
    if (typeof api.getState !== "function") return null;
    return api;
  }

  function readPerfHudState() {
    const api = getPerfHudApi();
    if (!api) return null;
    try { return api.getState() || null; } catch { return null; }
  }

  function syncPerfHudButton() {
    ensureMounted();
    const btn = state.perfBtn;
    if (!btn) return null;
    const hud = getPerfHudApi();
    const perfState = readPerfHudState();
    const visible = !!perfState?.visible;
    const detailOpen = !!perfState?.open;
    btn.classList.toggle("on", visible);
    btn.classList.toggle("detail", visible && detailOpen);
    btn.setAttribute("aria-pressed", visible ? "true" : "false");
    btn.textContent = "Perf";
    const title = !hud
      ? "Perf HUD unavailable"
      : visible
        ? (detailOpen ? "Perf HUD visible • detail open — click to hide" : "Perf HUD visible — click to hide")
        : "Perf HUD hidden — click to show";
    btn.title = title;
  ...
  function togglePerfHud() {
    ...
    const perfState = readPerfHudState();
    try {
      if (perfState?.visible) hud.hide?.("command-bar");
      else hud.show?.("command-bar");
    } catch {}
    syncPerfHudButton();
    return true;
  }
  ...
  function applyWindowScope(scopeRaw, opts = {}) {
    ...
    scheduleDockGeometryFlush("window-scope");
    return next;
  }
...
`;
  ...
  function scheduleLayoutCommit(zone = "main") {
    ...
    state.reflowRaf = W.requestAnimationFrame(() => {
      ...
      scheduleDockGeometryFlush("layout-commit");
    });
  }
  ...
  function isPointerNearPageRightEdge(clientXRaw = 0, clientYRaw = 0, anchorTopRaw = 0, anchorHeightRaw = 32) {
    const vw = Math.max(Number(W.innerWidth || 0), Number(D.documentElement?.clientWidth || 0));
    ...
    const centerY = Math.max(0, Math.round(Number(anchorTopRaw || 0) + (Number(anchorHeightRaw || 32) / 2)));
    const nearY = Math.abs(clientY - centerY) <= Math.max(96, Math.round(Number(anchorHeightRaw || 32) * 2.5));
    return nearX && nearY;
  }

  function isPointerNearSidebarEdge(clientXRaw = 0, clientYRaw = 0, edgePxRaw = 0, anchorTopRaw = 0, anchorHeightRaw = 32) {
    const edge = Math.max(0, Math.round(Number(edgePxRaw || 0)));
    const clientX = Math.max(0, Math.round(Number(clientXRaw || 0)));
    const clientY = Math.max(0, Math.round(Number(clientYRaw || 0)));
  ...
  function getVisibleProfileTopPx() {
    const nodes = Array.from(D.querySelectorAll('[data-testid="accounts-profile-button"]')).filter((el) => _isVisibleBox(el));
    let top = Number.POSITIVE_INFINITY;
    for (const node of nodes) {
      try {
        const rect = node.getBoundingClientRect();
        const nextTop = Number(rect?.top || 0);
        if (Number.isFinite(nextTop) && nextTop > 0) top = Math.min(top, nextTop);
      } catch {}
    }
    return Number.isFinite(top) ? Math.round(top) : 0;
  }

  function getFoldBottomPx(placementRaw = state.foldPlacement || getTabPlacement(), opts = {}) {
    const placement = normalizeTabPlacement(placementRaw);
    if (placement !== TAB_PLACEMENT_SIDEBAR_EDGE) return DOCK_FOLD_TAB_BOTTOM_PX;
    const viewportH = Number.isFinite(Number(opts.viewportHeight))
      ? Math.max(0, Math.round(Number(opts.viewportHeight || 0)))
      : Math.max(Number(W.innerHeight || 0), Number(D.documentElement?.clientHeight || 0), 0);
    const profileTop = Number.isFinite(Number(opts.profileTopPx))
      ? Math.max(0, Math.round(Number(opts.profileTopPx || 0)))
      : getVisibleProfileTopPx();
    if (viewportH > 0 && profileTop > 0) {
      return Math.max(DOCK_FOLD_TAB_BOTTOM_PX, Math.round((viewportH - profileTop) + DOCK_FOLD_PROFILE_GAP_PX));
    }
    return DOCK_FOLD_TAB_BOTTOM_PX;
  }
  ...
  function applyFoldTabPlacement(placementRaw, persist = false) {
    ensureMounted();
    const placement = persist ? setTabPlacement(placementRaw) : normalizeTabPlacement(placementRaw);
    state.foldPlacement = placement;
    try { state.root?.setAttribute?.("data-tab-placement", placement); } catch {}
    if (state.foldBtn) state.foldBtn.dataset.placement = placement;
    scheduleDockGeometryFlush("tab-placement");
    return placement;
  }

  function readDockGeometrySnapshot() {
    ...
    try {
      ...
      const placement = normalizeTabPlacement(state.foldPlacement || getTabPlacement());
      const viewportWidthPx = Math.max(Number(W.innerWidth || 0), Number(D.documentElement?.clientWidth || 0), 0);
      const viewportHeightPx = Math.max(Number(W.innerHeight || 0), Number(D.documentElement?.clientHeight || 0), 0);
      ...
      const sidebarRightPx = (prevLeft > 0 && Math.abs(leftRaw - prevLeft) <= 2) ? prevLeft : leftRaw;
      const sidebarMaskBg = getSidebarMaskColor();
      const profileTopPx = placement === TAB_PLACEMENT_SIDEBAR_EDGE ? getVisibleProfileTopPx() : 0;
      const foldBottomPx = getFoldBottomPx(placement, {
        viewportHeight: viewportHeightPx,
        profileTopPx,
      });
      const foldHotHeightPx = Math.max(1, Math.round(Number(state.foldBtn?.offsetHeight || state.foldHotRect?.height || 32)));
      const foldHotTopPx = Math.max(0, Math.round(viewportHeightPx - foldBottomPx - foldHotHeightPx));
      const dockLeftCss = collapsed ? "" : `${sidebarRightPx}px`;
      const dockRightCss = "0px";
      const dockWidthCss = (!collapsed && sidebarRightPx > 0) ? `calc(100% - ${sidebarRightPx}px)` : "";
      const dockWidthPx = collapsed ? 0 : Math.max(0, Math.round(viewportWidthPx - sidebarRightPx));
      const statusBounds = getStatusPanelBoundsPx(dockWidthPx);
      const statusPanelSizePx = resolveStatusPanelSizePx(statusBounds, state.statusPanelSizeCustom, state.statusPanelSizePx);
      const statusRowTargetWidthPx = collapsed ? 0 : Math.max(0, Math.round(statusPanelSizePx - STATUS_PANEL_RESERVED_OFFSET_PX));
      let hasOverflow = false;
      if (!collapsed && state.toolsLeft) {
        const currentDockWidthPx = Math.max(0, Math.round(Number(dock.getBoundingClientRect?.().width || dock.offsetWidth || 0)));
        const currentToolsWidthPx = Math.max(0, Math.round(Number(state.toolsLeft.clientWidth || 0)));
        const currentStatusWidthPx = state.statusRow?.isConnected
          ? Math.max(0, Math.round(Number(state.statusRow.getBoundingClientRect?.().width || state.statusRow.offsetWidth || 0)))
          : 0;
        const predictedToolsWidthPx = Math.max(
          0,
          Math.round(currentToolsWidthPx + (dockWidthPx - currentDockWidthPx) - (statusRowTargetWidthPx - currentStatusWidthPx))
        );
        hasOverflow = (Number(state.toolsLeft.scrollWidth || 0) - predictedToolsWidthPx) > 6;
      }
      const rawHeightPx = collapsed ? 0 : Math.max(0, Math.ceil(Number(dock.getBoundingClientRect?.().height || dock.offsetHeight || 0)));
      const prevHeightPx = Math.max(0, Math.ceil(Number(state.dockLastAppliedHeightPx || 0)));
      const dockHeightPx = collapsed
        ? 0
        : ((prevHeightPx > 0 && Math.abs(rawHeightPx - prevHeightPx) <= 1) ? prevHeightPx : rawHeightPx);
      return {
        collapsed,
        placement,
        sidebarRightPx,
        sidebarMaskBg,
        edgeMaskLeftPx: sidebarRightPx - SIDEBAR_MASK_W_PX,
        foldBottomPx,
        foldHotTopPx,
        foldHotHeightPx,
        dockLeftCss,
      ...
      recordDuration(PERF.readDockGeometrySnapshot, perfNow() - perfT0);
    }
  }

  function applyDockGeometrySnapshot(snapshot) {
    const perfT0 = perfNow();
    try {
      ensureMounted();
      const dock = state.root;
      if (!dock || !dock.isConnected || !snapshot) {
        state.foldHotRect = null;
        state.foldHotRectAt = 0;
        return;
      }
      const placement = normalizeTabPlacement(snapshot.placement || state.foldPlacement || getTabPlacement());
      const sidebarRightPx = Math.max(0, Math.round(Number(snapshot.sidebarRightPx || 0)));
      state.foldPlacement = placement;
      state.dockLastSidebarRightPx = sidebarRightPx;
      state.dockLastAppliedHeightPx = snapshot.collapsed ? 0 : Math.max(0, Math.ceil(Number(snapshot.dockHeightPx || 0)));
      dock.style.setProperty("--h2o-archive-sidebar-edge-px", `${sidebarRightPx}px`);
      dock.style.setProperty("--h2o-archive-sidebar-mask-bg", String(snapshot.sidebarMaskBg || "rgba(24,24,24,.98)"));
      if (snapshot.collapsed) {
        ...
        if (dock.style.left !== snapshot.dockLeftCss) dock.style.left = snapshot.dockLeftCss;
        if (dock.style.right !== snapshot.dockRightCss && dock.style.right !== "0") dock.style.right = snapshot.dockRightCss;
        if ((dock.style.width || "") !== snapshot.dockWidthCss) dock.style.width = snapshot.dockWidthCss;
      }
      if (state.edgeMask) {
        const leftCss = `${Math.round(Number(snapshot.edgeMaskLeftPx || 0))}px`;
        if (state.edgeMask.style.left !== leftCss) state.edgeMask.style.left = leftCss;
        if (state.edgeMask.style.width !== `${SIDEBAR_MASK_W_PX}px`) state.edgeMask.style.width = `${SIDEBAR_MASK_W_PX}px`;
      }
      if (state.foldBtn) {
        state.foldBtn.dataset.placement = placement;
        if (placement === TAB_PLACEMENT_SIDEBAR_EDGE) {
          const leftCss = `${sidebarRightPx}px`;
          if (state.foldBtn.style.left !== leftCss) state.foldBtn.style.left = leftCss;
          if (state.foldBtn.style.right !== "auto") state.foldBtn.style.right = "auto";
        } else {
          if (state.foldBtn.style.left !== "auto") state.foldBtn.style.left = "auto";
          if (state.foldBtn.style.right !== "0px" && state.foldBtn.style.right !== "0") state.foldBtn.style.right = "0px";
        }
        const bottomCss = `${Math.max(0, Math.round(Number(snapshot.foldBottomPx || DOCK_FOLD_TAB_BOTTOM_PX)))}px`;
        if (state.foldBtn.style.bottom !== bottomCss) state.foldBtn.style.bottom = bottomCss;
      }
      if (!snapshot.collapsed) {
        syncStatusPanelSizeToDock({
          dockWidthPx: snapshot.dockWidthPx,
          bounds: snapshot.statusBounds,
          skipOverflowSync: true,
          __h2oPerfFlush: 1,
        });
      }
      updateDockOverflowMask(snapshot.collapsed ? false : !!snapshot.hasOverflow);
      state.foldHotRect = {
        top: Math.max(0, Math.round(Number(snapshot.foldHotTopPx || 0))),
        height: Math.max(1, Math.round(Number(snapshot.foldHotHeightPx || 32))),
        edgePx: sidebarRightPx,
        placement,
      };
      state.foldHotRectAt = Date.now();
    } finally {
      recordDuration(PERF.applyDockGeometrySnapshot, perfNow() - perfT0);
    }
  }

  function scheduleDockGeometryFlush(reason = "layout", opts = {}) {
    const perfBucket = PERF.scheduleDockGeometryFlush;
    const why = String(reason || "layout").trim() || "layout";
    ...
    ensureMounted();
    if (opts.tail) {
      perfBucket.tailRequestedCount = Number(perfBucket.tailRequestedCount || 0) + 1;
      const tailDelayMs = Math.max(120, Math.round(Number(opts.tailDelayMs || 180)));
      state.dockGeomBurstUntilMs = Date.now() + tailDelayMs;
      if (state.dockGeomBurstTimer) {
        try { W.clearTimeout(state.dockGeomBurstTimer); } catch {}
      }
      state.dockGeomBurstTimer = W.setTimeout(() => {
        state.dockGeomBurstTimer = 0;
        state.dockGeomBurstUntilMs = 0;
        perfBucket.tailExecutedCount = Number(perfBucket.tailExecutedCount || 0) + 1;
        scheduleDockGeometryFlush(`${reason}:tail`);
      }, tailDelayMs);
    }
    ...
    state.dockGeomBurstRaf = W.requestAnimationFrame(() => {
      state.dockGeomBurstRaf = 0;
      let snapshot = null;
      try {
        snapshot = readDockGeometrySnapshot();
      } catch (e) {
        warn("dock geometry read failed", e);
      }
      if (!snapshot) return;
      try {
        applyDockGeometrySnapshot(snapshot);
      } catch (e) {
        warn("dock geometry apply failed", e);
      }
    });
  }

  function scheduleDockSidebarSettleFlush() {
    scheduleDockGeometryFlush("sidebar-transition", { tail: true });
  }
  ...
  function refreshDockSidebarObserver() {
    ...
    if (!state.dockSidebarResizeObserver) {
      state.dockSidebarResizeObserver = new ResizeObserver(() => {
        scheduleDockGeometryFlush("sidebar-resize", { tail: true });
      });
    }
    ...
    for (const node of prev) {
      try { node.removeEventListener("transitionstart", scheduleDockSidebarSettleFlush); } catch {}
      try { node.removeEventListener("transitionrun", scheduleDockSidebarSettleFlush); } catch {}
      try { node.removeEventListener("transitioncancel", scheduleDockSidebarSettleFlush); } catch {}
      try { node.removeEventListener("transitionend", scheduleDockSidebarSettleFlush); } catch {}
    }
    try { ro.disconnect(); } catch {}
    for (const node of nodes) {
      try { ro.observe(node); } catch {}
      try { node.addEventListener("transitionstart", scheduleDockSidebarSettleFlush, { passive: true }); } catch {}
      try { node.addEventListener("transitionrun", scheduleDockSidebarSettleFlush, { passive: true }); } catch {}
      try { node.addEventListener("transitioncancel", scheduleDockSidebarSettleFlush, { passive: true }); } catch {}
      try { node.addEventListener("transitionend", scheduleDockSidebarSettleFlush, { passive: true }); } catch {}
    }
    state.dockSidebarObservedEls = nodes;
    scheduleDockGeometryFlush("sidebar-observer");
    return ro;
  }
  ...
  function setRootDockHeightVar(pxRaw) {
    const root = D.documentElement || D.body;
    if (!root) return 0;
  ...
  function updateDockOverflowMask(hasOverflow) {
    const perfBucket = PERF.updateDockOverflowMask;
    perfBucket.totalCalls = Number(perfBucket.totalCalls || 0) + 1;
    ensureMounted();
    ...
    if (typeof hasOverflow === "boolean") {
      perfBucket.precomputedCalls = Number(perfBucket.precomputedCalls || 0) + 1;
    ...
    perfBucket.fallbackMeasurementCalls = Number(perfBucket.fallbackMeasurementCalls || 0) + 1;
    const perfT0 = perfNow();
    const overflowActive = (Number(tools.scrollWidth || 0) - Number(tools.clientWidth || 0)) > 6;
    perfBucket.fallbackMeasurementTotalMs = Number(perfBucket.fallbackMeasurementTotalMs || 0) + (perfNow() - perfT0);
    state.root.classList.toggle("has-overflow", overflowActive);
  ...
  function updateDockHeightVar() {
    ...
    if (!dock || !dock.isConnected) {
      ...
      setRootDockHeightVar(0);
      scheduleDockGeometryFlush("height");
      return;
    }
  ...
  function ensureDockHeightObserver() {
    ...
    const dock = state.root;
    if (!dock) return null;
    const ro = new ResizeObserver(() => scheduleDockGeometryFlush("dock-resize"));
    try {
      ro.observe(dock);
  ...
  function bindShellListeners() {
    ...
    D.addEventListener("pointermove", (ev) => {
      const perfBucket = PERF.pointerHotZone;
      perfBucket.seenCount = Number(perfBucket.seenCount || 0) + 1;
      const fold = state.foldBtn;
      if (!fold) return;
      const hotRect = state.foldHotRect;
      const root = state.root;
      const wasHot = !!root?.classList?.contains?.("edge-hot");
      let hot = false;
      if (hotRect) {
        const placement = hotRect.placement || state.foldPlacement || getTabPlacement();
        hot = placement === TAB_PLACEMENT_SIDEBAR_EDGE
          ? isPointerNearSidebarEdge(ev?.clientX || 0, ev?.clientY || 0, hotRect.edgePx || 0, hotRect.top || 0, hotRect.height || 32)
          : isPointerNearPageRightEdge(ev?.clientX || 0, ev?.clientY || 0, hotRect.top || 0, hotRect.height || 32);
      } else {
        perfBucket.missingFoldGeometryCount = Number(perfBucket.missingFoldGeometryCount || 0) + 1;
      }
      root?.classList.toggle("edge-hot", hot);
      if (root && wasHot !== hot) {
        perfBucket.toggledCount = Number(perfBucket.toggledCount || 0) + 1;
      }
    ...
    state.perfBtn?.addEventListener("click", (ev) => {
      try {
        ev?.preventDefault?.();
        ev?.stopPropagation?.();
      } catch {}
    ...
    try { W.addEventListener("evt:h2o:perf:hud-ready", () => syncPerfHudButton(), { passive: true }); } catch {}
    try { W.addEventListener("evt:h2o:perf:hud-statechanged", () => syncPerfHudButton(), { passive: true }); } catch {}
  ...
  API.getTabPlacement = () => normalizeTabPlacement(state.foldPlacement || getTabPlacement());
  API.setTabPlacement = (placementRaw) => applyFoldTabPlacement(placementRaw, true);
