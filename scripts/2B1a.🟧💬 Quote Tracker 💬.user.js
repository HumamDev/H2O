// ==UserScript==
// @h2o-id      2b.quote.tracker
// @name         2B.🟧💬 Quote Tracker 💬
// @namespace    H2O.Prime.CGX.QuoteTracker
// @version      1.0.7
...
(() => {
  ...
  function DOM_QT_getPrevAssistantForUserMsg(userMsgEl) {
    const all = [...document.querySelectorAll(SEL_QT_.ANY_MSG)];
  ...
  /** @critical */
  function DOM_QT_tryCaptureComposerQuote() {
    const chipBtn = document.querySelector(SEL_QT_.CHIP_BTN) || document.querySelector(SEL_QT_.CHIP_BTN_THREAD);

    let rawText = '';
    if (chipBtn) rawText = (chipBtn.innerText || chipBtn.textContent || '');
    else {
      const p = document.querySelector(SEL_QT_.CHIP_P_FALLBACK);
      if (!p) return;
    ...
    const maxIdx = DOM_QT_turnTotal() || document.querySelectorAll(SEL_QT_.ASSIST_MSG).length || document.querySelectorAll(SEL_QT_.USER_MSG).length || 0;
  ...
  function CORE_QT_installQuoteOriginCapture() {
    ...
    const mo = new MutationObserver(() => {
      const chipBtn = document.querySelector(SEL_QT_.CHIP_BTN);
      if (!chipBtn) return;
      ...
      DOM_QT_tryCaptureComposerQuote();
    });
  ...
  function DOM_QT_pickAssistantNearViewport() {
    ...
    const as = [...document.querySelectorAll(SEL_QT_.ASSIST_MSG)];
  ...
  function CORE_QT_installQuoteClickResolver() {
    ...
    const onClick = async (e) => {
      ...
      const maxNow = DOM_QT_turnTotal() || document.querySelectorAll(SEL_QT_.ASSIST_MSG).length || document.querySelectorAll(SEL_QT_.USER_MSG).length || 0;
  ...
  /** @helper */
  function scheduleResolve(reason = '') {
    const st = MOD.state;
  ...
  function runResolve(reason = '') {
    ...
    try {
      const pending = [...document.querySelectorAll(SEL_QT_.QBOX)].filter(qb => qb.dataset.hoQuoteConf !== '1');
    ...
    const mo = new MutationObserver((muts) => {
      ...
      for (const m of muts) {
        for (const n of (m.addedNodes || [])) {
          ...
          if (n.closest?.('form, #thread-bottom-container')) needChip = true;
          if (n.matches?.(SEL_QT_.ANY_MSG) || n.querySelector?.(SEL_QT_.ANY_MSG)) needScan = true;
          if (needScan && needChip) break;
        }
        if (needScan && needChip) break;
      }

      if (needChip) DOM_QT_tryCaptureComposerQuote();
      if (needScan) scheduleScan('mo');
    });
