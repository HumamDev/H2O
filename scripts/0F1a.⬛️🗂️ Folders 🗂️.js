...
`;
  ...
  function API_getBinding(chatIdOrHref) {
    const key = String(chatIdOrHref || '').trim();
    if (!key) return { folderId: '', folderName: '' };

    const d = STORE_readData();
    const folders = Array.isArray(d.folders) ? d.folders : [];
    const items = (d.items && typeof d.items === 'object') ? d.items : {};

    // Search by href match (native Folders storage) or by chatId in href
    for (const folder of folders) {
      const fid = String(folder.id || '');
      const arr = Array.isArray(items[fid]) ? items[fid] : [];
      for (const href of arr) {
        // Match: exact href, or href contains the chatId
        if (String(href) === key || String(href).includes(`/c/${key}`)) {
  ...
  function API_setBinding(chatIdOrHref, folderId) {
    const key = String(chatIdOrHref || '').trim();
    if (!key) return { folderId: '', folderName: '' };
    ...
    // Build href from chatId if needed
    let href = key;
    if (!href.startsWith('/')) {
      href = `/c/${key}`;
    }
    ...
    for (const existingFid of Object.keys(d.items)) {
      d.items[existingFid] = (d.items[existingFid] || []).filter(h => h !== href && !String(h).includes(`/c/${key}`));
    }

    // Assign to target folder (if not empty)
    if (fid && d.items[fid] !== undefined) {
      d.items[fid] = Array.isArray(d.items[fid]) ? d.items[fid] : [];
      d.items[fid].push(href);
    ...
    const folder = (Array.isArray(d.folders) ? d.folders : []).find(f => String(f.id || '') === fid);
    return { folderId: fid, folderName: String(folder?.name || '') };
  }
