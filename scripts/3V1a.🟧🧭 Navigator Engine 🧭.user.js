...
(() => {
  ...
  function getTurnsFromCore() {
    ...
    const qEls = Array.from(document.querySelectorAll('[data-message-author-role="user"]'));
    const aEls = Array.from(document.querySelectorAll('[data-message-author-role="assistant"]'));
    const rootNodes = Array.from(document.querySelectorAll('[data-message-author-role="user"], [data-message-author-role="assistant"]'));
    const turnsOut = [];
    let current = null;
    let idx = 0;
    for (const el of rootNodes) {
      ...
      if (role === 'user') {
        idx += 1;
        const qId = normalizeMsgId(el.getAttribute('data-message-id') || el.dataset?.messageId || '');
        current = { idx, turnId: qId ? `turn:${qId}` : `turn:${idx}`, qEl: el, qId, answers: [], primaryAEl: null, primaryAId: null };
        turnsOut.push(current);
      } else if (role === 'assistant') {
        if (!current) {
          idx += 1;
          current = { idx, turnId: `turn:${idx}`, qEl: null, qId: '', answers: [], primaryAEl: null, primaryAId: null };
          turnsOut.push(current);
        }
        const aId = normalizeMsgId(el.getAttribute('data-message-id') || el.dataset?.messageId || '');
        current.answers.push({ el, id: aId });
        current.primaryAEl = el;
        current.primaryAId = aId;
      }
    }
    return turnsOut;
  }
