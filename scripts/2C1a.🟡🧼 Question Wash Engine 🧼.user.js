// ==UserScript==
// @h2o-id             2c1a.question.wash.engine
// @name               2C1a.🟡🧼 Question Wash Engine 🧼
...
(function questionWashEngine() {
  ...
  function isMiniMapQaViewActive() {
    try {
      const host = D.querySelector('[data-cgxui="mnmp-panel"][data-cgxui-view], [data-cgxui="mnmp-minimap"][data-cgxui-view], [data-h2o-owner="minimap-v10"][data-cgxui-view]');
  ...
  function bindRuntimeEvents() {
    ...
    W.addEventListener('pageshow', () => scheduleRescan('pageshow'), true);
    W.addEventListener('focus', () => scheduleRescan('focus'), true);
  ...
  function bindObserverHub() {
    const hub = TOPW?.H2O?.obs || W?.H2O?.obs || TOPW?.H2O?.observerHub || W?.H2O?.observerHub || null;
  ...
  function applyAllWashes() {
    ...
    if (miniMapQaMode) requestMiniMapQaQuestionRepaint('qwash:apply');
    return stats;
  }
  ...
  function applyColorToCurrent(colorIdOrNull) {
    ...
    requestMiniMapQaQuestionRepaint('qwash:changed');
  ...
  function collectQuestionBlocks() {
    const out = [];
    const seen = new Set();
    const userHosts = Array.from(D.querySelectorAll([
      '[data-message-author-role="user"]',
      '[data-author-role="user"]',
      '[data-role="user"]',
      '[data-testid*="user-message"]',
    ...
    if (!out.length) {
      D.querySelectorAll('.cgxui-qswr, [data-h2o-qwrap-id], [data-ho-qwrap-id]').forEach((node) => {
