// ==UserScript==
// @h2o-id             0w2a.pagination.windowing
// @name               0W2a.⬛️🪟 Pagination Windowing 🪟
...
.cgxui-pgnw-sentinel{
  ...
  --pgnw-band-bg-hover: rgba(58,62,72,0.98);
}
...
.cgxui-pgnw-sentinel > button:hover:not(:disabled){
  ...
  border-color: color-mix(in srgb, var(--pgnw-band-border) 85%, white 15%);
  box-shadow: none;
}
...
.cgxui-pgnw-sentinel > button[data-page-band="normal"]{
  ...
  --pgnw-band-bg-hover: rgba(16,185,129,0.46);
}
...
.cgxui-pgnw-sentinel > button[data-page-band="teal"]{
  ...
  --pgnw-band-bg-hover: rgba(202,138,4,0.46);
}
...
.cgxui-pgnw-sentinel > button[data-page-band="blue"]{
  ...
  --pgnw-band-bg-hover: rgba(37,99,235,0.46);
}
...
.cgxui-pgnw-sentinel > button[data-page-band="darkred"]{
  ...
  --pgnw-band-bg-hover: rgba(185,28,28,0.46);
}
...
.cgxui-pgnw-sentinel > button[data-page-band="violet"]{
  ...
  --pgnw-band-bg-hover: rgba(124,58,237,0.46);
}
...
`;
  ...
  function buildPageDividerMapForTurnRange(startTurnIndex, endTurnIndex) {
    ...
    for (let answerIndex = CFG_PAGE_BAND_SIZE + 1; answerIndex <= answers.length; answerIndex += CFG_PAGE_BAND_SIZE) {
      const answer = answers[answerIndex - 1] || null;
      if (!answer) continue;

      const turnIndex = resolveAnswerWindowStartTurnIndex(answer);
      if (turnIndex < start || turnIndex > end) continue;
      if (map.has(turnIndex)) continue;

      const pageNum = getPageNumberForAnswerIndex(answerIndex);
      map.set(turnIndex, {
        pageNum,
        band: pageBandNameFromPageIndex(Math.max(0, pageNum - 1)),
        answerIndex,
      });
    }
    return map;
  ...
  function createInlinePageDivider(pageNum, band, answerIndex) {
  ...
  function updateSentinelState(win) {
    ...
    const answerRange = buildAnswerRangePayload(win);
    const pageFace = `Page ${S.pageIndex + 1}/${S.pageCount}`;
    const turnBand = pageBandNameFromTurnIndex(Math.max(0, answerRange.start - 1));

    S.ui.topBtn.disabled = !!onFirst;
    S.ui.bottomBtn.disabled = !!onLast;

    S.ui.topBtn.textContent = `↑ Load older · ${pageFace}`;

    S.ui.bottomBtn.textContent = onLast
      ? `↓ Latest page · ${pageFace}`
      : `↓ Load newer · ${pageFace}`;

    if (S.ui.topBox) {
      S.ui.topBox.setAttribute('data-page-index', String(S.pageIndex));
      S.ui.topBox.setAttribute('data-page-count', String(S.pageCount));
      S.ui.topBox.setAttribute('data-page-band', turnBand);
      S.ui.topBox.setAttribute('data-page-window', pageFace);
      S.ui.topBox.style.display = onFirst ? 'none' : '';
    }

    if (S.ui.bottomBox) {
      S.ui.bottomBox.setAttribute('data-page-index', String(S.pageIndex));
      S.ui.bottomBox.setAttribute('data-page-count', String(S.pageCount));
      S.ui.bottomBox.setAttribute('data-page-band', turnBand);
      S.ui.bottomBox.setAttribute('data-page-window', pageFace);
      S.ui.bottomBox.style.display = '';
    }

    S.ui.topBtn.setAttribute('data-page-band', turnBand);
    S.ui.bottomBtn.setAttribute('data-page-band', turnBand);
    S.ui.topBtn.title = onFirst ? 'You are already on the oldest loaded page' : `Load older turns • ${pageFace}`;
    S.ui.bottomBtn.title = onLast ? 'You are on the latest page' : `Load newer turns • ${pageFace}`;
  }
  ...
  function renderPage(nextPageIndex, reason, opts = {}) {
    ...
    for (let i = start; i <= end; i += 1) {
      ...
      if (divider) {
        turnNodes.push(createInlinePageDivider(divider.pageNum, divider.band, divider.answerIndex));
      }
  ...
  function scheduleCommandBarBindRetry() {
    ...
    S.commandBarBindTimer = W.setInterval(() => {
      const api = bindCommandBarFeature();
      if (api) clearCommandBarBindTimer();
    }, 350);
  ...
  function teardownRuntimeSession(reason = 'dispose', opts = {}) {
    ...
    if (!preserveApi && W.H2O_Pagination && W.H2O_Pagination.dispose === API.dispose) {
      try { delete W.H2O_Pagination; } catch (_) { W.H2O_Pagination = undefined; }
    }
