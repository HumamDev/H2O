// ==UserScript==
// @h2o-id             0z1b.data.tab.control.hub.plugin
...
function getApi(){
  ...
  function DATA_scrollBodyTop(panel){
    try{
      const detail = UTIL_q(SEL_CHUB_DETAIL, panel) || panel;
      if (!detail) return;
      if (typeof detail.scrollTo === 'function') detail.scrollTo({ top: 0, behavior: 'auto' });
      else detail.scrollTop = 0;
    } catch {}
  }

  function DATA_getDetailMount(ctx){
    const panel = ctx?.panel || ctx;
    if (!panel) return null;
    const actionWrap = ctx?.actionWrap;
    if (actionWrap && typeof actionWrap.appendChild === 'function') return actionWrap;
    const body = UTIL_q(SEL_CHUB_BODY, panel);
    return body?.parentElement || body || panel;
  }

  function DATA_getBodyMount(panel){
    return (panel && UTIL_q(SEL_CHUB_BODY, panel)) || panel || null;
  }

  function DATA_clearSubtabSurface(panel){
    if (!panel) return;
    try {
      panel.querySelectorAll('[data-h2o-data-subtab-surface="1"]').forEach((el) => {
        try { el.remove(); } catch {}
      });
    } catch {}
  }
...
${P} .h2o-chub-archive-card{margin-top:10px;padding:14px;border-radius:16px;background:linear-gradient(180deg, rgba(11,17,26,.88), rgba(9,13,21,.82));border:1px solid rgba(255,255,255,.08);display:grid;gap:14px}
${P} .h2o-chub-archive-head{display:grid;gap:6px}
${P} .h2o-chub-archive-kicker{font-size:11px;letter-spacing:.14em;text-transform:uppercase;opacity:.62}
${P} .h2o-chub-archive-title{font-size:15px;font-weight:700;line-height:1.25}
${P} .h2o-chub-archive-copy{font-size:12px;line-height:1.45;opacity:.82}
${P} .h2o-chub-archive-grid{display:grid;gap:12px}
${P} .h2o-chub-archive-section{display:grid;gap:8px;padding:10px 12px;border-radius:14px;border:1px solid rgba(255,255,255,.07);background:rgba(255,255,255,.03)}
${P} .h2o-chub-archive-label{font-size:11px;letter-spacing:.12em;text-transform:uppercase;opacity:.62}
${P} .h2o-chub-archive-select{width:100%;border-radius:12px;padding:9px 10px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);color:inherit}
${P} .h2o-chub-archive-status{display:grid;gap:7px}
${P} .h2o-chub-archive-row{display:flex;justify-content:space-between;gap:12px;font-size:12px;line-height:1.35}
${P} .h2o-chub-archive-row strong{font-weight:650;opacity:.78}
${P} .h2o-chub-archive-row code{font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace;font-size:11px}
${P} .h2o-chub-archive-pillrow{display:flex;gap:6px;flex-wrap:wrap}
${P} .h2o-chub-archive-pill{display:inline-flex;align-items:center;padding:4px 9px;border-radius:999px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.05);font-size:11px}
${P} .h2o-chub-archive-pill.warn{border-color:rgba(251,191,36,.4);background:rgba(245,158,11,.14);color:#fde68a}
${P} .h2o-chub-archive-pill.good{border-color:rgba(134,239,172,.38);background:rgba(34,197,94,.14);color:#dcfce7}
${P} .h2o-chub-archive-advanced{border-radius:14px;border:1px solid rgba(255,255,255,.07);background:rgba(255,255,255,.025);overflow:hidden}
${P} .h2o-chub-archive-advanced summary{cursor:pointer;list-style:none;padding:11px 12px;font-size:12px;font-weight:650}
${P} .h2o-chub-archive-advanced summary::-webkit-details-marker{display:none}
${P} .h2o-chub-archive-advanced-body{padding:0 12px 12px;display:grid;gap:8px}
${P} .h2o-chub-archive-toggle{display:flex;justify-content:space-between;gap:10px;align-items:flex-start;font-size:12px;line-height:1.4}
${P} .h2o-chub-archive-toggle input{margin-top:2px}
${P} .h2o-chub-archive-tools{display:flex;gap:8px;flex-wrap:wrap}
${P} .h2o-chub-archive-btn{border-radius:999px;padding:8px 11px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.06);color:inherit;cursor:pointer;font-size:12px;font-weight:650}
${P} .h2o-chub-archive-btn.primary{background:linear-gradient(180deg, rgba(125,211,252,.22), rgba(56,189,248,.18));border-color:rgba(125,211,252,.4)}
${P} .h2o-chub-archive-note{font-size:12px;line-height:1.45;opacity:.82}
${P} .h2o-chub-archive-empty{font-size:12px;line-height:1.45;opacity:.78}
...
function boot(api){
  ...
  if (typeof api.registerPlugin !== 'function') return false;
    ...
    SEL_CHUB_DETAIL = skin.detailSel || `.${CLS}-detail`;


    // -------------------- Data SubTabs (internal) --------------------
    let DATA_ACTIVE_SUBTAB = 'D';
    const DATA_SUBTABS = [
    ...
    const DATA_CONTROLS = [
      ...
      },
      {
        ...
        label:'Capture Snapshot 🗃️',
        help:'Capture what is on screen now and save a fresh archive snapshot for this chat.',
        buttons:[
          { label:'Capture Snapshot Now', primary:true, statusLoading:'Capturing…', action: DATA_captureArchiveAction },
        ],
      },
      {
        type:'action',
        key:'dataArchiveRuntime',
        group:'(A) Snapshot / Archive',
      ...
      },
      {
        ...
        group:'(C) Versions',
        label:'Versions 🧱',
        help:'Versions = labeled snapshots of this chat (saved via Capture).',
        buttons:[
          { label:'Save Labeled Version', primary:true, statusLoading:'Saving…', action: DATA_vaultSaveFromArchiveLatest },
    ...
    function DATA_renderActiveSubtabSurface(panel){
      if (!panel) return;
      DATA_clearSubtabSurface(panel);
      const active = DATA_getSubtab();
      if (active === 'A') {
        DATA_renderArchiveWorkspace(panel);
        return;
      }
      if (active === 'D') {
        DATA_renderSyncOverview(panel);
      }
    }
  ...
  function DATA_getArchiveBootApi(){
    const ab = H2O.archiveBoot;
    if (!ab || typeof ab !== 'object') return null;
    const required = ['captureNow', 'listSnapshots', 'loadSnapshot', 'exportBundle', 'importBundle'];
    for (const fn of required) {
      if (typeof ab[fn] !== 'function') return null;
    }
    return ab;
  }

  function DATA_archiveModuleMissingMessage(){
    return 'Archive module not loaded. Reload page to load 0B3a.⬛️🗄️ Data Archive 🗂️.';
  }

  function DATA_getArchiveUiApi(){
    const ab = DATA_getArchiveBootApi();
    if (!ab) return null;
    const required = ['getPageMode', 'setPageMode', 'getLoadStrategy', 'setLoadStrategy', 'getArchiveStatus'];
    for (const fn of required) {
      if (typeof ab[fn] !== 'function') return null;
    }
    return ab;
  ...
  function DATA_archiveAdvancedDefaults(){
    return {
  ...
  async function DATA_refreshArchiveSnapshotAction(){
    const ab = DATA_getArchiveBootApi();
    if (!ab) return { message: DATA_archiveModuleMissingMessage() };
    const chatId = DATA_archiveChatId();
    await ab.loadLatestSnapshot(chatId);
    if (typeof ab.getPageMode === 'function' && typeof ab.setPageMode === 'function') {
      const pageMode = await ab.getPageMode(chatId);
      if (pageMode && pageMode !== 'live_first') await ab.setPageMode(chatId, pageMode);
    }
    DATA_invalidate();
    return { ok: true, message: 'Snapshot refreshed.' };
  }

  async function DATA_openArchiveSavedChatsAction(){
    const ab = DATA_getArchiveBootApi();
    if (!ab || typeof ab.openSavedChats !== 'function') return { message: DATA_archiveModuleMissingMessage() };
    await ab.openSavedChats({ chatId: DATA_archiveChatId(), view: 'saved' });
    return { ok: true, message: 'Saved chats opened.' };
  }

  function DATA_renderArchiveWorkspace(ctx){
    const panel = ctx?.panel || ctx;
    if (!panel) return;
    const mountRoot = DATA_getBodyMount(panel);
    if (!mountRoot) return;

    const mount = D.createElement('section');
    mount.className = 'h2o-chub-archive-card';
    mount.setAttribute(ATTR_CHUB_ART, '1');
    mount.setAttribute('data-h2o-data-subtab-surface', '1');
    mount.innerHTML = `
      <div class="h2o-chub-archive-head">
        <div class="h2o-chub-archive-kicker">Archive Control Hub</div>
        <div class="h2o-chub-archive-title">Page Mode + Load Strategy</div>
        <div class="h2o-chub-archive-copy">Page Mode decides what transcript world is visible. Load Strategy only decides how that world loads.</div>
      </div>
      <div class="h2o-chub-archive-grid"></div>
    `;
    mountRoot.appendChild(mount);

    const grid = mount.querySelector('.h2o-chub-archive-grid');
    const uiApi = DATA_getArchiveUiApi();
    if (!uiApi) {
      const empty = D.createElement('div');
      empty.className = 'h2o-chub-archive-empty';
      empty.textContent = DATA_archiveModuleMissingMessage();
      grid.appendChild(empty);
      return;
    }

    const chatId = DATA_archiveChatId();
    const pageSection = D.createElement('section');
    pageSection.className = 'h2o-chub-archive-section';
    pageSection.innerHTML = `
      <div class="h2o-chub-archive-label">Page Mode</div>
      <select class="h2o-chub-archive-select" data-h2o-archive-page-mode>
        <option value="live_first">Live</option>
        <option value="archive_first">Archive Preview</option>
        <option value="archive_only">Archive Only</option>
      </select>
      <div class="h2o-chub-archive-note">Live shows native chat only. Archive Preview shows one archive-owned preview transcript. Archive Only hides the live page and shows archive only.</div>
    `;
    const strategySection = D.createElement('section');
    strategySection.className = 'h2o-chub-archive-section';
    strategySection.innerHTML = `
      <div class="h2o-chub-archive-label">Load Strategy</div>
      <select class="h2o-chub-archive-select" data-h2o-archive-load-strategy>
        <option value="rebuild_first">Safe</option>
        <option value="cache_first">Fast</option>
        <option value="auto">Auto</option>
      </select>
      <div class="h2o-chub-archive-note">Safe favors stable rebuild-style loading. Fast prefers snapshot-first loading. Auto prefers Fast when a usable snapshot exists.</div>
    `;
    const statusSection = D.createElement('section');
    statusSection.className = 'h2o-chub-archive-section';
    statusSection.innerHTML = `
      <div class="h2o-chub-archive-label">Status</div>
      <div class="h2o-chub-archive-pillrow" data-h2o-archive-pills></div>
      <div class="h2o-chub-archive-status" data-h2o-archive-status></div>
    `;
    const advancedWrap = D.createElement('details');
    advancedWrap.className = 'h2o-chub-archive-advanced';
    advancedWrap.innerHTML = `
      <summary>Advanced</summary>
      <div class="h2o-chub-archive-advanced-body" data-h2o-archive-advanced-body></div>
    `;
    const toolsSection = D.createElement('section');
    toolsSection.className = 'h2o-chub-archive-section';
    toolsSection.innerHTML = `
      <div class="h2o-chub-archive-label">Utilities</div>
      <div class="h2o-chub-archive-tools" data-h2o-archive-tools></div>
    `;

    grid.append(pageSection, strategySection, statusSection, advancedWrap, toolsSection);

    const pageSelect = pageSection.querySelector('[data-h2o-archive-page-mode]');
    const strategySelect = strategySection.querySelector('[data-h2o-archive-load-strategy]');
    const statusBody = statusSection.querySelector('[data-h2o-archive-status]');
    const pills = statusSection.querySelector('[data-h2o-archive-pills]');
    const advancedBody = advancedWrap.querySelector('[data-h2o-archive-advanced-body]');
    const tools = toolsSection.querySelector('[data-h2o-archive-tools]');

    const advancedDefs = [
      ['autoCaptureWhenSnapshotMissing', 'Auto-capture when snapshot is missing'],
      ['allowStaleSnapshotBoot', 'Allow stale snapshot boot'],
      ['keepPreviewUntilNativeStable', 'Keep preview until native is stable'],
      ['forceFallbackOnMountFailure', 'Force fallback on mount failure'],
      ['forceFallbackOnHostFailure', 'Force fallback on host failure'],
      ['showFallbackReason', 'Show fallback reason'],
      ['showSnapshotMeta', 'Show snapshot id and baseline count'],
      ['debugArchiveEvents', 'Debug archive events'],
    ];
    ...
    const refreshUi = async () => {
      const status = await uiApi.getArchiveStatus(chatId);
      const advanced = typeof uiApi.getAdvancedSettings === 'function'
        ? { ...DATA_archiveAdvancedDefaults(), ...(uiApi.getAdvancedSettings() || {}) }
        : DATA_archiveAdvancedDefaults();

      pageSelect.value = String(status?.pageMode || 'live_first');
      strategySelect.value = String(status?.requestedStrategy || 'rebuild_first');

      pills.textContent = '';
      const pillDefs = [
        [`Mode: ${status?.pageModeLabel || 'Live'}`, ''],
        [`Strategy: ${status?.requestedStrategyLabel || 'Safe'}`, ''],
        [`Effective: ${status?.effectiveStrategyLabel || 'Safe'}`, ''],
        [`Source: ${status?.activeSourceLabel || 'Native'}`, status?.activeSource === 'native' ? 'good' : ''],
        [`Snapshot: ${status?.snapshotStateLabel || 'Missing'}`, status?.snapshotReady ? 'good' : 'warn'],
        [`Fallback: ${status?.fallbackReasonLabel || 'None'}`, String(status?.fallbackReason || '') === 'none' ? '' : 'warn'],
      ];
      for (const [text, tone] of pillDefs) {
        const pill = D.createElement('span');
        pill.className = `h2o-chub-archive-pill ${tone}`.trim();
        pill.textContent = text;
        pills.appendChild(pill);
      }

      statusBody.textContent = '';
      const rows = Array.isArray(status?.statusLines) ? status.statusLines : [];
      for (const row of rows) {
        const line = D.createElement('div');
        line.className = 'h2o-chub-archive-row';
        const key = D.createElement('strong');
        key.textContent = String(row?.key || '');
        const value = D.createElement('span');
        const text = String(row?.value || '');
        if (/snapshot id/i.test(String(row?.key || ''))) {
      ...
      help.className = 'h2o-chub-archive-note';
      help.textContent = String(status?.helpText || '');
      statusBody.appendChild(help);

      advancedBody.textContent = '';
      for (const [key, label] of advancedDefs) {
        const row = D.createElement('label');
        row.className = 'h2o-chub-archive-toggle';
        const text = D.createElement('span');
        text.textContent = label;
        const input = D.createElement('input');
        input.type = 'checkbox';
        input.checked = !!advanced[key];
        input.addEventListener('change', async () => {
          if (typeof uiApi.setAdvancedSettings === 'function') {
            uiApi.setAdvancedSettings({ [key]: !!input.checked });
            DATA_invalidate();
          }
        });
      ...
      tools.textContent = '';
      for (const [label, tone, action] of toolDefs) {
        const btn = D.createElement('button');
        btn.type = 'button';
        btn.className = `h2o-chub-archive-btn ${tone}`.trim();
    ...
    pageSelect.addEventListener('change', async () => {
      await uiApi.setPageMode(chatId, pageSelect.value || 'live_first');
      DATA_invalidate();
    });
    strategySelect.addEventListener('change', async () => {
      await uiApi.setLoadStrategy(chatId, strategySelect.value || 'rebuild_first', { userInitiated: true });
      DATA_invalidate();
    });

    void refreshUi().catch((e) => {
      statusBody.textContent = '';
      const fail = D.createElement('div');
      fail.className = 'h2o-chub-archive-empty';
      fail.textContent = String(e?.message || e || 'Archive status unavailable.');
      statusBody.appendChild(fail);
    });
  }

  function DATA_toExportSnapshot(snapshot, chatIdHint){
    const src = snapshot && typeof snapshot === 'object' ? snapshot : null;
    if (!src) return null;
    const chatId = String(src.chatId || chatIdHint || H2O.util?.getChatId?.() || 'unknown');
    const messages = Array.isArray(src.messages) ? src.messages : [];
    return {
      schema: 'H2O.archive.v1',
      chatId,
      capturedAt: String(src.createdAt || src.capturedAt || new Date().toISOString()),
      href: location.href,
      messages: messages.map((m) => ({
        id: '',
        role: String(m?.role || 'assistant'),
        text: String(m?.text || ''),
        create_time: m?.createdAt ?? null,
      })),
    };
  }

  async function DATA_pickLatestArchiveSnapshot(){
    const ab = DATA_getArchiveBootApi();
    if (!ab) return { snapshot: null, reason: DATA_archiveModuleMissingMessage() };
    const chatId = String(H2O.util?.getChatId?.() || '');
    const rows = await ab.listSnapshots(chatId);
    if (!Array.isArray(rows) || !rows.length) return { snapshot: null, reason: 'No saved archive yet (Capture first).' };
    const sid = String(rows[0]?.snapshotId || '');
    if (!sid) return { snapshot: null, reason: 'Invalid snapshot list.' };
    const snapshot = await ab.loadSnapshot(sid);
    return { snapshot: DATA_toExportSnapshot(snapshot, chatId), chatId };
  }
  ...
  function DATA_getSummaryLines(){
    ...
    try {
      const ab = DATA_getArchiveBootApi();
      if (!ab) {
        lines.push(DATA_archiveModuleMissingMessage());
      } else if (typeof ab.list === 'function') {
        const archiveIndex = ab.list();
        if (Array.isArray(archiveIndex) && archiveIndex.length) {
          lines.push(`Archive index: ${archiveIndex.length} saved chat${archiveIndex.length === 1 ? '' : 's'}.`);
        }
      }
    } catch {}
    try {
      const ab = DATA_getArchiveBootApi();
      const latest = (ab && typeof ab.getLatest === 'function') ? ab.getLatest() : null;
      if (latest && typeof latest === 'object') {
        const when = DATA_formatDate(latest.capturedAt || latest.createdAt);
    ...
    try {
      ...
      if (Array.isArray(v) && v.length) lines.push(`Versions: ${v.length} snapshot${v.length === 1 ? '' : 's'} (current chat).`);
    } catch {}
    return lines;
  }
  ...
  async function DATA_captureArchiveAction(){
    const ab = DATA_getArchiveBootApi();
    if (!ab) return { message: DATA_archiveModuleMissingMessage() };
    const chatId = String(H2O.util?.getChatId?.() || '');
    const res = await ab.captureNow(chatId);
    const count = Number(res?.messageCount || 0);
    if (res?.ok === false) return { message: res?.message || res?.error || 'Archive capture failed.' };
    if (res?.deduped) return { ok: true, message: `No content changes (${count} message${count === 1 ? '' : 's'}).` };
    return { ok: true, message: `Captured ${count} message${count === 1 ? '' : 's'}.` };
  }
  ...
  function DATA_reloadPageAction(){
    ...
    return { ok: true, message: 'Reloading page…' };
  }
  ...
  async function DATA_exportLatestArchive2(kind){
    const ab = DATA_getArchiveBootApi();
    if (!ab) return { message: DATA_archiveModuleMissingMessage() };
    const picked = await DATA_pickLatestArchiveSnapshot();
    if (!picked.snapshot) return { message: picked.reason || 'No saved archive yet (Capture first).' };
  ...
  function DATA_vaultSaveFromArchiveLatest(){
    const v = H2O.vault;
    if (!v || typeof v.saveFromArchiveLatest !== 'function') return { message: 'Versions not available (update H2O Data core).' };
    const label = prompt('Vault label (optional):', '');
    const res = v.saveFromArchiveLatest({ label: String(label || '').trim() });
    if (!res?.ok) return { message: `Version save failed (${res?.reason || 'unknown'}).` };
    return { ok: true, message: `Version saved: ${res.vid}${res?.entry?.label ? ` — ${res.entry.label}` : ''}` };
  }

  function DATA_vaultList(){
    const v = H2O.vault;
    if (!v || typeof v.list !== 'function') return { message: 'Versions not available.' };
    const items = v.list() || [];
    if (!items.length) return { message: 'No versions saved for this chat yet.' };
    ...
    alert(`Versions (latest 10)\n\n${top}`);
    return { ok: true, message: `Versions: ${items.length} snapshot${items.length === 1 ? '' : 's'}.` };
  }

  async function DATA_exportLatestArchive(format){
...
function DATA_openTextModal(title, initialText, opts = {}) {
  ...
  function DATA_renderWebDAVStatusPill(ctx){
    const panel = ctx?.panel || ctx;
    const mountRoot = DATA_getBodyMount(panel);
    ...
    pill.setAttribute('data-h2o-data-subtab-surface', '1');
...
function DATA_renderSyncOverview(ctx){
  ...
  try {
    ...
    wrap.setAttribute('data-h2o-data-subtab-surface', '1');
