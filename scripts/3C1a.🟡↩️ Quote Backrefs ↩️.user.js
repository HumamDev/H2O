// ==UserScript==
// @h2o-id      3c.quote.backrefs
// @name         3C.🟡↩️ Quote Backrefs ↩️
// @namespace    H2O.Prime.CGX.MarginAnchor.Addon.QuoteBackrefs
// @version      0.2.5
...
(() => {
  ...
  const CSS_ = Object.freeze({
    ...
    VAR_LANE_X:   `--cgxui-${SkID}-lane-x`, // x position inside MA gutter width (percentage)
  ...
  const CFG_ = Object.freeze({
    ...
    // marker layout / lane placement (left of MA "status/symbol" lane)
    LANE_X_PCT: '22%',
  ...
  function CSS_text() {
    ...
    return `
      ...
      /* Quote Backref marker — dedicated lane LEFT of MA lanes */
      ${selScoped(UI_.MARK)}{
        position: absolute;
        left: 0;
        transform: translateX(calc(-100% - ${CFG_.LANE_OUT_GAP_PX}px));
