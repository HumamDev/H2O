// ==UserScript==
// @h2o-id      2a.question.wrapper
// @name         2A.🟧🧵 Question Wrapper 🧵
// @namespace    H2O.Prime.CGX.QuestionWrapper
// @version      5.4.1
...
function DOM_extractChatGPTQuoteNode(msgEl) {
  ...
  function DOM_scheduleScan(reason = '') {
    ...
    if ('requestIdleCallback' in window) {
      ...
      st.scanIsIdle = false;
      st.scanHandle = setTimeout(run, CFG_QWRAP_.FALLBACK_SCAN_DELAY_MS);
    }
  }
...
function DOM_runScan(reason = '') {
  ...
  try {
    ...
    if (st.scanQueued && !st.scrolling) DOM_scheduleScan('queued');
  }
}
...
function CORE_boot() {
  ...
  const startObserver = () => {
    if (st.qwrapMO || !document.body) return false;
  ...
  if (!startObserver()) {
    ...
    const tick = () => {
      if (cancelled || st.qwrapMO) return;
