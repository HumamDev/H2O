// ==UserScript==
// @h2o-id             1a2b.navigation.controls.minimap.add.on
// @name               1A2b.🟥💠🗺️ Navigation Controls (MiniMap ➕ Add-on) 🗺️
// @namespace          H2O.Premium.CGX.navigation.controls.minimap.add.on
// @author             HumamDev
// @version            1.3.9
// @revision           001
// @build              260304-102754
...
(() => {
  ...
  const CFG_ = Object.freeze({
    ...
    // Leave one higher layer free for the Workspace Dock overlay.
    FLOAT_Z_INDEX: 2147483645,
  ...
  const DOCK_ = Object.freeze({
    REG_TOP: `${SkID}-dock-top`,
    REG_LEFT: `${SkID}-dock-left`,
    REG_RIGHT: `${SkID}-dock-right`,
  });
  ...
  function VIEW_isSearchPanelOpen() {
    const cands = Array.from(document.querySelectorAll(VIEW_.SEARCH_SEL));
  ...
  function UTIL_pickComposerTextarea() {
    ...
    for (const el of candidates) {
      ...
      if (hostForm && form && hostForm === form) score += 4;
  ...
  function UTIL_pickComposerForm() {
    ...
    // Fallback: visible input host form if direct composer selectors miss.
    const prompt = document.getElementById?.('prompt-textarea') ||
      Array.from(document.querySelectorAll(SEL_.TEXTAREA)).find((el) => UTIL_isElementVisible(el)) ||
      null;
    const inputForm = prompt?.closest?.('form') || null;
    if (inputForm && UTIL_isElementVisible(inputForm)) return inputForm;

    // Last fallback: visible bottom-lane form that has a send button.
    let tail = null;
    let tailScore = -Infinity;
    for (const f of document.querySelectorAll(SEL_.ANY_FORM)) {
      if (!UTIL_isElementVisible(f)) continue;
  ...
  /** @helper */
  function MM_listRuntimeTurns() {
    try {
      const turns = W.H2O?.turnRuntime?.listTurns?.();
  ...
  /** @helper */
  function MM_nav_getAnswerIdFromEl(el) {
    if (!el) return '';
    const direct =
  ...
  /** @helper */
  function MM_listRuntimeAnswers() {
    const out = [];
    for (const turn of MM_listRuntimeTurns()) {
      ...
      if (answers.length) {
        for (const answer of answers) {
          ...
          const el = answer?.el || null;
          if (answerId || el) out.push({ answerId: answerId || MM_nav_getAnswerIdFromEl(el), el, turnId });
        }
        continue;
      }
      const el = turn?.answerEl || turn?.primaryAEl || null;
  ...
  function MM_nav_jumpFirst() {
    ...
    const answers = MM_listRuntimeAnswers();
    if (!answers.length) return false;
    const first = answers[0] || null;
    return MM_nav_jumpToAnswerId(first?.answerId || first?.el, { smooth: true, flash: true });
  }
  ...
  function MM_nav_jumpLast() {
    ...
    return MM_nav_jumpToAnswerId(last?.answerId || last?.el, { smooth: true, flash: true });
  }
  ...
  const S = VAULT.state;
  S.dockMode = !!S.dockMode;
  S.dockBridgeWired = !!S.dockBridgeWired;
  S.dockRegActive = !!S.dockRegActive;

  /** @helper */
  function UTIL_setStateToken(el, token, on) {
    if (!el || !token) return;
    const t = String(token).trim();
    if (!t) return;
    const raw = String(el.getAttribute(ATTR_.CGX_STATE) || '').trim();
    const set = new Set(raw ? raw.split(/\s+/g) : []);
    if (on) set.add(t);
    else set.delete(t);
    if (set.size) el.setAttribute(ATTR_.CGX_STATE, Array.from(set).join(' '));
    else el.removeAttribute(ATTR_.CGX_STATE);
  }
  ...
  /** @helper */
  function UI_applyBaseBoxStyles(navBoxEl, navBoxLeft, navBoxRight, dockMode = false) {
    const basePos = dockMode ? 'relative' : 'fixed';
    const baseZ = dockMode ? 'auto' : String(CFG_.FLOAT_Z_INDEX);
    ...
    navBoxRight.style.cssText = `
      ...
      bottom: auto;
    `;
    UTIL_setStateToken(navBoxEl, 'dock', dockMode);
    UTIL_setStateToken(navBoxLeft, 'dock', dockMode);
    UTIL_setStateToken(navBoxRight, 'dock', dockMode);
    UTIL_setStateToken(navBoxEl, 'slot-top', dockMode);
    UTIL_setStateToken(navBoxLeft, 'slot-left', dockMode);
    UTIL_setStateToken(navBoxRight, 'slot-right', dockMode);
  }
  ...
  function getIdsForColorInDomOrder(colorName) {
    ...
    // 3) Preserve canonical runtime order across assistant answers
    const answers = MM_listRuntimeAnswers();
    return answers
      .map(({ answerId, el }) => String(answerId || MM_nav_getAnswerIdFromEl(el) || '').trim())
  ...
  function getCurrentAssistantId() {
    ...
    const answers = MM_listRuntimeAnswers().filter(({ el }) => !!el);
    ...
    const chosen = bestCrossingTop || bestBelowTop || answers[0] || null;
    return String(chosen?.answerId || MM_nav_getAnswerIdFromEl(chosen?.el) || '').trim();
  }
  ...
  function UTIL_getComposerAnchor(anchorHost) {
    ...
    const bestSurface = UTIL_pickComposerSurface(ta || null);
    const surface =
      bestSurface ||
  ...
  /** @helper */
  function UTIL_rectOverlaps(a, b, pad = 0) {
    if (!a || !b) return false;
    const g = Number.isFinite(pad) ? Number(pad) : 0;
    return !(
      (a.right + g) <= b.left ||
      (a.left - g) >= b.right ||
      (a.bottom + g) <= b.top ||
  ...
  /** @helper */
  function UTIL_getArchiveInfoPanelRect() {
    try {
      if (!D.documentElement?.classList?.contains?.('h2o-archive-info-open')) return null;
      const panel =
        D.querySelector('.h2o-archive-dock .infoWrap.open .infoPanel') ||
        D.querySelector('.h2o-archive-dock .infoPanel[aria-hidden="false"]') ||
        null;
  ...
  /** @helper */
  function UTIL_isArchiveInfoOpen() {
    try {
      return !!D.documentElement?.classList?.contains?.('h2o-archive-info-open');
    } catch {
      return false;
    }
  }
  ...
  /** @helper */
  function DOCK_getApi() {
    return W.H2O?.InputDock?.api || null;
  }

  /** @helper */
  function DOCK_applyVisibility() {
    if (!S.navBoxEl || !S.navBoxLeft || !S.navBoxRight) return;
    if (!VIEW_shouldShow()) {
      S.navBoxEl.style.display = 'none';
      S.navBoxLeft.style.display = 'none';
      S.navBoxRight.style.display = 'none';
      return;
    }

    S.navBoxEl.style.display = 'flex';
    S.navBoxEl.style.flexShrink = '0';
    S.navBoxEl.style.marginLeft = 'auto';
    S.navBoxEl.style.marginRight = '0';
    S.navBoxEl.style.left = 'auto';
    S.navBoxEl.style.right = 'auto';
    S.navBoxEl.style.top = 'auto';
    S.navBoxEl.style.bottom = 'auto';

    const showLeft = (!UTIL_isArchiveInfoOpen()) && !!CFG_.ENABLE_SCROLL_PAIR_BUTTONS && S.navBoxLeft.children.length > 0;
    S.navBoxLeft.style.display = showLeft ? 'flex' : 'none';
    // Keep right lane visible in dock mode so Color Legend can reliably mount.
    S.navBoxRight.style.display = 'flex';
    S.navBoxLeft.style.left = 'auto';
    S.navBoxLeft.style.right = 'auto';
    S.navBoxLeft.style.top = 'auto';
    S.navBoxLeft.style.bottom = 'auto';
    S.navBoxRight.style.left = 'auto';
    S.navBoxRight.style.right = 'auto';
    S.navBoxRight.style.top = 'auto';
    S.navBoxRight.style.bottom = 'auto';
  }

  /** @helper */
  function DOCK_unregisterBoxes(apiRaw) {
    const api = apiRaw || DOCK_getApi();
    if (!api || typeof api.unregister !== 'function') return;
    try { api.unregister(DOCK_.REG_TOP); } catch {}
    try { api.unregister(DOCK_.REG_LEFT); } catch {}
    try { api.unregister(DOCK_.REG_RIGHT); } catch {}
    S.dockRegActive = false;
  }

  /** @helper */
  function DOCK_applyContainerModeStyles(dockMode) {
    const pos = dockMode ? 'relative' : 'fixed';
    const z = dockMode ? 'auto' : String(CFG_.FLOAT_Z_INDEX);
    for (const el of [S.navBoxEl, S.navBoxLeft, S.navBoxRight]) {
      if (!el) continue;
      el.style.position = pos;
      el.style.zIndex = z;
      el.style.flexShrink = dockMode ? '0' : '';
      el.style.marginLeft = dockMode && el === S.navBoxEl ? 'auto' : '';
      el.style.marginRight = dockMode && el === S.navBoxEl ? '0' : '';
      el.style.top = 'auto';
      el.style.right = 'auto';
      el.style.left = 'auto';
      el.style.bottom = 'auto';
    }
    UTIL_setStateToken(S.navBoxEl, 'dock', dockMode);
    UTIL_setStateToken(S.navBoxLeft, 'dock', dockMode);
    UTIL_setStateToken(S.navBoxRight, 'dock', dockMode);
    UTIL_setStateToken(S.navBoxEl, 'slot-top', dockMode);
    UTIL_setStateToken(S.navBoxLeft, 'slot-left', dockMode);
    UTIL_setStateToken(S.navBoxRight, 'slot-right', dockMode);
  }

  /** @helper */
  function DOCK_setMode(on) {
    if (!S.navBoxEl || !S.navBoxLeft || !S.navBoxRight) return;
    const next = !!on;
    if (S.dockMode === next) {
      if (next) DOCK_applyVisibility();
      return;
    }
    S.dockMode = next;
    DOCK_applyContainerModeStyles(next);
    if (next) {
      detachAnchorObservers();
      DOCK_applyVisibility();
      return;
    }
  ...
  /** @helper */
  function DOCK_registerBoxes(api) {
    if (!api || typeof api.register !== 'function') return false;
    if (!S.navBoxEl || !S.navBoxLeft || !S.navBoxRight) return false;

    const okTop = !!api.register({
      id: DOCK_.REG_TOP,
      slot: 'top',
      el: S.navBoxEl,
      mode: 'move',
    ...
    S.dockRegActive = !!(okTop || okLeft || okRight);
    return !!okTop;
  }

  /** @helper */
  function DOCK_trySync() {
    if (!VIEW_shouldShow()) {
      if (S.dockMode || S.dockRegActive) DOCK_unregisterBoxes();
      DOCK_setMode(false);
      if (S.navBoxEl) S.navBoxEl.style.display = 'none';
      if (S.navBoxLeft) S.navBoxLeft.style.display = 'none';
      if (S.navBoxRight) S.navBoxRight.style.display = 'none';
      return false;
    }
    const api = DOCK_getApi();
    const canDock = !!(
      api &&
    ...
    if (!canDock) {
      if (S.dockMode || S.dockRegActive) DOCK_unregisterBoxes(api);
      DOCK_setMode(false);
      return false;
    }

    try {
      const top = api.getSlot?.('top');
      const left = api.getSlot?.('left');
      const right = api.getSlot?.('right');
      if (
        S.dockMode &&
        S.dockRegActive &&
        top && left && right &&
        S.navBoxEl?.parentElement === top &&
        S.navBoxLeft?.parentElement === left &&
        S.navBoxRight?.parentElement === right
      ) {
        DOCK_applyVisibility();
        return true;
      }
    ...
    DOCK_setMode(true);
    const ok = DOCK_registerBoxes(api);
    if (!ok) {
      DOCK_unregisterBoxes(api);
      DOCK_setMode(false);
      return false;
    }
    DOCK_applyVisibility();
  ...
  /** @helper */
  function DOCK_installBridge() {
    if (S.dockBridgeWired) return;
    S.dockBridgeWired = true;
  ...
  function scheduleLayout(navBoxEl, navBoxLeft, navBoxRight) {
    if (!navBoxEl || !navBoxLeft || !navBoxRight) return;
    if (!VIEW_shouldShow()) {
  ...
  function positionNavBoxLeft(navBoxLeft, anchorHost) {
    ...
    const bh = navBoxLeft.getBoundingClientRect().height || 46;
    ...
    const archiveInfoRect = UTIL_getArchiveInfoPanelRect();
    if (archiveInfoRect) {
      const navRect = { left: leftPx, top: topPx, right: (leftPx + bw), bottom: (topPx + bh) };
      if (UTIL_rectOverlaps(navRect, archiveInfoRect, 2)) {
        navBoxLeft.style.display = 'none';
        return;
      }
    }
  ...
  function CORE_bindLayoutReactions() {
    ...
    UTIL_on(W, 'popstate', () => UTIL_followFor(12), { passive: true });
    UTIL_on(W, 'hashchange', () => UTIL_followFor(12), { passive: true });
    UTIL_on(W, 'pageshow', () => UTIL_followFor(12), { passive: true });
  ...
  function CORE_bindSyncHooks() {
    ...
    const evIndexHydrated = 'evt:h2o:minimap:index:hydrated';
    const evIndexAppended = 'evt:h2o:minimap:index:appended';
    ...
    listenDual(evIndexHydrated, () => { onSync(); scheduleLayout(S.navBoxEl, S.navBoxLeft, S.navBoxRight); }, { passive: true });
    listenDual(evIndexAppended, () => { onSync(); scheduleLayout(S.navBoxEl, S.navBoxLeft, S.navBoxRight); }, { passive: true });
  ...
  function CORE_dispose() {
    try { DOCK_unregisterBoxes(); } catch {}
