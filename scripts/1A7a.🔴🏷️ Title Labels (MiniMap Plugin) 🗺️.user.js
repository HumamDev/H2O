// ==UserScript==
// @h2o-id      1a7.title.labels.minimap.plugin
// @name         1A7.🔴🏷️ Title Labels (MiniMap Plugin) 🗺️
// @namespace    h2o.prm.cgx.tlabels
// @version      1.5.0
// @description  MiniMap Titles plugin: sticky title label panels + tooltip sync via evt:h2o:title:set. Source of truth = shared titles store (Answer Title writer).
...
(() => {
  ...
  // IMPORTANT: this plugin deliberately uses MiniMap skin id for UI hooks, so existing MiniMap CSS applies.
  const SkID  = 'mnmp';
  ...
  const SEL_ = Object.freeze({
    ...
    MM_BTN_BY_ID: (id) => `${SEL_.MM_BTN}[${ATTR_.ID}="${CSS.escape(String(id))}"]`,
    MM_BTN_BY_PRIMARY_A_ID: (id) => `${SEL_.MM_BTN}[${ATTR_.PRIMARY_A_ID}="${CSS.escape(String(id))}"]`,
  });
  ...
  const KEY_ = Object.freeze({
    // shared canonical key (same as Answer Title module)
  ...
  function setTitleOnMiniMap(answerOrTurnId, title) {
    ...
    const btn = D.querySelector(SEL_.MM_BTN_BY_ID(id)) || D.querySelector(SEL_.MM_BTN_BY_PRIMARY_A_ID(id));
  ...
  // The only "write" path: emit the canonical event. The Answer Title module persists to disk.
...
`;
  ...
  function ensureStickyTitlePanel(answerId, btn) {
    ...
    if (!panel || !panel.isConnected) {
      ...
      // 🧩 HTML (exact structure)
      // <div class="cgxui-title-pill" data-cgxui-owner="SkID" data-cgxui="mm-title-panel">
      //   <div class="cgxui-title-pill-dot" data-cgxui-owner="SkID" data-cgxui="mm-title-dot"></div>
      //   <input class="cgxui-title-pill-input" data-cgxui-owner="SkID" data-cgxui="mm-title-input" type="text" readonly />
      // </div>
  ...
  function positionStickyPanelForButton(panel, btn) {
    ...
    // Match MiniMap row button height exactly so label and button stay aligned.
    const r = btn.getBoundingClientRect();
    const h = Math.max(1, Math.round(r.height || 0));
    if (h > 0) panel.style.height = `${h}px`;

    // Keep right edge radius consistent with MiniMap row buttons.
    ...
    // Use half the row height for the left cap, avoiding radius compression on the right.
  ...
  function repositionAllStickyPanels() {
    stickyTitlePanels.forEach((panel, id) => {
      ...
      // Try to find the button for this id
  ...
  function onTitleSet(detail) {
    ...
    // Also mirror to turnId when known (for tooltip lookup convenience only)
    try {
    ...
    // Update MiniMap tooltip immediately (both answerId + possible turnId)
  ...
  function syncMiniMapTooltipsOnce() {
    ...
    for (const btn of btns) {
      ...
      const aId = normalizeId(btn.dataset.primaryAId);
      const key = aId || id;
      const t = getCurrentTitleForId(key);
      if (t) btn.setAttribute(ATTR_.TITLE, t);
    }
  }

  // Observe minimap container for new buttons (remount-safe)
  const mo = new MutationObserver(UTIL_debounce(() => { syncMiniMapTooltipsOnce(); repositionAllStickyPanels(); }, 120));
