// ==UserScript==
// @h2o-id             s2z1a.question.timestamp.studio
// @name               S2Z1a. 🎬 Question Timestamp - Studio
...
function DOM_QT_readStampedCreateTime(userMsgDiv) {
  const nodes = [
    userMsgDiv,
    userMsgDiv?.closest?.('[data-h2o-create-time]'),
    userMsgDiv?.querySelector?.('[data-h2o-create-time]'),
    DOM_QT_findTurnRoot(userMsgDiv)
  ].filter(Boolean);
...
function DOM_QT_findQuestionAnchor(userMsgDiv) {
  ...
  function DOM_QT_ensureTimestamp(userMsgDiv) {
    const stampedTs = DOM_QT_readStampedCreateTime(userMsgDiv);
    const nativeTs =
      W.H2O?.time?.getCreateTime?.(userMsgDiv) ||
      DOM_QT_findCreateTimeFromReact(userMsgDiv);

    const ts = stampedTs || nativeTs || 0;
    if (!ts) return null;
  ...
  function DOM_QT_scan(users = null) {
    ...
    const scope = DOM_QT_getConversationRoot() || document;

    const turns = scope.querySelectorAll(`${SEL_QTIMESTAMP_.TURN_ROOT_A},${SEL_QTIMESTAMP_.TURN_ROOT_B}`);
    turns.forEach(DOM_QT_bindTurn);

    const usersAll = scope.querySelectorAll(SEL_QTIMESTAMP_.USER_MSG);
