// ==UserScript==
// @h2o-id             s2a1a.question.wrapper.studio
// @name               S2A1a. 🎬 Question Wrapper - Studio
...
(() => {
  ...
  function DOM_getStudioConversationRoot() {
    try {
      return (
  ...
  function DOM_getScanScopeRoot() {
    if (UTIL_isStudioMode()) {
      const root = DOM_getStudioConversationRoot();
      if (root) return root;
    }
    return document;
  }

  function DOM_getObserverRoot() {
    if (UTIL_isStudioMode()) {
      const root = DOM_getStudioConversationRoot();
  ...
  function UTIL_qsa(sel, root = null) {
    const scope = root || DOM_getScanScopeRoot();
...
function DOM_extractChatGPTQuoteNode(msgEl) {
  ...
  function DOM_hasUnwrappedUsers() {
    const root = DOM_getScanScopeRoot();
    try { return !!root.querySelector?.(SEL_QWRAP_.NOT_WRAPPED_USER); } catch {}
    return false;
  }
...
function CORE_boot() {
  ...
  const ensureChipObserver = () => {
    if (st.isStudio) return true; // no live composer chip in Studio reader
  ...
  const startObserver = () => {
    const obsRoot = DOM_getObserverRoot();
    if (!obsRoot) return false;
    ...
    qwrapMO.observe(obsRoot, { childList: true, subtree: true });
    st.qwrapMO = qwrapMO;
    st.cleanup.push(() => {
