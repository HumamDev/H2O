// ==UserScript==
// @h2o-id             s0d3a.transcript.archive.engine.studio
// @name               S0D3a. 🎬 Transcript Archive Engine - Studio
...
(() => {
  ...
  async function captureNow(chatIdRaw) {
    ...
    const capturedAt = hints.updatedAt;
    const meta = {
      href: W.location.href,          // kept for backward compat
      sourceUrl: W.location.href,     // canonical field (matches mobile SnapshotMeta)
      title: D.title || "",
      source: "browser",              // was: "dom" — mobile reads this to determine origin
      sourceType: "chatgpt-live-capture",
      capturedAt,                     // explicit in meta for cross-platform reading
    ...
    // chatgptId: native ChatGPT UUID enables future cross-platform dedup.
    // Only written for real UUID-style chat IDs, not for the path-hash fallback.
    if (!chatId.startsWith("path:")) meta.chatgptId = chatId;
