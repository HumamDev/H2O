/* H2O Desktop Sync - F13.0.6 binding convergence bookkeeping
 *
 * Desktop/Tauri-only finalization for one successful reviewed binding-add
 * convergence result.
 *
 * Safety invariants:
 *   - Bookkeeping only. No bind, no second mutation, no publication, no
 *     enqueue, no upload/download, no WebDAV, no remote mutation, no timers,
 *     no polling, and no mobile write-back.
 *   - Requires bindingResult.bound === true, valid binding/chat/folder subject
 *     hashes, lineageId, and a valid redacted binding applyEvent receipt source.
 *   - Records exactly two durable rows after receipt validation:
 *       1. consumed-operation ledger row with consumedStatus === "consumed"
 *       2. convergence watermark row for the binding subject/revision
 *   - applyEvent is evidence of a completed local binding add, never a remote
 *     apply command.
 */
(function (global) {
  'use strict';
  ...
  if (H2O.Desktop.Sync.__bindingConvergenceBookkeepingInstalled) return;

  var SCHEMA = 'h2o.desktop.sync.binding-convergence-bookkeeping.v1';
  ...
  function peerIdFromApplyEvent(envelope) {
    ...
    return cleanLower(peer.syncPeerIdHash);
  }

  function postStateHash(envelope) {
    var payload = safeObject(envelope.payload);
    var postState = safeObject(payload.postState);
    return cleanLower(payload.postStateHash || postState.hash);
  }
  ...
  function validateBindingResult(result, blockers, warnings) {
    ...
    if (!isSha256Hex(row.bindingSubjectId)) addCode(blockers, 'bindingSubjectId-invalid');
    ...
    if (!cleanString(row.auditMaintenanceId)) addCode(blockers, 'auditMaintenanceId-required');
    if (!cleanString(row.transactionId)) addCode(blockers, 'transactionId-required');
    var forbidden = foreverNoKey(row);
    if (forbidden) {
      addCode(blockers, 'binding-result-contains-forbidden-field');
  ...
  function validateApplyEvent(envelope, expectedSubjectId, expectedLineageId, blockers, warnings) {
    ...
    if (cleanString(env.operationIntent) !== 'create') addCode(blockers, 'operationIntent-invalid');
    if (!isSha256Hex(env.subjectId)) addCode(blockers, 'applyEvent-subjectId-invalid');
    if (expectedSubjectId && cleanLower(env.subjectId) !== expectedSubjectId) {
    ...
    if (!isStateHash(payload.preStateHash)) addCode(blockers, 'applyEvent-preStateHash-invalid');
    if (!isStateHash(payload.postStateHash)) addCode(blockers, 'applyEvent-postStateHash-invalid');
    if (!isSha256Hex(payload.bindingSubjectId)) addCode(blockers, 'applyEvent-bindingSubjectId-invalid');
    if (!isSha256Hex(payload.chatSubjectId)) addCode(blockers, 'applyEvent-chatSubjectId-invalid');
    if (!isSha256Hex(payload.folderSubjectId)) addCode(blockers, 'applyEvent-folderSubjectId-invalid');
    if (postState.state !== 'present') addCode(blockers, 'applyEvent-postState-invalid');
    ...
    if (cleanString(payload.auditMaintenanceId) === '') addCode(blockers, 'applyEvent-auditMaintenanceId-required');
  ...
  async function buildApplyEventReceipt(bindingResult, blockers, warnings) {
    var sync = H2O.Desktop.Sync;
    if (!sync || typeof sync.buildBindingApplyEvent !== 'function') {
      addCode(blockers, 'binding-applyEvent-builder-unavailable');
      return null;
    }
    var built = null;
    try {
      built = safeObject(await sync.buildBindingApplyEvent({ bindingResult: bindingResult }));
    } catch (_) {
      addCode(blockers, 'binding-applyEvent-build-failed');
      return null;
    }
    ...
    if (built.ok !== true || !isObject(built.applyEvent)) {
      if (!blockers.length) addCode(blockers, 'binding-applyEvent-build-failed');
      return safeObject(built.applyEvent);
    }
    return safeObject(built.applyEvent);
  }

  async function finalizeBindingConvergence(input) {
    var args = safeObject(input);
    var bindingResult = safeObject(args.bindingResult || args.result);
    var blockers = [];
    var warnings = [];
    var sync = H2O.Desktop.Sync;
    ...
    validateBindingResult(bindingResult, blockers, warnings);

    var applyEvent = null;
    if (!blockers.length) {
      applyEvent = await buildApplyEventReceipt(bindingResult, blockers, warnings);
    }
    if (applyEvent && !blockers.length) {
      validateApplyEvent(
        applyEvent,
        cleanLower(bindingResult.subjectId),
        cleanString(bindingResult.lineageId),
        blockers,
        warnings
      );
    }
    ...
    var subjectId = cleanLower(bindingResult.subjectId || applyEvent.subjectId);
    var lineageId = cleanString(bindingResult.lineageId || applyEvent.lineageId);
    ...
    var consumedResult = safeObject(await sync.recordConsumedOperation({
      eventDigest: cleanLower(applyEvent.eventDigest),
      dedupeKey: cleanLower(applyEvent.dedupeKey),
      lineageId: lineageId,
      subjectId: subjectId,
      sourcePeerId: peerId,
      ...
      reason: 'local-binding-convergence-finalized',
      validationSummary: {
        ok: true,
    ...
    if (forbidden) {
      return failure(['binding-bookkeeping-output-contains-forbidden-field'], ['blocked-forbidden-key-' + forbidden], consumedRow, watermarkRow, null);
    }
    return output;
  }

  H2O.Desktop.Sync.finalizeBindingConvergence = finalizeBindingConvergence;
  H2O.Desktop.Sync.__bindingConvergenceBookkeepingInstalled = true;
  H2O.Desktop.Sync.__bindingConvergenceBookkeepingVersion = VERSION;
