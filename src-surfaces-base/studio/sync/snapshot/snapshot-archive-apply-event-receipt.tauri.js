/* H2O Desktop Sync - F14.4.8a snapshot archive applyEvent receipt builder
 *
 * Receipt-only builder for successful hypothetical Native-owned snapshot
 * archive operations. This module never executes Native/F5, applies,
...
(function (global) {
  ...
  if (H2O.Desktop.Sync.__snapshotArchiveApplyEventInstalled) return;

  var VERSION = '0.1.0-f14.4.8a';
  var RESULT_SCHEMA = 'h2o.desktop.sync.snapshot-archive-apply-event-receipt.v1';
  var ENVELOPE_SCHEMA = 'h2o.crossPlatform.envelope.v1';
  var CONSUMED_PREVIEW_SCHEMA = 'h2o.desktop.sync.snapshot-archive-consumed-operation-preview.v1';
  var WATERMARK_PREVIEW_SCHEMA = 'h2o.desktop.sync.snapshot-archive-watermark-target-preview.v1';
  ...
  var CAPABILITY_TAG = 'h2o.platform.capabilities.v1#f14.4.8a-desktop-snapshot-archive-apply-event-v1';
  var DEFAULT_PREDICATE = 'h2o.snapshot.archive.predicate.v1';
  var AUDIT_POLICY_VERSION = 'h2o.snapshot.native-owner-archive-receipt.v1';
  ...
  async function validateCandidate(envelope, row, blockers, warnings) {
    ...
    if (env.operation !== OP_ARCHIVE_PROPOSED) addCode(blockers, 'proposal-operation-not-archive');
    ...
    if (cleanString(transition.toState) !== 'archived') addCode(blockers, 'proposal-archive-target-invalid');
    if (expected.lifecycleState !== 'archived') addCode(blockers, 'proposal-expectedPostState-lifecycle-invalid');
    if (row) {
    ...
    return {
      ...
      applyOperation: OP_ARCHIVE_APPLIED,
  ...
  function validateOperationResult(operationResult, candidate, blockers, warnings) {
    ...
    if (op && op !== candidate.proposalOperation && op !== candidate.applyOperation && op !== 'archive') {
    ...
    if (typeof row.archived === 'boolean' && row.archived !== true) addCode(blockers, 'operation-result-archive-target-mismatch');
    if (cleanString(row.lifecycleState) && cleanString(row.lifecycleState) !== 'archived') addCode(blockers, 'operation-result-lifecycle-target-mismatch');
    ...
    try {
      replay = kernel.composeReplayDefense({
        candidate: {
          ...
          operation: candidate.applyOperation,
          operationKind: 'snapshot.archive.applyEvent',
          ...
          actorPeer: actorPeer,
          originTag: {
            originKind: 'owner-handoff-result',
  ...
  function proposedConsumedOperation(candidate, identity, actorPeer, eventDigest, appliedAt, validationSummary) {
    var row = {
      ...
      reason: 'snapshot archive applyEvent receipt generated',
      validationSummary: {
        ok: true,
  ...
  async function buildSnapshotArchiveApplyEventReceipt(input) {
    ...
    validateReplay(candidate, identity, actorPeer, preHash, postHash, blockers, warnings);
    if (blockers.length) return failure(blockers, warnings);
  ...
  H2O.Desktop.Sync.buildSnapshotArchiveApplyEventReceipt = buildSnapshotArchiveApplyEventReceipt;
  H2O.Desktop.Sync.__snapshotArchiveApplyEventInstalled = true;
  H2O.Desktop.Sync.__snapshotArchiveApplyEventVersion = VERSION;
