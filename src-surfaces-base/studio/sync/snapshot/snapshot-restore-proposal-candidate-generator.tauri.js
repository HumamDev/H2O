...
(function (global) {
  ...
  function buildKernelProposalShapes(args, warnings) {
    ...
    var lifecycleTransition = shapeWithKernel('shapeLifecycleTransition', {
      ...
      metadata: {
        operationIntent: OPERATION_INTENT,
        legacySourceState: cleanString(args.legacySourceState),
  ...
  async function generateSnapshotRestoreProposalCandidate(input) {
    ...
    var kernelShapes = buildKernelProposalShapes({
      ...
      targetHash: targetHash,
      operation: OPERATION,
      operationKind: 'snapshot.restore.proposal',
      fromState: snapshot.lifecycleState,
      fromStateContext: cleanString(snapshot.lifecycleState) === 'tombstoned' ? 'restorable' : '',
      toState: 'active',
      legacySourceState: snapshot.lifecycleState,
      legacyTargetState: TARGET_LIFECYCLE_STATE,
