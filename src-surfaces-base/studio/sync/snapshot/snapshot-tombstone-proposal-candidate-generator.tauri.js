/* H2O Desktop Sync - F14.4.5 snapshot tombstone proposal candidate generator
 *
...
(function (global) {
  ...
  if (H2O.Desktop.Sync.__snapshotTombstoneProposalInstalled) return;

  var VERSION = '0.1.0-f14.4.5';
  var RESULT_SCHEMA = 'h2o.desktop.sync.snapshot-tombstone-proposal-candidate-generator.v1';
  ...
  var OPERATION = 'snapshot-lifecycle-tombstone-proposed';
  var DOMAIN_OPERATION = 'tombstone';
  var OPERATION_INTENT = 'update';
  var STATUS_GENERATED = 'generated';
  var PREDICATE_VERSION = 'h2o.snapshot.tombstone.predicate.v1';
  var CAPABILITY_TAG = 'h2o.platform.capabilities.v1#f14.4.5-desktop-snapshot-tombstone-proposal-v1';
  ...
  function buildKernelProposalShapes(args, warnings) {
    ...
    var lifecycleTransition = shapeWithKernel('shapeLifecycleTransition', {
      ...
      metadata: {
        operationIntent: OPERATION_INTENT,
        legacyTargetState: cleanString(args.legacyTargetState),
        previewOnly: true
      }
  ...
  function normalizeSnapshotRecord(record) {
    ...
    var lifecycle = cleanString(next.lifecycleState || next.lifecycle_state || next.state || next.status).toLowerCase();
    if (lifecycle === 'captured') next.lifecycleState = 'active';
    return next;
  }
  function nonNoopBlockers(preflight) {
    return codeList(preflight && preflight.blockers).filter(function (code) {
      return code !== 'snapshot-tombstone-present';
    });
  }
  ...
  async function buildPreflightProofDigest(preflight) {
    ...
    var snapshot = safeObject(p.canonicalSnapshot);
    return sha256Hex({
      schema: 'h2o.desktop.sync.snapshot-tombstone-preflight-proof.v1',
      subjectType: SUBJECT_TYPE,
      subjectId: cleanLower(snapshot.subjectId),
      ...
      targetLifecycleState: 'tombstoned',
  ...
  async function expectedPostStateHash(subjectId, baseHash) {
    return sha256Hex({
      schema: 'h2o.desktop.sync.snapshot-tombstone-expected-post-state.v1',
      subjectType: SUBJECT_TYPE,
      subjectId: subjectId,
      baseHash: baseHash,
  ...
  function validateProposalEnvelope(envelope, blockers) {
    ...
    if (cleanString(safeObject(op.lifecycleTransition).toState) !== 'tombstoned') addCode(blockers, 'operation-intent-wrong-for-kind');
    if (expected.lifecycleState !== 'tombstoned') addCode(blockers, 'envelope-schema-too-new');
    ...
    try {
      var replay = kernel.composeReplayDefense({
        candidate: {
          ...
          subjectId: subjectId,
          operation: DOMAIN_OPERATION,
          operationKind: 'snapshot.tombstone',
  ...
  async function generateSnapshotTombstoneProposalCandidate(input) {
    ...
    if (requestedOperation !== DOMAIN_OPERATION) addCode(blockers, 'operation-not-tombstone');
    if (requestedOperation === 'archive') addCode(blockers, 'archive-not-supported');
    if (requestedOperation === 'restore') addCode(blockers, 'restore-not-supported');
    ...
    var normalizedSnapshotRecord = normalizeSnapshotRecord(args.snapshotRecord);
    var expectedTarget = isObject(args.expectedTarget)
      ? Object.assign({}, args.expectedTarget, { lifecycleState: 'tombstoned' })
      : { lifecycleState: 'tombstoned' };
    var preflight;
    try {
    ...
    if (preflight && preflight.noop === true && nonNoopBlockers(preflight).length === 0) {
      return noopResult(warnings, preflight);
    }
    ...
    if (preflight.operation !== DOMAIN_OPERATION) {
      addCode(blockers, 'preflight-operation-not-tombstone');
      return failure(blockers, warnings, { preflightSummary: summarizePreflight(preflight) });
    }
    ...
    if (lifecycle !== 'active' && lifecycle !== 'archived') addCode(blockers, 'snapshot-tombstone-transition-not-supported');
    ...
    var kernelShapes = buildKernelProposalShapes({
      ...
      targetHash: targetHash,
      operation: OPERATION,
      operationKind: 'snapshot.tombstone.proposal',
    ...
    var row = {
      ...
      targetState: { lifecycleState: 'tombstoned' },
  ...
  H2O.Desktop.Sync.generateSnapshotTombstoneProposalCandidate = generateSnapshotTombstoneProposalCandidate;
  H2O.Desktop.Sync.__snapshotTombstoneProposalInstalled = true;
  H2O.Desktop.Sync.__snapshotTombstoneProposalVersion = VERSION;
  H2O.Desktop.Sync.__snapshotTombstoneProposalLedgerKey = LEDGER_KEY;
