/* H2O Desktop Sync - F14.4.4 snapshot archive proposal candidate generator
 *
 ...
 * enqueues relay outbox rows, applies, restores, executes Native owner
...
(function (global) {
  ...
  if (H2O.Desktop.Sync.__snapshotArchiveProposalInstalled) return;

  var VERSION = '0.1.0-f14.4.4';
  var RESULT_SCHEMA = 'h2o.desktop.sync.snapshot-archive-proposal-candidate-generator.v1';
  ...
  var OPERATION = 'snapshot-lifecycle-archive-proposed';
  var DOMAIN_OPERATION = 'archive';
  var OPERATION_INTENT = 'update';
  var STATUS_GENERATED = 'generated';
  var PREDICATE_VERSION = 'h2o.snapshot.archive.predicate.v1';
  var CAPABILITY_TAG = 'h2o.platform.capabilities.v1#f14.4.4-desktop-snapshot-archive-proposal-v1';
  ...
  async function buildPreflightProofDigest(preflight) {
    ...
    var snapshot = safeObject(p.canonicalSnapshot);
    return sha256Hex({
      schema: 'h2o.desktop.sync.snapshot-archive-preflight-proof.v1',
      subjectType: SUBJECT_TYPE,
      subjectId: cleanLower(snapshot.subjectId),
  ...
  async function expectedPostStateHash(subjectId, baseHash) {
    return sha256Hex({
      schema: 'h2o.desktop.sync.snapshot-archive-expected-post-state.v1',
      subjectType: SUBJECT_TYPE,
      subjectId: subjectId,
      baseHash: baseHash,
  ...
  function validateProposalEnvelope(envelope, blockers) {
    ...
    if (cleanString(safeObject(op.lifecycleTransition).toState) !== 'archived') addCode(blockers, 'operation-intent-wrong-for-kind');
    if (expected.lifecycleState !== 'archived') addCode(blockers, 'envelope-schema-too-new');
  ...
  async function generateSnapshotArchiveProposalCandidate(input) {
    ...
    if (requestedOperation !== DOMAIN_OPERATION) addCode(blockers, 'operation-not-archive');
    ...
    var expectedTarget = isObject(args.expectedTarget)
      ? Object.assign({}, args.expectedTarget, { lifecycleState: 'archived' })
      : { lifecycleState: 'archived' };
    var preflight;
    try {
    ...
    if (cleanString(snapshot.lifecycleState) === 'archived') addCode(blockers, 'snapshot-already-archived');
  ...
  H2O.Desktop.Sync.generateSnapshotArchiveProposalCandidate = generateSnapshotArchiveProposalCandidate;
  H2O.Desktop.Sync.__snapshotArchiveProposalInstalled = true;
  H2O.Desktop.Sync.__snapshotArchiveProposalVersion = VERSION;
  H2O.Desktop.Sync.__snapshotArchiveProposalLedgerKey = LEDGER_KEY;
