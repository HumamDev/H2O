/* H2O Desktop Sync - F14.4.7a snapshot Native archive handoff preview
 *
 * Read-only handoff preview for generated snapshot archive proposal
 * candidates. This module validates and shapes a Native owner handoff request
 * only. It never executes a handoff, calls Native/F5, applies, publishes,
 * enqueues relay outbox rows, advances watermarks, or records consumed ops.
 */
(function (global) {
  'use strict';
  ...
  if (H2O.Desktop.Sync.__snapshotNativeArchiveHandoffInstalled) return;

  var VERSION = '0.1.0-f14.4.7a';
  var RESULT_SCHEMA = 'h2o.desktop.sync.snapshot-native-archive-handoff-preview.v1';
  var HANDOFF_REQUEST_SCHEMA = 'h2o.desktop.sync.snapshot-native-archive-handoff-request.v1';
  ...
  var OP_ARCHIVE = 'snapshot-lifecycle-archive-proposed';
  ...
  function validateCandidate(args, resolved, blockers, warnings) {
    ...
    if (envelope.operation !== OP_ARCHIVE) addCode(blockers, 'candidate-operation-not-archive');
    ...
    if (cleanString(safeObject(op.lifecycleTransition).toState) !== 'archived') {
    ...
    if (expected.lifecycleState !== 'archived') addCode(blockers, 'candidate-expectedPostState-lifecycle-invalid');
    ...
    var summary = {
      ...
      expectedTarget: { lifecycleState: 'archived' },
  ...
  function validateOwnerBoundary(args, summary, blockers, warnings) {
    ...
    if (typeof H2O.Desktop.Sync.runSnapshotNativeOwnerReachabilityProbe === 'function') {
      try {
        var reachability = H2O.Desktop.Sync.runSnapshotNativeOwnerReachabilityProbe({
        ...
        addCode(warnings, 'snapshot-native-owner-reachability-probe-threw');
      }
  ...
  function buildHandoffInput(args, owner, candidateSummary, createdAtIso) {
    return {
      ...
      handoffReason: 'snapshot-native-archive-owner-preview',
      createdAtIso: createdAtIso,
      expiresAtIso: cleanString(args.expiresAtIso),
  ...
  async function previewSnapshotNativeArchiveHandoff(input) {
  ...
  H2O.Desktop.Sync.previewSnapshotNativeArchiveHandoff = previewSnapshotNativeArchiveHandoff;
  H2O.Desktop.Sync.__snapshotNativeArchiveHandoffInstalled = true;
  H2O.Desktop.Sync.__snapshotNativeArchiveHandoffVersion = VERSION;
