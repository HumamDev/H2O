/* H2O Desktop Sync - F10.8.6g2 proposal publication
 *
 * Desktop/Tauri-only manual publisher for generated proposal candidates.
 *
 * Publication means:
 *   generated proposal candidate -> relay outbox row
 *
 * Safety invariants:
 *   - Proposal only. No conflictCandidate publication, upload, WebDAV, apply,
 *     applyEvent, convergence, remote mutation, automatic merge, timers,
 *     polling, or mobile write-back.
 *   - Publishes only an existing generated proposal candidate. It never creates
 *     a proposal candidate and never mutates candidate rows.
...
(function (global) {
  ...
  if (H2O.Desktop.Sync.__proposalPublicationInstalled) return;

  var RESULT_SCHEMA = 'h2o.desktop.sync.proposal-publication.v1';
  ...
  var VERSION = '0.1.0-f10.8.6g2';
  var APPROVAL_TOKEN = 'I_APPROVE_PROPOSAL_PUBLICATION';
  ...
  function validatePublicationHistory(ledger, row, blockers) {
    ...
    for (var i = 0; i < rows.length; i += 1) {
      var item = rows[i];
      if (cleanString(item.candidateKind) !== 'proposal') continue;
      ...
      if (sameCandidate && status === 'published') addCode(blockers, 'proposal-already-published');
      if (sameCandidate && status === 'withdrawn') addCode(blockers, 'proposal-withdrawn');
      if (sameCandidate && status === 'superseded') addCode(blockers, 'proposal-superseded');
      if (sameCandidate && status === 'expired') addCode(blockers, 'proposal-expired');
  ...
  function validateCandidate(row, envelope, blockers, warnings) {
    ...
    if (!Array.isArray(payload.justifyingEvidenceDigests) || !payload.justifyingEvidenceDigests.length) {
      addCode(blockers, 'proposal-justification-missing');
    }
    ...
    if (!cleanString(payload.predicateVersion)) addCode(blockers, 'proposal-predicate-missing');
  ...
  async function appendPublishedLedgerRow(row, enqueueResult, blockers, warnings) {
    ...
    try {
      var result = await sync.appendPublicationLedgerEvent({
        ...
        sourceLedgerKey: PROPOSAL_LEDGER_KEY,
  ...
  async function publishProposalCandidate(input) {
  ...
  H2O.Desktop.Sync.publishProposalCandidate = publishProposalCandidate;
  H2O.Desktop.Sync.__proposalPublicationInstalled = true;
  H2O.Desktop.Sync.__proposalPublicationVersion = VERSION;
  H2O.Desktop.Sync.__proposalPublicationApprovalToken = APPROVAL_TOKEN;
