// ==UserScript==
// @h2o-id             s0d3e.transcript.studio.host.studio
// @name               S0D3e. 🎬 Transcript Studio Host - Studio
...
(function studioH2OHost() {
  ...
  function getTurnsRoot() {
    return STATE.turnsEl && STATE.turnsEl.isConnected ? STATE.turnsEl : null;
  }
  ...
  function ensureReaderMarkers() {
    ...
    const turnsEl = STATE.turnsEl;
    const scrollEl = STATE.scrollEl;
    if (!root || !turnsEl || !scrollEl) return;

    try { root.setAttribute("data-h2o-studio-reader", "1"); } catch {}
    try { turnsEl.setAttribute("data-testid", "conversation-turns"); } catch {}
    try { scrollEl.setAttribute("data-scroll-root", "1"); } catch {}
  }

  function clearReaderMarkers() {
    const root = STATE.readerRoot;
    const scrollEl = STATE.scrollEl;
    try { root?.removeAttribute("data-h2o-studio-reader"); } catch {}
  ...
  function mount(opts = {}) {
    ...
    const turnsEl = opts.turnsEl || opts.turnRoot || opts.conversationRoot || null;
